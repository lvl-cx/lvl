RegisterNetEvent('arma-notify:client:SendAlert')
AddEventHandler('arma-notify:client:SendAlert', function(data)
	DoCustomHudText(data.type, data.text, data.length, data.style)
end)

RegisterNetEvent('arma-notify:client:PersistentHudText')
AddEventHandler('arma-notify:client:PersistentHudText', function(data)
	PersistentHudText(data.action, data.id, data.type, data.text, data.style)
end)

RegisterNetEvent('armaCUSTOMNOTIFY:notify')
AddEventHandler('armaCUSTOMNOTIFY:notify', function(type, message)
  exports['arma-notify']:SendAlert(type, message)
end)
