...
RegisterCommand("hs", function()
    ...
    if playHeadshotSounds then
        SentryNotify("~y~Experimental Hitmarkers Are Now ~g~Enabled")
    else
        SentryNotify("~y~Experimental Hitmarkers Are Now ~r~Disabled")
    end
end)



RegisterNetEvent("Sentry:Settings:Hitsounds")
AddEventHandler("Sentry:Settings:Hitsounds", function(bool)
...
RegisterNetEvent("Sentry:ComingSoon")
AddEventHandler("Sentry:ComingSoon", function()
    SentryNotify("~r~Error: ~w~This Feature Is Coming Soon")
end)

RegisterNetEvent("SentryCli:Notify")
AddEventHandler("SentryCli:Notify", function(txt)
    SentryNotify(txt)
end)
                    ...
                    if hit then
                        if headBones[bone] then
                            ...
                            --if GetSelectedPedWeapon(ped) == GetHashKey("weapon_stungun") then
                                --TriggerEvent("Sentry:PDSideTazer")
                            --end
...
AddEventHandler("Sentry:PlaySound", function(soundname)
