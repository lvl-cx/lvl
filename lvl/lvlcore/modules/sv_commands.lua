...
RegisterCommand("getmyid", function(source)
    ...
    --    template = '^7[LVL^7]:^7' .. " Perm ID: " .. LVL.getUserId(source) .. '</div>',
    --    args = { playerName, msg }
    --  })
end)
...
RegisterCommand("getmytempid", function(source)
    ...
    --TriggerClientEvent('chat:addMessage', source, {
    --    template = '^7[LVL^7]:^7' .. " Your Temp ID: " .. source,
    --    args = { playerName, msg }
    --  })
end)
...
RegisterCommand('getid', function(source, args)
    if args and args[1] then 
        ...
        if userid then 
            ...
            --TriggerClientEvent('chat:addMessage', source, {
            --    template = '^7[LVL^7]:^7' .. "This Users Perm ID is: " .. userid, "alert",
            --    args = { playerName, msg }
            --  })
        else 
            TriggerClientEvent('chatMessage', source, '^7[^1LVL^7]:', {255, 0, 0}, "Temp ID cannot be found! This user is most likely offline.", "alert")
            --TriggerClientEvent('chat:addMessage', source, {
            --    template = '^7[LVL^7]:^7' .. "Temp ID cannot be found! This user is most likely offline.", "alert",
            --    args = { playerName, msg }
            --  })
        ...
        --TriggerClientEvent('chat:addMessage', source, {
        --    template = '^7[LVL^7]:^7' .. "Please specify a user eg: /getid [tempid]", "alert",
        --    args = { playerName, msg }
        --  })
    end
end)
...
RegisterCommand("s", function(source,args, rawCommand)
    ...
    if LVL.hasPermission(user_id2, "admin.whitelisted") then
