...
AddEventHandler('NHSMenu:ClockOn', function(nhsrank)
    ...
    elseif LVL.hasGroup(user_id,nhsrank) then
...
AddEventHandler('NHSMenu:CheckPermissions', function()
    ...
    if LVL.hasPermission(user_id, "clockon.nhs") then
...
AddEventHandler('NHSMenu:ClockOff', function()
    ...
    if user_id == nil then
        LVLclient.notify(source,{"~r~You are a nil User ID, please relog."})
    elseif user_id ~= nil and LVL.hasGroup(user_id, "Head Chief Medical Officer Clocked") then
        LVL.removeUserGroup(user_id,"Head Chief Medical Officer Clocked")
        LVLclient.notify(source,{"You have clocked off"})
    elseif user_id ~= nil and LVL.hasGroup(user_id, "Assistant Chief Medical Officer Clocked") then
        LVL.removeUserGroup(user_id,"Assistant Chief Medical Officer Clocked")
        LVLclient.notify(source,{"You have clocked off"})
    elseif user_id ~= nil and LVL.hasGroup(user_id, "Deputy Chief Medical Officer Clocked") then
        LVL.removeUserGroup(user_id,"Deputy Chief Medical Officer Clocked")
        LVLclient.notify(source,{"You have clocked off"})
    elseif user_id ~= nil and LVL.hasGroup(user_id, "Captain Clocked") then
        LVL.removeUserGroup(user_id,"Captain Clocked")
        LVLclient.notify(source,{"You have clocked off"})
    elseif user_id ~= nil and LVL.hasGroup(user_id, "Consultant Clocked") then
        LVL.removeUserGroup(user_id,"Consultant Clocked")
        LVLclient.notify(source,{"You have clocked off"})
    elseif user_id ~= nil and LVL.hasGroup(user_id, "Specialist Clocked") then
        LVL.removeUserGroup(user_id,"Specialist Clocked")
        LVLclient.notify(source,{"You have clocked off"})
    elseif user_id ~= nil and LVL.hasGroup(user_id, "Senior Doctor Clocked") then
        LVL.removeUserGroup(user_id,"Senior Doctor Clocked")
        LVLclient.notify(source,{"You have clocked off"})
    elseif user_id ~= nil and LVL.hasGroup(user_id, "Junior Doctor Clocked") then
        LVL.removeUserGroup(user_id,"Junior Doctor Clocked")
        LVLclient.notify(source,{"You have clocked off"})
    elseif user_id ~= nil and LVL.hasGroup(user_id, "Critical Care Paramedic Clocked") then
        LVL.removeUserGroup(user_id,"Critical Care Paramedic Clocked")
        LVLclient.notify(source,{"You have clocked off"})
    elseif user_id ~= nil and LVL.hasGroup(user_id, "Paramedic Clocked") then
        LVL.removeUserGroup(user_id,"Paramedic Clocked")
        LVLclient.notify(source,{"You have clocked off"})
    elseif user_id ~= nil and LVL.hasGroup(user_id, "Trainee Paramedic Clocked") then
        LVL.removeUserGroup(user_id,"Trainee Paramedic Clocked")
        LVLclient.notify(source,{"You have clocked off"})
    end
    if #(coords - comparison) > 20 then
