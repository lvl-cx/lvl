...
-- [RageUI Menu]
RMenu.Add('SellerMenu', 'main', RageUI.CreateMenu("", "~w~LVL Seller",1300, 50, "seller", "seller"))

RageUI.CreateWhile(1.0, true, function()
  if RageUI.Visible(RMenu:Get('SellerMenu', 'main')) then
      RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
    
    RageUI.Button("Sell Gold" , nil, {RightLabel = "~w~→"}, true, function(Hovered, Active, Selected)
        if Selected then
    ...
    RageUI.Button("Sell Diamond" , nil, {RightLabel = "~w~→"}, true, function(Hovered, Active, Selected)
      if Selected then
