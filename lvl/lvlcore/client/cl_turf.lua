...
RegisterNetEvent('LVL:TakenTurf')
AddEventHandler('LVL:TakenTurf', function(isnTurf)
...
RegisterNetEvent('LVL:OutOfZone')
AddEventHandler('LVL:OutOfZone', function(isnTurf)
	inTurf = false
...
RegisterNetEvent('LVL:PlayerDied')
...
RegisterNetEvent('LVL:TurfComplete')
AddEventHandler('LVL:TurfComplete', function(reward, name)
	inTurf = false
	secondsRemaining = 0
...
RegisterNetEvent("LVL:DontIt")
AddEventHandler("LVL:DontIt", function(bool)
	TakenTurf = bool 
end)

RegisterNetEvent("LVL:DontItWeed")
AddEventHandler("LVL:DontItWeed", function(bool)
	TakenTurfWeed = bool 
end)

RegisterNetEvent("LVL:DontItCocaine")
AddEventHandler("LVL:DontItCocaine", function(bool)
	TakenTurfCocaine = bool 
end)


RegisterNetEvent("LVL:DontItLSD")
AddEventHandler("LVL:DontItLSD", function(bool)
	TakenTurfLSD = bool 
end)

RegisterNetEvent("LVL:DontItHeroin")
AddEventHandler("LVL:DontItHeroin", function(bool)
...
function changeComision()
    ...
    if (GetOnscreenKeyboardResult()) then
		...
		if  resultID then
            TriggerServerEvent("LVL:ChangeCommision", tonumber(resultID))
...
function changeComision2()
    ...
    if (GetOnscreenKeyboardResult()) then
        local resultID = GetOnscreenKeyboardResult()
		if  resultID then
            TriggerServerEvent("LVL:ChangeCommisionWeed", tonumber(resultID))
...
function changeComision3()
    ...
    if (GetOnscreenKeyboardResult()) then
        local resultID = GetOnscreenKeyboardResult()
		if  resultID then
            TriggerServerEvent("LVL:ChangeCommisionCocaine", tonumber(resultID))
...
function changeComision4()
    ...
    if (GetOnscreenKeyboardResult()) then
        local resultID = GetOnscreenKeyboardResult()
		if  resultID then
            TriggerServerEvent("LVL:ChangeCommisionLSD", tonumber(resultID))
...
function changeComision5()
    ...
    if (GetOnscreenKeyboardResult()) then
        local resultID = GetOnscreenKeyboardResult()
		if  resultID then
            TriggerServerEvent("LVL:ChangeCommisionHeroin", tonumber(resultID))
				...
				if not inTurf then
					...
					if (Vdist(pos.x, pos.y, pos.z, pos3.x, pos3.y, pos3.z) < 1.4) then
						...
						if (IsControlJustReleased(1, 51)) then
							TriggerServerEvent('LVL:TakeTurf', k)
...
RegisterNetEvent("LVL:MakeTurfTrue")
AddEventHandler("LVL:MakeTurfTrue", function(isit)
	if isit then
		inTurf = true
	else 
		inTurf = false 
