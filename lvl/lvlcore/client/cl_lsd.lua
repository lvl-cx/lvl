          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                TriggerServerEvent('LVL:GatherLSD')
                
                exports.rprogress:Start("", 5000)
                ClearPedTasksImmediately(ped)
                Action = false
              else
                LVL.notify({"~r~You cant gather LSD while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TriggerServerEvent('LVL:ProcessLSD')
...
-- [RageUI Menu]
RMenu.Add('LSDMenu', 'LSD Seller', RageUI.CreateMenu("", "~w~LVL LSD Seller",1300, 50, "lsd", "lsd"))

RageUI.CreateWhile(1.0, true, function()
  if RageUI.Visible(RMenu:Get('LSDMenu', 'LSD Seller')) then
      RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
    
    RageUI.Button("Sell LSD" , nil, {RightLabel = "~w~→"}, true, function(Hovered, Active, Selected)
        if Selected then
