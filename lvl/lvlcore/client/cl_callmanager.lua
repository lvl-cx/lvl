...
RMenu.Add('callmanager', 'main', RageUI.CreateMenu("", '~w~LVL Call Manager', 1300, 50, 'manager', 'manager'))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('callmanager', 'main')) then
        ...
        if isPlayerAdmin then
            RageUI.Button("Admin Tickets", nil, {RightLabel = "~w~→"}, true, function(Hovered, Active, Selected)
                if (Hovered) then
        ...
        if isPlayerPD then 
            RageUI.Button("Police Calls", nil, {RightLabel = "~w~→"}, true, function(Hovered, Active, Selected)
                if (Hovered) then
        ...
        if isPlayerNHS then
            RageUI.Button("NHS Calls", nil, {RightLabel = "~w~→"}, true, function(Hovered, Active, Selected)
                if (Hovered) then
    ...
    if RageUI.Visible(RMenu:Get("callmanager", "admin")) then
        ...
        if adminCalls ~= nil then
            for k,v in pairs(adminCalls) do
                RageUI.Button(string.format("[%s] %s" .. "  :  " .. v[3], v[2], v[1]), nil, {RightLabel = "~w~→"}, true, function(Hovered, Active, Selected)
                    if (Selected) then
                        if v[2] == GetPlayerServerId(PlayerId()) then
                            ...
                            if not isInTicket then
                                    savedCoords = GetEntityCoords(PlayerPedId())
                                        ...
                                        TriggerServerEvent("LVL:returnMe", v[1], v[2], v[3])
    ...
    if RageUI.Visible(RMenu:Get("callmanager", "nhs")) then
        ...
        if nhsCalls ~= nil then
            for k,v in pairs(nhsCalls) do
                RageUI.Button(string.format("[ %s ] %s" .. "  :  " .. v[3], v[2], v[1]), "Press ~r~[ENTER] ~w~To accept  ~r~" .. v[1] .. "'s ~w~call!", {RightLabel = "~w~→"}, true, function(Hovered, Active, Selected)
                    if (Selected) then
                        if v[2] == GetPlayerServerId(PlayerId()) then
                            notify("~r~You can't take your own Call!")
                        else
...
AddEventHandler("staffon", function()
    ...
    TriggerEvent("LVL:OMioDioMode",true)
    isInTicket = true
end)
