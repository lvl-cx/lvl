...
RMenu.Add('Ammo', 'main', RageUI.CreateMenu("", "~b~LVL " .. ammo.name, 1300, 50, ammo.banner, ammo.banner))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("Ammo", "confirm")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Separator("Ammo Price: ~b~£" .. getMoneyStringFormatted(cPrice * Index), function() end)
            RageUI.Separator("Current Trader: ~b~" .. ammo.name, function() end)
