          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                TriggerServerEvent('LVL:GatherWeed')
                
                exports.rprogress:Start("", 5000)
                ClearPedTasksImmediately(ped)
                Action = false
              else
                LVL.notify({"~r~You cant gather Weed while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TriggerServerEvent('LVL:ProcessWeed')
...
-- [RageUI Menu]
RMenu.Add('WeedMenu', 'Weed Seller', RageUI.CreateMenu("", "~w~LVL Weed Seller",1300, 50, "weed", "weed"))

RageUI.CreateWhile(1.0, true, function()
  if RageUI.Visible(RMenu:Get('WeedMenu', 'Weed Seller')) then
      RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
    
    RageUI.Button("Sell Weed" , nil, {RightLabel = "~w~→"}, true, function(Hovered, Active, Selected)
        if Selected then
 
            TriggerServerEvent('LVL:SellWeed')
        end
    end)
