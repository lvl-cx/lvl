          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                TriggerServerEvent('LVL:GatherHeroin')
                
                exports.rprogress:Start("", 5000)
                ClearPedTasksImmediately(ped)
                Action = false
              else
                LVL.notify({"~r~You cant gather Heroin while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TriggerServerEvent('LVL:ProcessHeroin')
...
-- [RageUI Menu]
RMenu.Add('HeroinMenu', 'Heroin Seller', RageUI.CreateMenu("", "~w~LVL Heroin Seller",1300, 50, "heroin", "heroin"))

RageUI.CreateWhile(1.0, true, function()
  if RageUI.Visible(RMenu:Get('HeroinMenu', 'Heroin Seller')) then
      RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
    
    RageUI.Button("Sell Heroin" , nil, {RightLabel = "~w~→"}, true, function(Hovered, Active, Selected)
        if Selected then
 
            TriggerServerEvent('LVL:SellHeroin')
        end
    end)
