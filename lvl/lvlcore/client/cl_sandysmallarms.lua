...
RMenu.Add('SandySmallArms', 'main', RageUI.CreateMenu("", "~w~LVL " .. sandysmall.name, 1300, 50, sandysmall.banner, sandysmall.banner))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('SandySmallArms', 'main')) then
        ...
        RageUI.Button("Level 1 Armour ~w~[25%]" , nil, {RightLabel = "~w~£25,000"}, true, function(Hovered, Active, Selected)
            if Selected then
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("SandySmallArms", "confirm")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Separator("Current Gunstore: ~w~" .. sandysmall.name, function() end)
        ...
        RageUI.Button("Decline" , nil, {RightLabel = "~w~→"}, true, function(Hovered, Active, Selected) end, RMenu:Get("SandySmallArms", "main"))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("SandySmallArms", "confirma")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            RageUI.Separator("Armour Plate: ~w~" .. 'Level 1 [25%]', function() end)
            RageUI.Separator("Armour Plate Price: ~w~£" .. '25,000', function() end)
            RageUI.Separator("Current Gunstore: ~w~" .. sandysmall.name, function() end)
