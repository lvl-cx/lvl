...
RMenu.Add('LVLStores', 'main', RageUI.CreateMenu("", "~b~LVL Shop", 1300, 50, 'shop', 'shop'))
    ...
    if RageUI.Visible(RMenu:Get("LVLStores", "main")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for k, v in pairs(cfg.shopItems) do
                RageUI.Button(v.name, nil, {RightLabel = "~b~£".. getMoneyStringFormatted(v.price)}, true, function(Hovered, Active, Selected)
                    if Selected then
                        cPrice = v.price
                        cHash = v.itemID
                        cName = v.name
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("LVLStores", "confirm")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
        RageUI.Separator("Item Name: ~b~" .. cName, function() end)
        RageUI.Separator("Item Price: ~b~£" .. getMoneyStringFormatted(cPrice * Index), function() end)
        RageUI.Separator("Item Description: ~b~" .. cDescription, function() end)
        ...
        RageUI.Button("Confirm Purchase" , nil, {RightLabel = "~b~→"}, true, function(Hovered, Active, Selected)
            if Selected then
