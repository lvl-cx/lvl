...
RMenu.Add('adminmenu', 'main', RageUI.CreateMenu("", "~g~LVL Admin Menu", 1300, 50, 'admin', 'admin'))
...
RMenu.Add("adminmenu", "settings", RageUI.CreateSubMenu(RMenu:Get("adminmenu", "main"),"","~g~LVL Admin Menu", 1300, 50,"admin","admin"))
...
RMenu.Add('SettingsMenu', 'MainMenu', RageUI.CreateMenu("", "~g~LVL Settings Menu", 1300, 50,"settings","settings")) 
...
Citizen.CreateThread(function()
    while true do
        while streetnames do
            ...
            drawTxt2(UI.Left_x + 0.003 , UI.Bottom_y - 0.227 , 0.4, "~g~"..Zone.." / ~w~"..GetStreetNameFromHashKey(rua), 255, 255, 255, 255, 0) -- Street
            Citizen.Wait(1)
        end
        Citizen.Wait(1)
    end
                ...
                RageUI.Button("Fix Vehicle", nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected)
                    if Selected then
                        ...
                        if IsPedInAnyVehicle(playerPed, false) then
                            ...
                            tLVL.notify("~g~Your vehicle has been fixed!")
                        else
                ...
                RageUI.Button("Clean Vehicle", nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected)
                    if Selected then
                        ...
                        if IsPedInAnyVehicle(playerPed, false) then
                            ...
                            tLVL.notify("~g~Your vehicle has been cleaned!")
                        else
...
Citizen.CreateThread(function()
    while (true) do
        ...
        if InSpectatorMode then
            ...
            Draw2DText(0.22, 0.86, 'Armor: ~g~' .. GetPedArmour(targetPed) .. " / " .. GetPlayerMaxArmour(targetPlayerId), 0.65)
			for i=0, 32, 1 do
				if i ~= PlayerId() then
...
AddEventHandler('LVLAdmin:Spectate', function(plr,tpcoords)
    ...
    if not Spectating then
        ...
        tLVL.notify('~g~Spectating Player.')
    else 
