...
function tLVL.toggleHandcuff()
  ...
  if handcuffed then
    ...
    tLVL.playAnim(true,{{"mp_arresting","idle",1}},true)
  else
    DeleteEntity(cuffs)
    tLVL.stopAnim(true)
...
function tLVL.setHandcuffed(flag)
  if handcuffed ~= flag then
    tLVL.toggleHandcuff()
  end
end

function tLVL.isHandcuffed()
  return handcuffed
end

-- (experimental, based on experimental getNearestVehicle)
function tLVL.putInNearestVehicleAsPassenger(radius)
...
function tLVL.putInNetVehicleAsPassenger(net_veh)
...
function tLVL.putInVehiclePositionAsPassenger(x,y,z)
  local veh = tLVL.getVehicleAtPosition(x,y,z)
  if IsEntityAVehicle(veh) then
    for i=1,GetVehicleMaxNumberOfPassengers(veh) do
      if IsVehicleSeatFree(veh,i) then
        SetPedIntoVehicle(GetPlayerPed(-1),veh,i)
...
function tLVL.jail(x,y,z,radius)
  tLVL.teleport(x,y,z) -- teleport to center
  jail = {x+0.0001,y+0.0001,z+0.0001,radius+0.0001}
end

-- unjail the player
function tLVL.unjail()
  jail = nil
end

function tLVL.isJailed()
  return jail ~= nil
end
...
function tLVL.applyWantedLevel(new_wanted)
  Citizen.CreateThread(function()
...
Citizen.CreateThread(function()
  while true do
    ...
    if nwanted_level ~= wanted_level then
      ...
      LVLserver.updateWantedLevel({wanted_level})
    end
  end
end)
