...
function tLVL.addBlip(x,y,z,idtype,idcolor,text)
...
-- return native id
function tLVL.setNamedBlip(name,x,y,z,idtype,idcolor,text)
  tLVL.removeNamedBlip(name) -- remove old one

  named_blips[name] = tLVL.addBlip(x,y,z,idtype,idcolor,text)
  return named_blips[name]
end

-- remove a named blip
function tLVL.removeNamedBlip(name)
  if named_blips[name] ~= nil then
    tLVL.removeBlip(named_blips[name])
...
-- return marker id
function tLVL.addMarker(x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
...
-- return id
function tLVL.setNamedMarker(name,x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
  tLVL.removeNamedMarker(name) -- remove old marker

  named_markers[name] = tLVL.addMarker(x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
  return named_markers[name]
end

function tLVL.removeNamedMarker(name)
  if named_markers[name] ~= nil then
    tLVL.removeMarker(named_markers[name])
...
function tLVL.setArea(name,x,y,z,radius,height)
...
-- locked: boolean
-- doorswing: -1 to 1
function tLVL.setStateOfClosestDoor(doordef, locked, doorswing)
...
function tLVL.openClosestDoor(doordef)
  tLVL.setStateOfClosestDoor(doordef, false, 0)
end

function tLVL.closeClosestDoor(doordef)
  tLVL.setStateOfClosestDoor(doordef, true, 0)
end