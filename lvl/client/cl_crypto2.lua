...
RMenu.Add('CryptoMiner', 'main', RageUI.CreateMenu("", "~b~LVL Crypto Systems",1300, 50, 'crypto', 'crypto'))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('CryptoMiner', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            RageUI.Button("Buy Systems", nil, {RightLabel = '~b~→'}, true, function(Hovered, Active, Selected) 
                if Selected then end
            end, RMenu:Get("CryptoMiner", "buy_systems"))

            RageUI.Button("Manage Systems", nil, {RightLabel = '~b~→'}, true, function(Hovered, Active, Selected) 
                if Selected then 
                    TriggerServerEvent("LVLCrypto:CRYPTO:ReceiveMiners")
                end
    ...
    if RageUI.Visible(RMenu:Get('CryptoMiner', 'buy_systems')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for i=1, #cfg.systems do
                RageUI.Button(cfg.systems[i].name, nil, {RightLabel = '~b~£' .. tostring(getMoneyStringFormatted(cfg.systems[i].price))}, true, function(Hovered, Active, Selected)
                    if Selected then 
    ...
    if RageUI.Visible(RMenu:Get('CryptoMiner', 'buy_systems_manage')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Button("Buy", nil, {}, true, function(Hovered, Active, Selected)
                if Selected then 
                    TriggerServerEvent('LVLCrypto:buy_crypto_system', selected_system)
                end
    ...
    if RageUI.Visible(RMenu:Get('CryptoMiner', 'manage_systems')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for i=1, #machines_owned do
                RageUI.Button('Machine ID: '..machines_owned[i].machineid, nil, {RightLabel = '~b~' .. cfg.systems[machines_owned[i].pc_id].name}, true, function(Hovered, Active, Selected)
                    if Selected then 
    ...
    if RageUI.Visible(RMenu:Get('CryptoMiner', 'manage_systems_sub')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.Button("Withdraw BTC - ~b~[" .. '£'..roundToWholeNumber((selected_managed_system.amountmined) * cryptoprice) .. ']', nil, {}, true, function(Hovered, Active, Selected)
                if Selected then 
                    TriggerServerEvent('LVLCrypto:Withdraw:Crypto', selected_managed_system)
                end
            end, RMenu:Get("CryptoMiner", "main"))
            RageUI.Button("Sell Machine - ~b~[£" .. roundToWholeNumber(cfg.systems[selected_managed_system.pc_id].price / 4) .. ']',nil, {}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent('LVLCrypto:Sell:System', selected_managed_system)
                end
            end, RMenu:Get("CryptoMiner", "main"))
...
RegisterNetEvent('LVLCryptocore:setCryptoPrice', function(price)
    cryptoprice = price
end)

RegisterNetEvent('LVLCrypto:CRYPTO:SetMiners', function(miners)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1000)
        TriggerServerEvent('LVLCryptocore:getCryptoPrice')
    end
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        if isInArea(v3, 10.0) then 
            Draw3DText(v3, '~r~BTC: ~b~'..cryptoprice) 
        end
        Citizen.Wait(0)
    end
end)
