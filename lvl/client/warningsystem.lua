...
RegisterNetEvent("lvl:showWarningsOfUser")
AddEventHandler("lvl:showWarningsOfUser",function(lvlwarningstables)
	showWarningSystem = true
	lvlWarnings = lvlwarningstables
end)

RegisterNetEvent("lvl:recievedRefreshedWarningData")
AddEventHandler("lvl:recievedRefreshedWarningData",function(lvlwarningstables)
	lvlWarnings = lvlwarningstables
end)
...
RegisterCommand('warnings', function()
    ...
    if showWarningSystem then
        TriggerServerEvent("lvl:refreshWarningSystem")
    end
end)
...
Citizen.CreateThread(function()
	while true do
		if showWarningSystem then
			...
			DrawAdvancedText(0.59, 0.198, 0.005, 0.0028, 0.619, 'LVL' .. ' Warnings', 255, 255, 255, 255, 7, 0)
			...
			for warningID,warningTable in pairs(lvlWarnings) do
...
RegisterCommand("warn",function()
	...
	TriggerServerEvent("lvl:warnPlayer",tonumber(userIDtoWarn),userWarningMessage)
end)
