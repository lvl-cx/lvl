...
Citizen.CreateThread(function()
    while true do
        ...
        if noclip then
            ...
            local dx, dy, dz = tLVL.getCamDirection()
...
RegisterNetEvent("LVL:OMioDioMode")
AddEventHandler("LVL:OMioDioMode",function(DioMode)
	...
	if not OMioDioMode then
		...
		tLVL.setCustomization(adminTicketSavedCustomization)
		tLVL.giveWeapons(savedAdminTicketGuns,true)
        TriggerServerEvent("hello", false)

	else
        adminTicketSavedCustomization = tLVL.getCustomization()
		savedAdminTicketGuns = tLVL.getWeapons()
