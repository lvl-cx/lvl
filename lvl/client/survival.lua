...
function tLVL.setArmour(armour)
...
Citizen.CreateThread(function() -- coma thread
    ...
    while true do
        ...
        if IsEntityDead(GetPlayerPed(-1)) and not in_coma then --Wait for death check
            ...
            LVLserver.StoreWeaponsDead()
            LVLserver.Coma()
            TriggerEvent('LVL:IsInMoneyComa', true)
            TriggerServerEvent('LVL:InComa')
            LVLserver.MoneyDrop()
            TriggerEvent('LVL:3Seconds')
...
RegisterNetEvent("LVL:3Seconds")
AddEventHandler("LVL:3Seconds", function()
    DeathAnim = 3
    while DeathAnim <= 3 and DeathAnim >= 0 do
        Wait(1000)
        DeathAnim = DeathAnim - 1
...
function tLVL.disableComa()
    in_coma = false
end

function tLVL.isInComa()
    return in_coma
end

RegisterNetEvent("LVL:FixClient")
AddEventHandler("LVL:FixClient", function()
    ...
    Citizen.CreateThread(function()
        while true do 
            ...
            if resurrectspamm == true then
                ...
                tLVL.stopScreenEffect(cfg.coma_effect)
                DoScreenFadeIn(500)
                Citizen.Wait(500)
            end 
        end
...
function tLVL.reviveComa()
    local ped = GetPlayerPed(-1)
    SetEntityInvincible(ped,false)
    tLVL.setRagdoll(false)
    tLVL.stopScreenEffect(cfg.coma_effect)
    SetEntityHealth(GetPlayerPed(-1), 200) 
end

-- kill the player if in coma
function tLVL.killComa()
    if in_coma then
        coma_left = 0
    end
end
...
function tLVL.varyHealth(variation)
    local ped = GetPlayerPed(-1)
...
function tLVL.getHealth()
    return GetEntityHealth(GetPlayerPed(-1))
end

function tLVL.getArmour()
    return GetPedArmour(GetPlayerPed(-1))
end

function tLVL.setHealth(health)
    local n = math.floor(health)
    SetEntityHealth(GetPlayerPed(-1),n)
end


function tLVL.setFriendlyFire(flag)
    NetworkSetFriendlyFireOption(flag)
    SetCanAttackFriendly(GetPlayerPed(-1), flag, flag)
end

function tLVL.setPolice(flag)
