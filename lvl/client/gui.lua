...
-- pause
AddEventHandler("LVL:pauseChange", function(paused)
  SendNUIMessage({act="pause_change", paused=paused})
end)

-- MENU

function tLVL.openMenuData(menudata)
  SendNUIMessage({act="open_menu", menudata = menudata})
end

function tLVL.closeMenu()
  SendNUIMessage({act="close_menu"})
end

-- PROMPT

function tLVL.prompt(title,default_text)
...
function tLVL.request(id,text,time)
...
RegisterNUICallback("menu",function(data,cb)
  if data.act == "close" then
    LVLserver.closeMenu({data.id})
  elseif data.act == "valid" then
    LVLserver.validMenuChoice({data.id,data.choice,data.mod})
  end
end)
...
RegisterNUICallback("prompt",function(data,cb)
  if data.act == "close" then
    SetNuiFocus(false)
    SetNuiFocus(false)
    LVLserver.promptResult({data.result})
  end
end)
...
RegisterNUICallback("request",function(data,cb)
  if data.act == "response" then
    LVLserver.requestResult({data.id,data.ok})
  end
end)
...
function tLVL.announce(background,content)
...
-- try to fix missing cfg issue (cf: https://github.com/ImagicTheCat/LVL/issues/89)
for i=1,5 do
...
function tLVL.setProgressBar(name,anchor,text,r,g,b,value)
...
function tLVL.setProgressBarValue(name,value)
  SendNUIMessage({act="set_pbar_val", name = name, value = value})
end

-- set progress bar text
function tLVL.setProgressBarText(name,text)
  SendNUIMessage({act="set_pbar_text", name = name, text = text})
end

-- remove a progress bar
function tLVL.removeProgressBar(name)
...
Citizen.CreateThread(function()
  while true do
    ...
    -- open general menu
    if IsControlJustPressed(table.unpack(cfg.controls.phone.open)) and (not tLVL.isInComa() or not cfg.coma_disable_menu) and (not tLVL.isHandcuffed() or not cfg.handcuff_disable_menu) and GetLastInputMethod(0) then 
      LVLserver.openMainMenu({})
    end
    ...
    if pause_menu and not paused then
      paused = true
      TriggerEvent("LVL:pauseChange", paused)
