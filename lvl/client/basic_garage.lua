...
function tLVL.spawnGarageVehicle(vtype, name, pos) -- vtype is the vehicle type (one vehicle per type allowed at the same time)
    ...
    if AllowMoreThenOneCar or vehicle == nil then
        ...
        tLVL.notify("You can only have one Veh type: " .. vtype .. " of vehicle out.")
    end
end

function tLVL.despawnGarageVehicle(vtype, max_range)
    local vehicle = vehicles[vtype]
    if vehicle then
        ...
        if GetDistanceBetweenCoords(x, y, z, px, py, pz, true) < max_range then -- check distance with the vehicule
            ...
            Citizen.InvokeNative(0xEA386986E786A54F, Citizen.PointerValueIntInitialized(vehicle[3]))
            vehicles[vtype] = nil
            tLVL.notify("Vehicle stored.")
        else
            tLVL.notify("Too far away from the vehicle.")
        end
    end
end
...
Citizen.CreateThread(function()
  ...
  for k,v in pairs(vehicles) do
    ...
    elseif v.pos then -- not valid, respawn if with a valid position
      print("[LVL] invalid vehicle "..v[1]..", respawning...")
      tLVL.spawnGarageVehicle(v[1], v[2], v.pos)
    end
  end
end)
--]]
...
function tLVL.getNearestVehicle(radius)
    local x, y, z = tLVL.getPosition()
...
function tLVL.fixeNearestVehicle(radius)
    local veh = tLVL.getNearestVehicle(radius)
    if IsEntityAVehicle(veh) then
        SetVehicleFixed(veh)
    end
end

function tLVL.replaceNearestVehicle(radius)
    local veh = tLVL.getNearestVehicle(radius)
    if IsEntityAVehicle(veh) then
...
function tLVL.getVehicleAtPosition(x, y, z)
    x = x + 0.0001
    y = y + 0.0001
    z = z + 0.0001
...
function tLVL.getNearestOwnedVehicle(radius)
    local px, py, pz = tLVL.getPosition()
    for k, v in pairs(vehicles) do
...
-- return ok,x,y,z
function tLVL.getAnyOwnedVehiclePosition()
    for k, v in pairs(vehicles) do
...
-- return x,y,z
function tLVL.getOwnedVehiclePosition(vtype)
    local vehicle = vehicles[vtype]
    local x, y, z = 0, 0, 0
...
function tLVL.getOwnedVehicleId(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
...
-- vehicle commands
function tLVL.vc_openDoor(vtype, door_index)
    local vehicle = vehicles[vtype]
    if vehicle then
...
function tLVL.vc_closeDoor(vtype, door_index)
    local vehicle = vehicles[vtype]
    if vehicle then
...
function tLVL.vc_detachTrailer(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
...
function tLVL.vc_detachTowTruck(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
...
function tLVL.vc_detachCargobob(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
...
function tLVL.vc_toggleEngine(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
...
function tLVL.vc_toggleLock(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
        local veh = vehicle[3]
        ...
        if locked then -- unlock
            ...
            tLVL.notify("~r~Vehicle unlocked.")
        else -- lock
