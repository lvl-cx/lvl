...
Tunnel.bindInterface("LVL",tLVL)

-- get server interface
LVLserver = Tunnel.getInterface("LVL","LVL")

-- add client proxy interface (same as tunnel interface)
Proxy.addInterface("LVL",tLVL)

-- functions


function tLVL.teleport(x,y,z)
  tLVL.unjail() -- force unjail before a teleportation
  SetEntityCoords(GetPlayerPed(-1), x+0.0001, y+0.0001, z+0.0001, 1,0,0,1)
  LVLserver.updatePos({x,y,z})
end

-- return x,y,z
function tLVL.getPosition()
  local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
  return x,y,z
end

--returns the distance between 2 coordinates (x,y,z) and (x2,y2,z2)
function tLVL.getDistanceBetweenCoords(x,y,z,x2,y2,z2)
...
function tLVL.addPlayer(player)
  players[player] = true
end

function tLVL.removePlayer(player)
  players[player] = nil
end

function tLVL.getNearestPlayers(radius)
  local r = {}

  local ped = GetPlayerPed(i)
  local pid = PlayerId()
  local px,py,pz = tLVL.getPosition()
...
function tLVL.getNearestPlayer(radius)
  local p = nil

  local players = tLVL.getNearestPlayers(radius)
...
function tLVL.notify(msg)
  SetNotificationTextEntry("STRING")
  AddTextComponentString(msg)
  DrawNotification(true, false)
end

function tLVL.notifyPicture(icon, type, sender, title, text)
    SetNotificationTextEntry("STRING")
...
function tLVL.playScreenEffect(name, duration)
...
-- name, see https://wiki.fivem.net/wiki/Screen_Effects
function tLVL.stopScreenEffect(name)
  StopScreenEffect(name)
end
...
-- looping: if true, will LVLly loop the first element of the sequence until stopAnim is called
function tLVL.playAnim(upper, seq, looping)
  if seq.task ~= nil then -- is a task (cf https://github.com/ImagicTheCat/LVL/pull/118)
    tLVL.stopAnim(true)
...
function tLVL.playSpatializedSound(dict,name,x,y,z,range)
  PlaySoundFromCoord(-1,name,x+0.0001,y+0.0001,z+0.0001,dict,0,range+0.0001,0)
end

-- play sound
function tLVL.playSound(dict,name)
  PlaySound(-1,name,dict,0,0,1)
end

function tLVL.playFrontendSound(dict, name)
  PlaySoundFrontend(-1, dict, name, 0)
end

--[[
-- not working
function tLVL.setMovement(dict)
  if dict then
...
AddEventHandler("playerSpawned",function()
  TriggerServerEvent("LVLcli:playerSpawned")
end)

AddEventHandler("onPlayerDied",function(player,reason)
  TriggerServerEvent("LVLcli:playerDied")
end)
...
TriggerServerEvent('LVL:CheckID')

RegisterNetEvent('LVL:CheckIdRegister')
AddEventHandler('LVL:CheckIdRegister', function()
