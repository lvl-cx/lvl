...
RMenu.Add('PDArmoury', 'main', RageUI.CreateMenu("", "~b~LVL " .. armoury.name, 1300, 50, armoury.banner, armoury.banner))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('PDArmoury', 'main')) then
        ...
        for i , p in pairs(armoury.guns) do 
            RageUI.Button(p.name , nil, { RightLabel = '~b~→'}, true, function(Hovered, Active, Selected)
                if Selected then
        ...
        RageUI.Button("Level 4 Armour ~b~[100%]" , nil, {RightLabel = "~b~→"}, true, function(Hovered, Active, Selected) end, RMenu:Get("PDArmoury", "confirma"))
    end) 
end
end)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("PDArmoury", "confirma")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Separator("Armour Plate Price: ~b~£" .. '0', function() end)
            RageUI.Separator("Current Gunstore: ~b~" .. armoury.name, function() end)
        ...
        RageUI.Button("Decline" , nil, {RightLabel = "~b~→"}, true, function(Hovered, Active, Selected) end, RMenu:Get("PDArmoury", "main"))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("PDArmoury", "confirm")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Separator("Weapon Price: ~b~£0", function() end)
            RageUI.Separator("Current Gunstore: ~b~" .. armoury.name, function() end)
