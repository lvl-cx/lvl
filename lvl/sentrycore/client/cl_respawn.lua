...
RMenu.Add("SpawnMenu", "main", RageUI.CreateMenu("", "~b~LVL Spawn Menu",1300, 50, 'spawn', 'spawn'))

RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('SpawnMenu', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
			FreezeEntityPosition(PlayerPedId(), true)
			RageUI.Button("St Thomas Medical Center", nil, { RightLabel = "~b~→" }, true, function(Hovered, Active, Selected)
				if Active then
			...
			RageUI.Button("Legion Gunstore", nil, { RightLabel = "~b~→" }, true, function(Hovered, Active, Selected)
				if Active then
			...
			RageUI.Button("Paleto Bay Medical Center", nil, { RightLabel = "~b~→" }, true, function(Hovered, Active, Selected)
				if Active then
			...
			RageUI.Button("Sandy Shores Medical Center", nil, { RightLabel = "~b~→" }, true, function(Hovered, Active, Selected)
				if Active then
			...
			if policeenabled then
				RageUI.Button("Mission Row PD", nil, { RightLabel = "~b~→" }, true, function(Hovered, Active, Selected)
					if Active then
				...
				RageUI.Button("Paleto PD", nil, { RightLabel = "~b~→" }, true, function(Hovered, Active, Selected)
					if Active then
				...
				RageUI.Button("Vespucci PD", nil, { RightLabel = "~b~→" }, true, function(Hovered, Active, Selected)
					if Active then
			...
			if rebellicense then 
				RageUI.Button("Rebel Diner", nil, { RightLabel = "~b~→" }, true, function(Hovered, Active, Selected)
					if Active then
			...
			if houseName == nil then 
				...
				RageUI.Button(houseName, nil, { RightLabel = "~b~→" }, true, function(Hovered, Active, Selected)
					if Active then
