...
function LVL.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  LVLclient.openMenuData(source,{menudata})
end

-- force close player menu
function LVL.closeMenu(source)
  LVLclient.closeMenu(source,{})
end
...
function LVL.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  LVLclient.prompt(source,{title,default_text})
end
...
function LVL.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  LVLclient.request(source,{id,text,time}) -- send request to client
...
function LVL.registerMenuBuilder(name, builder)
...
function LVL.buildMenu(name, data, cbr)
...
function LVL.openMainMenu(source)
  LVL.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    LVL.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tLVL.closeMenu(id)
...
function tLVL.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tLVL.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tLVL.requestResult(id,ok)
...
function tLVL.openMainMenu()
  LVL.openMainMenu(source)
end
...
function LVL.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for LVL.addStaticMenuChoices calls
...
AddEventHandler("LVL:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    LVLclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
