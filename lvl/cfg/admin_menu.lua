...
cfg.Buttons = {
    ...
    TriggerServerEvent('LVLAdmin:Ban', self, hours, reason)
    end, "player.ban"},
    ["Kick"] = {function(self)
    ...
    if (GetOnscreenKeyboardResult()) then
        ...
        if result then 
            result = result
            TriggerServerEvent('LVLAdmin:Kick', self, result, false)
        end
    end
    end, "player.kick"},
    ["No Warning Kick"] = {function(self)
        ...
        if (GetOnscreenKeyboardResult()) then
            ...
            if result then 
                result = result
                TriggerServerEvent('LVLAdmin:Kick', self, result, true)
            end
        end
    end, "player.kick"},
    ["Revive"] = {function(self)
        TriggerServerEvent('LVLAdmin:Revive', self)
    end, "player.revive"},
    ["Slap"] = {function(self)
        TriggerServerEvent('LVLAdmin:SlapPlayer', self)
    end, "player.slap"},
    ["Spectate"] = {function(self)
        TriggerServerEvent('LVLAdmin:SpectatePlr', self)
    end, "player.spectate"},
    ...
    ["Add Car"] = {function(self)
        ...
        if (GetOnscreenKeyboardResult()) then
            local result = GetOnscreenKeyboardResult()
            if result then 
                TriggerServerEvent('LVLAdmin:AddCar', self, result)
            end
        end
    end, "player.addcar"},
    ["TP To Player"] = {function(self)
        TriggerServerEvent('LVLAdmin:TPTo', self)
    end, "player.tpto"},
    ["Bring Player"] = {function(self)
        TriggerServerEvent('LVLAdmin:Bring', self)
    end, "player.tpbring"},
    ["Add Group"] = {function(self)
        ...
        if (GetOnscreenKeyboardResult()) then
            local result = GetOnscreenKeyboardResult()
            if result then 
                TriggerServerEvent('LVLAdmin:AddGroup', self, result)
            end
        end
    ...
    ["Warn Player"] = {function(self)
        ...
        if userWarningMessage then
            TriggerServerEvent("lvl:warnPlayer",self,userWarningMessage)
        else
            tLVL.notify('Please enter a warning message!')
        end
    end, "player.kick"},
...
cfg.MiscButtons = {
    ["Entity Cleanup"] = {function(self)
        TriggerServerEvent('LVLAdmin:PropCleanup')
    ...
    ["Remove Warning"] = {function(self)
        ...
        if (GetOnscreenKeyboardResult()) then
            local result = GetOnscreenKeyboardResult()
            if result then 
                TriggerServerEvent('LVL:RemoveWarning', result)
            end
        end
    end, "admin.removewarning", "Removes a warning"},
    ["Entity Cleanup Gun"] = {function(self)
        TriggerServerEvent('LVLAdmin:EntityCleanupGun')
    ...
    ["Shutdown Server"] = {function(self)
        TriggerServerEvent('LVLAdmin:ServerShutdown')
        ...
        if (GetOnscreenKeyboardResult()) then
            ...
            if result then 
                ...
                while not HasModelLoaded(mhash) and i < 10000 do
                    ...
                    i = i+1
                    if i > 10000 then 
                        tLVL.notify('~r~Model could not be loaded!')
                        break 
                    end
                end
                -- spawn car
