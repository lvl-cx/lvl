...
local function play_eat(player)
  ...
  LVLclient.playAnim(player,{true,seq,false})
end
...
local function gen(ftype, vary_hunger, vary_thirst)
  local fgen = function(args)
    ...
    local name = LVL.getItemName(idname)

    choices[act] = {function(player,choice)
      local user_id = LVL.getUserId(player)
      if user_id ~= nil then
        if LVL.tryGetInventoryItem(user_id,idname,1,false) then
          if vary_hunger ~= 0 then LVL.varyHunger(user_id,vary_hunger) end
