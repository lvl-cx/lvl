        ...
        if (result.length > 0) {
            ...
            fivemexports.ghmattimysql.execute("SELECT * FROM `arma_users` WHERE id = ?", [adminpermid], (result) => {
                if (result) {
                    ...
                    fivemexports.ghmattimysql.execute("INSERT INTO `arma_user_notes` (user_id, text, admin_name, admin_id) VALUES(?,?,?,?) ", [params[0], [params.join(' ').replace(params[0], '')], adminname, adminpermid], (result) => {
                        if (result) {
                            let embed = {
                                "title": "Added Note",
