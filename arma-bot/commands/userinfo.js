...
exports.runcmd = async(fivemexports, client, message, params) => {
    ...
    fivemexports.ghmattimysql.execute("SELECT * FROM `arma_user_ids` WHERE user_id = ?", [params[0]], (discordid) => {
        ...
        fivemexports.ghmattimysql.execute("SELECT * FROM arma_user_moneys WHERE user_id = ?", [params[0]], (result) => {
            if (result) {
                ...
                fivemexports.ghmattimysql.execute("SELECT * FROM `arma_users` WHERE id = ?", [params[0]], (userdata) => {
                    if (userdata) {
