...
async function RegisterVueComponent(component, path) {
    const response = await fetch("nui://oasis/ui/pnc/components/"+path);
        ...
        methods: {
            addMarker() {
                if(!this.doesPlayerHaveWarningMarkerOfType(this.selectedWarningMarkerType)) {
                    $.post('https://oasis/addmarker', JSON.stringify({
            ...
            wipeAllMarkers() {
                $.post('https://oasis/wipeallmarkers', JSON.stringify({
            ...
            addPoint() {
                $.post('https://oasis/addpoint', JSON.stringify({
            ...
            switchPage(page) {
                if(this.personPages.includes(page)){
                    if (this.selectedPerson.id){
                        ...
                        console.log("[OASIS PNC] Person needs to be selected to enter this page")
                    }
                }else {
                    this.page = page;
                }
            ...
            closePNC() {
                $.post('https://oasis/exit', JSON.stringify({}));
            },
            searchPerson(){
                let first = this.personFirstName;
                let last = this.personLastName;
                if (first == "" || last == "") {
                    console.log("[OASIS PNC] Full name needs to be provided")
                }else{
                    $.post('https://oasis/personsearch', JSON.stringify({
                        firstname: first,
                        lastname: last
                    }));
                }
            ...
            searchPlate(){
                ...
                $.post("https://oasis/platesearch", JSON.stringify({
                    plate: this.plate
                }))
            },
            selectPerson(person){
            ...
            submitFine() {
                ...
                $.post('https://oasis/submitfine', JSON.stringify({
            ...
            addAD(){
                ...
                if (this.ADInput.length === 0) {
                    ...
                    $.post('https://oasis/addattentiondrawn', JSON.stringify({
                        ad: this.ADInput
                    }));
                }
            ...
            removeAD(index, ad) {
                if(this.askedIfSure) {
                    ...
                    $.post('https://oasis/removeattentiondrawn', JSON.stringify({
            ...
            saveNotes() {
                $.post('https://oasis/savenotes', JSON.stringify({
            ...
            savePersonNotes() {
                $.post("https://oasis/savepersonnotes", JSON.stringify({
                    user_id: this.selectedPerson.id,
                    notes: this.selectedPerson.notes
                }))
            },
            GetWarrant() {
                $.post("https://oasis/generatewarrant", JSON.stringify({}))
            }
            },
