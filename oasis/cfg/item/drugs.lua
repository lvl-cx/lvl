...
cocaine_sniff["Take"] = {function(player,choice)
  ...
  if user_id ~= nil then
    if OASIS.tryGetInventoryItem(user_id,"Cocaine",1) then
      OASISclient.notify(player,{"~g~Taking Cocaine."})
      TriggerEvent('OASIS:RefreshInventory', player)
      TriggerClientEvent('OASIS:cocaineEffect', player)
    end
  end
end}
...
heroin_take["Take"] = {function(player,choice)
  local user_id = OASIS.getUserId(player)
  if user_id ~= nil then
    if OASIS.tryGetInventoryItem(user_id,"Heroin",1) then
      OASISclient.notify(player,{"~g~Taking Heroin."})
      TriggerEvent('OASIS:RefreshInventory', player)
      TriggerClientEvent('OASIS:heroinEffect', player)
    end
  end
end}
...
lsd_take["Take"] = {function(player,choice)
  local user_id = OASIS.getUserId(player)
  if user_id ~= nil then
    if OASIS.tryGetInventoryItem(user_id,"LSD",1) then
      OASISclient.notify(player,{"~g~Taking LSD."})
      TriggerEvent('OASIS:RefreshInventory', player)
      TriggerClientEvent('OASIS:doAcid', player)
    end
  end
end}
...
morphine_choices["Take"] = {function(player,choice)
  local user_id = OASIS.getUserId(player)
  if user_id ~= nil then
    if OASIS.tryGetInventoryItem(user_id,"Morphine",1) then
      TriggerEvent('OASIS:RefreshInventory', player)
      TriggerClientEvent('OASIS:applyMorphine', player)
    end
  end
end}
...
taco_choices["Take"] = {function(player,choice)
  local user_id = OASIS.getUserId(player)
  if user_id ~= nil then
    if OASIS.tryGetInventoryItem(user_id,"Taco",1) then
      TriggerEvent('OASIS:RefreshInventory', player)
      TriggerClientEvent('OASIS:eatTaco', player)
    end
  end
end}
