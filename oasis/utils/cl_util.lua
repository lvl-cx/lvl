...
function tOASIS.getStreetNameAtCoord(v, w, H)
...
function tOASIS.randomNum(d, e)
    math.randomseed(GetGameTimer() * math.random() * 2)
    return math.random(d, e)
end
soundEventCode = 0
TriggerServerEvent("OASIS:soundCodeServer")
RegisterNetEvent("OASIS:soundCode",function(aH)
    soundEventCode = aH
end)
RegisterNetEvent("OASIS:playClientNuiSound",function(aI, aJ, ap) -- use for cuffing and other sounds in areas
    local ax = tOASIS.getPlayerCoords()
    if #(ax - aI) <= ap then
        SendNUIMessage({transactionType = aJ})
    end
end)
function tOASIS.getNetId(aW, aX)
    if aX == nil then
        aX = ""
    end
    local aY = 0
...
Citizen.CreateThread(function()
    local a = module("oasis-vehicles", "garages")
...
function tOASIS.getVehicleNameFromId(b7)
    if b2[string.lower(b7)] then
        return b2[string.lower(b7)].name
    end
    return ""
end
function tOASIS.getGarageNameFromId(b7)
