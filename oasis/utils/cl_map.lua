globalBlips = {}
function tOASIS.addBlip(a, b, c, d, e, f, g, h)
...
function tOASIS.removeBlip(j)
    RemoveBlip(j)
end
function tOASIS.removeBlipAtCoords(x,y,z) -- used to remove green house blips for everyone when purchased
...
local k = {}
function tOASIS.setNamedBlip(l, a, b, c, d, e, f, g)
    tOASIS.removeNamedBlip(l)
    k[l] = tOASIS.addBlip(a, b, c, d, e, f, g)
    return k[l]
end
function tOASIS.removeNamedBlip(l)
    if k[l] ~= nil then
        tOASIS.removeBlip(k[l])
        k[l] = nil
    end
end
function tOASIS.setGPS(a, b)
    SetNewWaypoint(a + 0.0001, b + 0.0001)
end
function tOASIS.setBlipRoute(j)
    SetBlipRoute(j, true)
end
local m = {}
...
local p = {}
local q = {}
function tOASIS.addMarker(a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    local I = {
...
function tOASIS.removeMarker(j)
    if m[j] ~= nil then
        m[j] = nil
        n:free(j)
    end
...
function tOASIS.setNamedMarker(l, a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    tOASIS.removeNamedMarker(l)
    o[l] = tOASIS.addMarker(a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    return o[l]
end
function tOASIS.removeNamedMarker(l)
    if o[l] ~= nil then
        tOASIS.removeMarker(o[l])
        o[l] = nil
    end
end
local J = {}
...
Citizen.CreateThread(
    function()
        while true do
            q = tOASIS.getNearbyMarkers()
            Citizen.Wait(10000)
        end
    end
)
function tOASIS.getNearbyMarkers()
    local N = {}
    local O = tOASIS.getPlayerCoords()
    local P = 0
...
local Q = {}
local R = {}
function tOASIS.getNearbyAreas()
    local S = {}
    local M = tOASIS.getPlayerCoords()
    local P = 0
    for K, L in pairs(Q) do
...
function tOASIS.setArea(l, a, b, c, T, U)
...
RegisterCommand("nextblip",function()
    a5 = a5 + 1
    if a4 then
        tOASIS.removeBlip(a4)
    end
    a4 = tOASIS.addBlip(1103.9739990234, 211.95138549805, -49.440101623535, a5, 0, "Chips Cashier", 0.8, true)
end)
