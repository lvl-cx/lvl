...
RegisterNetEvent("OASIS:attemptBeginBusJob")
AddEventHandler("OASIS:attemptBeginBusJob",function()
    local source=source
    local user_id=OASIS.getUserId(source)
    local BusJobTable={}
    if OASIS.hasGroup(user_id,"Bus Driver")then
        if not usersInBusJob[user_id] then
            ...
            TriggerClientEvent("OASIS:beginBusJob",source)
            Wait(100)
            TriggerClientEvent("OASIS:setNextBusJobBlip",source,busJobStops[BusJobTable.stopNumber].stopPosition)
        else
            OASISclient.notify(source,{"~r~You are already a Bus Driver."})
        end
    else
        OASISclient.notify(source,{"~r~You are not clocked on as a Bus Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(busJobStops)do
                    if OASIS.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(OASIS.getUserSource(k)))-busJobStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            OASIS.giveBankMoney(k,pay)
                            TriggerClientEvent("OASIS:nextStopReachedBusJob",OASIS.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#busJobStops then
                                TriggerClientEvent("OASIS:endBusJob", OASIS.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInBusJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("OASIS:setNextBusJobBlip",OASIS.getUserSource(k),busJobStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
