	...
	if not chatCooldown[source] then 
		for word in pairs(blockedWords) do
			if(string.gsub(string.gsub(string.gsub(string.gsub(string.gsub(string.gsub(message:lower(), "-", ""), ",", ""), "%.", ""), " ", ""), "*", ""), "+", ""):find(blockedWords[word])) then
				TriggerClientEvent('OASIS:chatFilterScaleform', source, 10, 'That word is not allowed.')
				CancelEvent()
				return
			end
		end
		tOASIS.sendWebhook('anon', "OASIS Chat Logs", "```"..message.."```".."\n> Player Name: **"..GetPlayerName(source).."**\n> Player PermID: **"..user_id.."**\n> Player TempID: **"..source.."**")
		TriggerClientEvent('chatMessage', -1, "^4Twitter @^1Anonymous: ", { 128, 128, 128 }, message, "ooc")
		chatCooldown[source] = true
	end
end)

function tOASIS.ooc(source, args, raw)
	if #args <= 0 then 
		return 
	end
	local source = source
	...
	if not chatCooldown[source] then 
		...
		tOASIS.sendWebhook('ooc', "OASIS Chat Logs", "```"..message.."```".."\n> Player Name: **"..GetPlayerName(source).."**\n> Player PermID: **"..user_id.."**\n> Player TempID: **"..source.."**")
	else
		TriggerClientEvent('chatMessage', source, "^1[OASIS]", { 128, 128, 128 }, " Chat Spam | Retry in 3 Seconds", "alert")
		chatCooldown[source] = true
	end
end

RegisterCommand("ooc", function(source, args, raw)
    tOASIS.ooc(source, args, raw)
end)
...
function OASIS.ooc(source, args) -- ooc from chat with //
	args[1] = args[1]:sub(3)
	tOASIS.ooc(source, args)
end


RegisterCommand('cc', function(source, args, rawCommand)
    local user_id = OASIS.getUserId(source)
    if OASIS.hasPermission(user_id, 'admin.ban') then
