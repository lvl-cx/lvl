...
function OASIS.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  OASISclient.openMenuData(source,{menudata})
end

-- force close player menu
function OASIS.closeMenu(source)
  OASISclient.closeMenu(source,{})
end
...
function OASIS.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  OASISclient.prompt(source,{title,default_text})
end
...
function OASIS.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  OASISclient.request(source,{id,text,time}) -- send request to client
...
function OASIS.registerMenuBuilder(name, builder)
...
function OASIS.buildMenu(name, data, cbr)
...
function OASIS.openMainMenu(source)
  OASIS.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    OASIS.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tOASIS.closeMenu(id)
...
function tOASIS.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tOASIS.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tOASIS.requestResult(id,ok)
...
function tOASIS.openMainMenu()
  OASIS.openMainMenu(source)
end
...
function OASIS.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for OASIS.addStaticMenuChoices calls
...
AddEventHandler("OASIS:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    OASISclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
