...
AddEventHandler("OASIS:playerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
        TriggerClientEvent('OASIS:policeSyncAllDoors', source, doors, doorStates)
    end
end)

RegisterNetEvent("OASIS:policeSetDoorState")
AddEventHandler("OASIS:policeSetDoorState", function(doorHash, state)
    local source = source
    local user_id = OASIS.getUserId(source)
    if OASIS.hasPermission(user_id, 'police.onduty.permission') then
        doorStates[doorHash] = state
        TriggerClientEvent('OASIS:policeSyncDoor', -1, doorHash, state)
    end
end)

RegisterNetEvent("OASIS:enterPoliceSyncDoors")
AddEventHandler("OASIS:enterPoliceSyncDoors", function()
    local source = source
    TriggerClientEvent('OASIS:gotAllDoorStates', source, doorStates)
end)