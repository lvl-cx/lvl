...
MySQL.createCommand("OASIS/get_weapons", "SELECT weapon_info FROM oasis_weapon_whitelists WHERE user_id = @user_id")
MySQL.createCommand("OASIS/set_weapons", "UPDATE oasis_weapon_whitelists SET weapon_info = @weapon_info WHERE user_id = @user_id")
MySQL.createCommand("OASIS/add_user", "INSERT IGNORE INTO oasis_weapon_whitelists SET user_id = @user_id")
MySQL.createCommand("OASIS/get_all_weapons", "SELECT * FROM oasis_weapon_whitelists")
MySQL.createCommand("OASIS/create_weapon_code", "INSERT IGNORE INTO oasis_weapon_codes SET user_id = @user_id, spawncode = @spawncode, weapon_code = @weapon_code")
MySQL.createCommand("OASIS/remove_weapon_code", "DELETE FROM oasis_weapon_codes WHERE weapon_code = @weapon_code")
MySQL.createCommand("OASIS/get_weapon_codes", "SELECT * FROM oasis_weapon_codes")

AddEventHandler("playerJoining", function()
    local user_id = OASIS.getUserId(source)
    MySQL.execute("OASIS/add_user", {user_id = user_id})
end)
...
RegisterNetEvent("OASIS:getCustomWeaponsOwned")
AddEventHandler("OASIS:getCustomWeaponsOwned",function()
    local source = source
    local user_id = OASIS.getUserId(source)
    ...
    MySQL.query("OASIS/get_weapons", {user_id = user_id}, function(weaponWhitelists)
        if weaponWhitelists[1]['weapon_info'] ~= nil then
            ...
            TriggerClientEvent('OASIS:gotCustomWeaponsOwned', source, ownedWhitelists)
        end
    end)
end)

RegisterNetEvent("OASIS:requestWhitelistedUsers")
AddEventHandler("OASIS:requestWhitelistedUsers",function(spawncode)
    local source = source
    local user_id = OASIS.getUserId(source)
    local whitelistOwners = {}
    MySQL.query("OASIS/get_all_weapons", {}, function(weaponWhitelists)
        for k,v in pairs(weaponWhitelists) do
            if v['weapon_info'] ~= nil then
                data = json.decode(v['weapon_info'])
                for a,b in pairs(data) do
                    if b[spawncode] then
                        whitelistOwners[v['user_id']] = (exports['ghmattimysql']:executeSync("SELECT username FROM oasis_users WHERE id = @user_id", {user_id = v['user_id']})[1]).username
                    end
                end
            end
        end
        TriggerClientEvent('OASIS:getWhitelistedUsers', source, whitelistOwners)
    end)
end)

RegisterNetEvent("OASIS:generateWeaponAccessCode")
AddEventHandler("OASIS:generateWeaponAccessCode",function(spawncode, id)
    local source = source
    local user_id = OASIS.getUserId(source)
    ...
    for a,b in pairs(whitelistedGuns) do
        for c,d in pairs(b) do
            if b[spawncode] and d[6]== user_id then
                MySQL.execute("OASIS/create_weapon_code", {user_id = id, spawncode = spawncode, weapon_code = code})
                TriggerClientEvent('OASIS:generatedAccessCode', source, code)
            end
        end
    end
end)

RegisterNetEvent("OASIS:requestNewGunshopData")
AddEventHandler("OASIS:requestNewGunshopData",function()
    local source = source
    local user_id = OASIS.getUserId(source)
    MySQL.query("OASIS/get_weapons", {user_id = user_id}, function(weaponWhitelists)
        ...
        tOASIS.getSubscriptions(user_id, function(cb, plushours, plathours)
            if cb then
                if plathours > 0 and OASIS.hasPermission(user_id, "vip.gunstore") then
                    for k,v in pairs(VIPWithPlat) do
                        gunstoreData["VIP"][k] = v
                    end
                end
            end
            if OASIS.hasPermission(user_id, 'advancedrebel.license') then
                for k,v in pairs(RebelWithAdvanced) do
                    gunstoreData["Rebel"][k] = v
                end
            end
            TriggerClientEvent('OASIS:recieveFilteredGunStoreData', source, gunstoreData)
        end)
    end)
end)
...
function gunStoreLogs(weaponshop, webhook, title, text)
    if weaponshop == 'policeLargeArms' or weaponshop == 'policeSmallArms' then
        tOASIS.sendWebhook('pd-armoury', 'OASIS Police Armoury Logs', text)
    elseif weaponshop == 'NHS' then
        tOASIS.sendWebhook('nhs-armoury', 'OASIS NHS Armoury Logs', text)
    elseif weaponshop == 'prisonArmoury' then
        tOASIS.sendWebhook('hmp-armoury', 'OASIS HMP Armoury Logs', text)
    elseif weaponshop == 'LFB' then
        tOASIS.sendWebhook('lfb-armoury', 'OASIS LFB Armoury Logs', text)
    end
    tOASIS.sendWebhook(webhook,title,text)
end

RegisterNetEvent("OASIS:buyWeapon")
AddEventHandler("OASIS:buyWeapon",function(spawncode, price, name, weaponshop, purchasetype, vipstore)
    local source = source
    local user_id = OASIS.getUserId(source)
    local hasPerm = false
    local gunstoreData = deepcopy(cfg.GunStores)
            ...
            if k == '_config' then
                ...
                if json.encode(v[5]) ~= '[""]' then
                    ...
                    for a,b in pairs(v[5]) do
                        if OASIS.hasPermission(user_id, b) then
                    ...
                    for c,d in pairs(gunstoreData[weaponshop]) do
                        if c ~= '_config' then
                            if hasPerm then
                                    ...
                                    if name == d[1] then
                                        if purchasetype == 'armour' then
                                            if string.find(spawncode, "fillUp") then
                                                ...
                                                if OASIS.tryPayment(user_id,price) then
                                                    OASISclient.notify(source, {'~g~You bought '..name..' for £'..getMoneyStringFormatted(price)..'.'})
                                                    TriggerClientEvent("oasis:PlaySound", source, 1)
                                                    OASISclient.setArmour(source, {100, true})
                                                    gunStoreLogs(weaponshop, 'weapon-shops',"OASIS Weapon Shop Logs", "> Player Name: **"..GetPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Purchased: **"..name.."**\n> Price: **£"..getMoneyStringFormatted(price).."**\n> Weapon Shop: **"..weaponshop.."**\n> Purchase Type: **"..purchasetype.."**")
                                                    return
                                                end
                                            elseif GetPedArmour(GetPlayerPed(source)) >= (price/1000) then
                                                OASISclient.notify(source, {'~r~You already have '..GetPedArmour(GetPlayerPed(source))..'% armour.'})
                                                return
                                            end
                                            if OASIS.tryPayment(user_id,d[2]) then
                                                OASISclient.notify(source, {'~g~You bought '..name..' for £'..getMoneyStringFormatted(price)..'.'})
                                                TriggerClientEvent("oasis:PlaySound", source, 1)
                                                OASISclient.setArmour(source, {price/1000, true})
                                                gunStoreLogs(weaponshop, 'weapon-shops',"OASIS Weapon Shop Logs", "> Player Name: **"..GetPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Purchased: **"..name.."**\n> Price: **£"..getMoneyStringFormatted(price).."**\n> Weapon Shop: **"..weaponshop.."**\n> Purchase Type: **"..purchasetype.."**")
                                                if weaponshop == 'LargeArmsDealer' then
                                                    OASIS.turfSaleToGangFunds(price, 'LargeArms')
                                                end
                                            else
                                                OASISclient.notify(source, {'~r~You do not have enough money for this purchase.'})
                                                TriggerClientEvent("oasis:PlaySound", source, 2)
                                            end
                                        elseif purchasetype == 'weapon' then
                                            OASISclient.hasWeapon(source, {spawncode}, function(hasWeapon)
                                                if hasWeapon then
                                                    OASISclient.notify(source, {'~r~You must store your current '..name..' before purchasing another.'})
                                                else
                                                    if OASIS.tryPayment(user_id,d[2]) then
                                                        if price > 0 then
                                                            OASISclient.notify(source, {'~g~You bought a '..name..' for £'..getMoneyStringFormatted(price)..'.'})
                                                            if weaponshop == 'LargeArmsDealer' then
                                                                OASIS.turfSaleToGangFunds(price, 'LargeArms')
                                                            end
                                                        else
                                                            OASISclient.notify(source, {'~g~'..name..' purchased.'})
                                                        end
                                                        TriggerClientEvent("oasis:PlaySound", source, 1)
                                                        OASISclient.giveWeapons(source, {{[spawncode] = {ammo = 250}}, false})
                                                        gunStoreLogs(weaponshop, 'weapon-shops',"OASIS Weapon Shop Logs", "> Player Name: **"..GetPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Purchased: **"..name.."**\n> Price: **£"..getMoneyStringFormatted(price).."**\n> Weapon Shop: **"..weaponshop.."**\n> Purchase Type: **"..purchasetype.."**")
                                                    else
                                                        OASISclient.notify(source, {'~r~You do not have enough money for this purchase.'})
                                                        TriggerClientEvent("oasis:PlaySound", source, 2)
                                        ...
                                        elseif purchasetype == 'ammo' then
                                            ...
                                            if OASIS.tryPayment(user_id,price) then
                                                if price > 0 then
                                                    OASISclient.notify(source, {'~g~You bought 250x Ammo for £'..getMoneyStringFormatted(price)..'.'})
                                                    if weaponshop == 'LargeArmsDealer' then
                                                        OASIS.turfSaleToGangFunds(price, 'LargeArms')
                                                    end
                                                else
                                                    OASISclient.notify(source, {'~g~250x Ammo purchased.'})
                                                end
                                                TriggerClientEvent("oasis:PlaySound", source, 1)
                                                OASISclient.giveWeapons(source, {{[spawncode] = {ammo = 250}}, false})
                                                gunStoreLogs(weaponshop, 'weapon-shops',"OASIS Weapon Shop Logs", "> Player Name: **"..GetPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Purchased: **"..name.."**\n> Price: **£"..getMoneyStringFormatted(price).."**\n> Weapon Shop: **"..weaponshop.."**\n> Purchase Type: **"..purchasetype.."**")
                                ...
                                if weaponshop == 'policeLargeArms' or weaponshop == 'policeSmallArms' then
                                    OASISclient.notify(source, {"~r~You shouldn't be in here, ALARM TRIGGERED!!!"})
                                else
                                    OASISclient.notify(source, {"~r~You do not have permission to access this store."})
                                end
                            end
                        end
                    end
