...
local function Get_Client_Discord_ID(source)
	return exports['ghmattimysql']:executeSync("SELECT discord_id FROM `oasis_verification` WHERE user_id = @user_id", {user_id = OASIS.getUserId(source)})[1].discord_id
end
	...
	if discord_roles then
		...
		for _, role_id in pairs(does_not_have_roles) do
			for k,v in pairs(cfg.Guild_Roles[guild_name]) do
                if v == role_id and OASIS.hasGroup(user_id, k) then
		...
		for _, role_id in pairs(has_roles) do
            for k,v in pairs(cfg.Guild_Roles[guild_name]) do
                if v == role_id and not OASIS.hasGroup(user_id, k) then
                    OASIS.addUserGroup(user_id, k)
                end
            end
		end
	end
	OASIS.getJobSelectors(OASIS.getUserSource(user_id))
end

local tracked = {}
RegisterNetEvent('OASIS:getFactionWhitelistedGroups')
AddEventHandler('OASIS:getFactionWhitelistedGroups', function()
	local source = source
	tOASIS.getFactionGroups(source)
end)

function tOASIS.getFactionGroups(source)
    local source = source
...
function tOASIS.checkForRole(user_id, role_id)
	local discord_id = exports['ghmattimysql']:executeSync("SELECT discord_id FROM `oasis_verification` WHERE user_id = @user_id", {user_id = user_id})[1].discord_id
