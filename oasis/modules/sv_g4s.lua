...
RegisterNetEvent("OASIS:toggleShiftG4S")
AddEventHandler("OASIS:toggleShiftG4S", function(Status)
    local source=source
    local user_id=OASIS.getUserId(source)
    if OASIS.hasGroup(user_id,"G4S Driver")then
        if Status then
            currentUsersOnG4SJob[user_id]={currentJob=""}
            TriggerClientEvent("OASIS:startShiftG4S",source,math.random(1000,9999))
            jobsCompleted[user_id] = 0
            Wait(10000)
            TriggerClientEvent("OASIS:updateJobInformation",source,GetCollection(user_id))
        else
            if currentUsersOnG4SJob[user_id]~=nil then
                currentUsersOnG4SJob[user_id]=nil
                jobsCompleted[user_id] = nil
                TriggerClientEvent("OASIS:endShiftG4S",source)
            end
        end
    else
        OASISclient.notify(source,{"~r~You are not clocked on as currentUsersOnG4SJob G4S Driver."})
    end
end)

RegisterNetEvent("OASIS:acceptJob")
AddEventHandler("OASIS:acceptJob",function()
    local source=source
    local user_id=OASIS.getUserId(source)
    if OASIS.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
            if k == user_id then    
                ...
                TriggerClientEvent("OASIS:updateJobInformation",source,v.currentJob)
            end
        end
    else
        OASISclient.notify(source,{"~r~You are not clocked on as currentUsersOnG4SJob G4S Driver."})
...
RegisterNetEvent("OASIS:updateMoneyInVehicle")
AddEventHandler("OASIS:updateMoneyInVehicle", function()
    local source=source
    local user_id=OASIS.getUserId(source)
    if OASIS.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
...
RegisterNetEvent("OASIS:updateMoneyOutVehicle")
AddEventHandler("OASIS:updateMoneyOutVehicle", function()
    local source=source
    local user_id=OASIS.getUserId(source)
    if OASIS.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
    ...
    while true do
        ...
        for k,v in pairs(currentUsersOnG4SJob)do
            if v.currentJob.jobActive then
                if OASIS.getUserSource(k)then
                    if not v.currentJob.collected then
                        if #(GetEntityCoords(GetPlayerPed(OASIS.getUserSource(k)))-v.currentJob.collectionCoords)<1.5 then
                            v.currentJob.collected=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("OASIS:updateJobInformation",OASIS.getUserSource(k),v.currentJob)
                            TriggerClientEvent("OASIS:requestMoneyInVehicle",OASIS.getUserSource(k))
                        end
                    elseif v.currentJob.moneyInVehicle and not v.currentJob.depositing and not v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(OASIS.getUserSource(k)))-v.currentJob.depositCoords)<20 then
                            v.currentJob.depositing=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("OASIS:updateJobInformation",OASIS.getUserSource(k),v.currentJob)
                            TriggerClientEvent("OASIS:requestMoneyOutVehicle",OASIS.getUserSource(k))
                        end
                    elseif not v.currentJob.deposited and v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(OASIS.getUserSource(k)))-v.currentJob.depositCoords)<1.5 then
                            v.currentJob.deposited=true
                            v.currentJob.depositing=false
                            TriggerClientEvent("OASIS:updateJobInformation",OASIS.getUserSource(k),GetCollection(k))
                            local pay = grindBoost*math.random(8000,12000)
                            OASIS.giveBankMoney(k,pay)
                        end
                    end
                end
            end
...
RegisterCommand('g4s', function(source, args)
    ...
    for k,v in pairs(currentG4SUsersOnJob) do
        if k == user_id then
            TriggerClientEvent("OASIS:openG4SMenu", source)
        end
    end
end)