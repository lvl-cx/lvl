...
RegisterNetEvent('OASIS:getStoreRobBlips')
AddEventHandler('OASIS:getStoreRobBlips', function()
    local source = source
    TriggerClientEvent('OASIS:updateStoreRobBlips', -1, stores)
end)


RegisterNetEvent('OASIS:initiateStoreRobbery')
AddEventHandler('OASIS:initiateStoreRobbery', function(store)
    local source = source
    local user_id = OASIS.getUserId(source)
    if OASIS.hasPermission(user_id, "police.onduty.permission") then
        TriggerClientEvent('OASIS:resetStorePed', source, store)
    else
        if #OASIS.getUsersByPermission('police.onduty.permission') > 2 then
            for k,v in pairs(stores) do
                if k == store then
                    if v.cooldown == 0 then
                        ...
                        v.cooldown = 300
                        TriggerClientEvent('OASIS:updateStoreRobBlips', -1, stores)
                        TriggerClientEvent('OASIS:beginStoreRobbingAnimations', -1, store)
                        TriggerClientEvent('OASIS:storeRobberyInProgress', source, true, store)
                        ...
                        TriggerEvent('OASIS:PDRobberyCall', source, v.storename, v.position)
                    else
            ...
            OASISclient.notify(source, {'~r~There are not enough police on duty to rob a store.'})
            TriggerClientEvent('OASIS:resetStorePed', source, store)
        end
    end
end)

RegisterNetEvent('OASIS:completeSafeCracking')
AddEventHandler('OASIS:completeSafeCracking', function(store)
    local source = source
    local user_id = OASIS.getUserId(source)
    OASIS.giveMoney(user_id, math.random(85000, 150000))
    TriggerClientEvent('OASIS:syncCloseSafeDoor', -1, store)
    TriggerClientEvent('OASIS:resetStorePed', -1, store)
    for k,v in pairs(stores) do
        if k == store then
            v.beingrobbed = false
        end
    ...
    TriggerClientEvent('OASIS:updateStoreRobBlips', -1, stores)
    TriggerClientEvent('OASIS:storeRobberyInProgress', source, false, store)
end)

RegisterNetEvent('OASIS:forceEndRobbery')
AddEventHandler('OASIS:forceEndRobbery', function(store)
    local source = source
    local user_id = OASIS.getUserId(source)
    TriggerClientEvent('OASIS:resetStorePed', -1, store)
    for k,v in pairs(stores) do
...
RegisterNetEvent('OASIS:syncOpenSafeDoor')
AddEventHandler('OASIS:syncOpenSafeDoor', function(store)
    local source = source
    TriggerClientEvent('OASIS:syncOpenSafeDoor', -1, store)
end)
