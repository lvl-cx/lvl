...
RegisterNetEvent("OASIS:attemptBeginRoyalMailJob")
AddEventHandler("OASIS:attemptBeginRoyalMailJob",function()
    local source=source
    local user_id=OASIS.getUserId(source)
    local RoyalMailJobTable={}
    if OASIS.hasGroup(user_id,"Royal Mail Driver")then
        if not usersInRoyalMailJob[user_id] then
            ...
            TriggerClientEvent("OASIS:beginRoyalMailJob",source)
            Wait(100)
            TriggerClientEvent("OASIS:royalMailJobSetNextBlip",source,RoyalMailDrops[RoyalMailJobTable.stopNumber].dropPosition)
        else
            OASISclient.notify(source,{"~r~You are already a Royal Mail Driver."})
        end
    else
        OASISclient.notify(source,{"~r~You are not clocked on as a Royal Mail Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(RoyalMailDrops)do
                    if OASIS.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(OASIS.getUserSource(k)))-RoyalMailDrops[v.currentJob.stopNumber].dropPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            OASIS.giveBankMoney(k,pay)
                            TriggerClientEvent("OASIS:royalMailReachedNextStop",OASIS.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition,pay)
                            if v.currentJob.stopNumber==#RoyalMailDrops then
                                TriggerClientEvent("OASIS:royalMailJobEnd", OASIS.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInRoyalMailJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("OASIS:royalMailJobSetNextBlip",OASIS.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition)
                            end
                        end
                    end
                end
