...
RegisterNetEvent("OASIS:attemptBeginDeliverooJob")
AddEventHandler("OASIS:attemptBeginDeliverooJob",function()
    local source=source
    local user_id=OASIS.getUserId(source)
    local deliverooJobTable={}
    if OASIS.hasGroup(user_id,"Deliveroo")then
        if not usersInDeliverooJob[user_id] then
            ...
            TriggerClientEvent("OASIS:beginDeliverooJob",source)
            Wait(100)
            TriggerClientEvent("OASIS:deliverooJobSetNextBlip",source,deliverooStops[deliverooJobTable.stopNumber].stopPosition)
        else
            OASISclient.notify(source,{"~r~You are already a Deliveroo Driver."})
        end
    else
        OASISclient.notify(source,{"~r~You are not clocked on as a Deliveroo Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(deliverooStops)do
                    if OASIS.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(OASIS.getUserSource(k)))-deliverooStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            OASIS.giveBankMoney(k,pay)
                            TriggerClientEvent("OASIS:deliverooJobReachedNextStop",OASIS.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#deliverooStops then
                                TriggerClientEvent("OASIS:deliverooJobEnd", OASIS.getUserSource(k), true)
                                v.currentJob.jobActive=false
                                usersInDeliverooJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("OASIS:deliverooJobSetNextBlip",OASIS.getUserSource(k),deliverooStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
