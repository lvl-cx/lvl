...
RMenu.Add('oasishighrollers','casino',RageUI.CreateMenu("","",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('oasishighrollers','casino'):SetSubtitle("~b~MEMBERSHIP")
RMenu.Add('oasishighrollers','confirmadd',RageUI.CreateSubMenu(RMenu:Get('oasishighrollers','casino'),"","~b~Are you sure?",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight()))
RMenu.Add('oasishighrollers','confirmremove',RageUI.CreateSubMenu(RMenu:Get('oasishighrollers','casino'),"","~b~Are you sure?",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight()))

RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('oasishighrollers', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            end,RMenu:Get('oasishighrollers','confirmadd'))
            ...
            end,RMenu:Get('oasishighrollers','confirmremove'))
        end)
    end
    if RageUI.Visible(RMenu:Get('oasishighrollers', 'confirmadd')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("No","",{RightLabel="→→→"},true,function(b,c,d)
                if d then 
                    tOASIS.notify("~y~Cancelled!")
                end 
            end,RMenu:Get('oasishighrollers','casino'))
            RageUI.ButtonWithStyle("Yes","",{RightLabel="→→→"},true,function(b,c,d)
                if d then 
                    TriggerServerEvent("OASIS:purchaseHighRollersMembership")
                end 
            end,RMenu:Get('oasishighrollers','casino'))
        end)
    end
    if RageUI.Visible(RMenu:Get('oasishighrollers', 'confirmremove')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("No","",{RightLabel="→→→"},true,function(b,c,d)
                if d then 
                    tOASIS.notify("~y~Cancelled!")
            ...
            RageUI.ButtonWithStyle("Yes","",{RightLabel="→→→"},true,function(b,c,d)
                if d then 
                    TriggerServerEvent("OASIS:removeHighRollersMembership")
                end 
            end,RMenu:Get('oasishighrollers','casino'))
        end)
    end
...
function showCasinoMembership(e)
    RageUI.Visible(RMenu:Get('oasishighrollers','casino'),e)
end
Citizen.CreateThread(function()
    ...
    for g,h in pairs(a)do 
        local i=tOASIS.loadModel('u_f_m_casinocash_01')
...
AddEventHandler("OASIS:onClientSpawn",function(D, E)
    if E then
        ...
        for q,h in pairs(a)do 
            tOASIS.addBlip(h.entryPosition.x,h.entryPosition.y,h.entryPosition.z,682,0,"Casino Memberships",0.7,true)
            tOASIS.addMarker(h.entryPosition.x,h.entryPosition.y,h.entryPosition.z,1.0,1.0,1.0,138,43,226,70,50,27)
            tOASIS.createArea("casinomembership_"..q,h.entryPosition,1.5,6,m,o,p,{})
        end 
	end
end)