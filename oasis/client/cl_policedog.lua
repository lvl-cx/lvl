RMenu.Add("policedog","main",RageUI.CreateMenu("Dog Support Unit", "~b~Dog Management", tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight()))
AddEventHandler("OASIS:setPoliceOnDuty",function(a)
    if a then
        RegisterKeyMapping("k9attack", "Police Dog Attack", "keyboard", "INSERT")
    end
end)
AddEventHandler("OASIS:setPrisonGuardOnDuty",function(b)
    if b then
        RegisterKeyMapping("k9attack", "Police Dog Attack", "keyboard", "INSERT")
    end
end)
...
local i = "K9"
local j = "K9TARGET"
RegisterNetEvent("OASIS:policeDogMenu",function()
    ...
    if globalOnPrisonDuty and tOASIS.hasHmpCallsign() then
        RMenu:Get("oasisscenemenu", "main"):SetSubtitle("~b~HMP~w~: " .. tOASIS.getHmpCallsign() .. " - " .. tOASIS.getHmpRank() .. " - " .. tOASIS.getHmpName())
    end
    RageUI.Visible(RMenu:Get("policedog", "main"), true)
end)
RegisterNetEvent("OASIS:sendClientRagdoll",function()
    SetPedToRagdoll(tOASIS.getPlayerPed(), 12000, 12000, 0, 0, 0, 0)
    showNotification("~y~~h~Alert~h~~s~: " .. "You have been bitten by a police dog.")
end)
RegisterNetEvent("OASIS:policeDogAttack",function()
...
local m = {breed = 1}
local function n()
    local o = GetResourceKvpString("oasis_policedog_customisations")
...
local function r(s)
    ...
    local q = k[m.breed]
    p[q] = s
    SetResourceKvp("oasis_policedog_customisations", json.encode(p))
end
...
function tpDog()
    local z = GetEntityCoords(tOASIS.getPlayerPed(), true)
                                    ...
                                    if M < 2.0 then
                                        ...
                                        for O, P in pairs(N) do
                                            if I == GetPlayerPed(P) then
                                                ...
                                                TriggerServerEvent("OASIS:performDogLog","Instructed dog to attack ID: " .. Q)
                                                TriggerServerEvent("OASIS:serverDogAttack", Q)
                                                H = true
                                                Wait(1000)
    ...
    while f.currentAction == d.SearchNearby and DoesEntityExist(f.handle) do
        ...
        for W, X in pairs(GetActivePlayers()) do
            ...
            if GetVehiclePedIsUsing(Y) == 0 and IsEntityVisible(Y) and #(GetEntityCoords(Y, true) - L) < 5.0 then
                ...
                local _ = tOASIS.clientGetUserIdFromSource(Z)
                if tOASIS.getJobType(_) == "" then
        ...
        if #V > 0 then
            ...
            TriggerServerEvent("OASIS:policeDogSniffPlayer", a0.source)
        end
        Citizen.Wait(5000)
    end
end
function putInVehicle()
    local a2 = tOASIS.getNearestVehicle(7.0)
    Citizen.Trace(a2)
...
function removeFromVehicle()
    if IsPedInAnyVehicle(tOASIS.getPlayerPed(), true) then
...
function newDog(q, a4)
    local a5 = tOASIS.getPlayerPed()
    if not IsPedInAnyPoliceVehicle(a5) then
        local a6 = tOASIS.loadModel(q)
        ...
        if DoesEntityExist(f.handle) then
            ...
            TriggerServerEvent("OASIS:performDogLog", "Created a new police dog")
        end
    end
end
function deleteDog()
    ...
    f.active = false
    f.handle = 0
    TriggerServerEvent("OASIS:performDogLog", "Deleted dog")
end
function dogFollow()
    if not f.active then
        return
    ...
    if f.currentAction ~= d.Follow then
        ClearPedTasks(f.handle)
        TaskFollowToOffsetOfEntity(f.handle, tOASIS.getPlayerPed(), 0.0, 0.0, 0.0, 7.0, -1, 10.0, true)
        f.currentAction = d.Follow
    else
        ClearPedTasks(f.handle)
        f.currentAction = d.Follow
...
RegisterNetEvent("OASIS:policeDogIndicate",function(ad)
    ...
    while f.currentAction == d.Indicate and DoesEntityExist(af) do
        ...
        elseif GetGameTimer() - ag > 3500 then
            if not IsEntityPlayingAnim(f.handle, g.Sit.dict, g.Sit.anims.base, 3) then
                tOASIS.loadAnimDict(g.Sit.dict)
