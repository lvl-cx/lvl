...
local b = false
RMenu.Add("oasisatm","mainmenu",RageUI.CreateMenu("", "", tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight(), "banners", "atm"))
RMenu:Get("oasisatm", "mainmenu"):SetSubtitle("~b~ATM")
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('oasisatm', 'mainmenu')) then
                ...
                if Selected then
                    ...
                    if tonumber(e) then
                        if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                            if a then
                                tOASIS.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                                TriggerServerEvent('OASIS:Deposit', tonumber(e))
                            else
                                tOASIS.notify("~r~Not near ATM.")
                            end
                        else
                            tOASIS.notify("~r~Get out your vehicle to use the ATM")
                        end
                    else
                ...
                if Selected then
                    ...
                    if tonumber(e) then
                        if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                            if a then
                                tOASIS.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                                TriggerServerEvent('OASIS:Withdraw', tonumber(e))
                            else
                                tOASIS.notify("~r~Not near ATM.")
                            end
                        else
            ...
            RageUI.Button("Deposit All", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then
                    if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                        if a then
                            tOASIS.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                            TriggerServerEvent('OASIS:DepositAll')
                        else
                            tOASIS.notify("~r~Not near ATM.")
                        end
                    else
            ...
            RageUI.Button("Withdraw All", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then
                    if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                        if a then
                            tOASIS.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                            TriggerServerEvent('OASIS:WithdrawAll')
                        else
                            tOASIS.notify("~r~Not near ATM.")
                        end
                    else
...
local function g()
    RageUI.CloseAll()
    RageUI.Visible(RMenu:Get("oasisatm", "mainmenu"), true)
end
local function h()
    RageUI.CloseAll()
    RageUI.Visible(RMenu:Get("oasisatm", "mainmenu"), false)
end
Citizen.CreateThread(function()
    ...
    for m, n in pairs(a.atms) do
        tOASIS.createArea("atm_" .. m, n, 1.5, 6, i, k, l, {atmId = m})
        local o = tOASIS.addBlip(n.x, n.y, n.z, 108, 4, "ATM", 0.8, true)
        tOASIS.addMarker(n.x, n.y, n.z, 0.7, 0.7, 0.5, 0, 255, 125, 125, 50, 29, false, false, true)
...
function tOASIS.createAtm(t, u)
    ...
    tOASIS.createArea(v,u,1.5,6,i,k,function()end)
    local w = tOASIS.addMarker(u.x, u.y, u.z, 0.7, 0.7, 0.5, 0, 255, 125, 125, 50, 29, false, false, true)
    s[t] = {area = v, marker = w}
end
function tOASIS.deleteAtm(t)
    local x = s[t]
    if x then
        tOASIS.removeMarker(x.marker)
        tOASIS.removeArea(x.area)
        s[t] = nil
    end
end
local y = false
local function z(A)
    ...
    for p = 1, math.random(3, 4) do
        ...
        local E = true
        tOASIS.minigameCircularProgressBar({Difficulty = D, Timeout = 25000, onComplete = function(F)
            B = F
            E = false
        ...
        if not B then
            ...
            tOASIS.startCircularProgressBar("",2000,nil,function()end)
            TriggerServerEvent("OASIS:atmWireCutSparks", A, false)
            Citizen.Wait(2000)
        end
    end
    C = true
...
RegisterNetEvent("OASIS:atmWireCuttingSuccess",function()
...
RegisterNetEvent("OASIS:atmWireCutSparks",function(A)
    local L = a.robberyAtms[A]
    local M = G(L)
    tOASIS.loadPtfx("core")
...
RegisterNetEvent("OASIS:startAtmWireCutting",function(A)
    y = true
    local V = PlayerPedId()
    local L = a.robberyAtms[A]
    tOASIS.setCanAnim(false)
    tOASIS.setPlayerCombatTimer(30, false)
    tOASIS.setWeapon(V, "WEAPON_UNARMED", true)
    ...
    tOASIS.loadClipSet("move_ped_crouched")
...
RegisterNetEvent("OASIS:atmInkArea",function(A)
    local L = a.robberyAtms[A]
    tOASIS.loadPtfx("veh_xs_vehicle_mods")
    for p = 1, 10 do
...
RegisterNetEvent("OASIS:atmGenericAlarm",function(A)
    local L = a.robberyAtms[A]
...
local function a1(j)
    Y = math.random(6, 12)
    a0 = 0
    TriggerServerEvent("OASIS:getAtmHasBeenRobbed", j.robberyId)
end
local function a2(j)
    if y then
        TriggerServerEvent("OASIS:atmStopWireCutting", j.robberyId)
    end
    Z = 0
end
local function a3()
    ...
    Citizen.Wait(500)
    local a4 = tOASIS.getPlayerCoords()
...
local function a7()
    ...
    tOASIS.loadPtfx(a8[1])
    Citizen.Wait(500)
    UseParticleFxAsset(a8[1])
    local a4 = tOASIS.getPlayerCoords() + GetEntityForwardVector(PlayerPedId()) * 1.0
    ...
    if select(2, GetCurrentPedWeapon(V)) == "WEAPON_CROWBAR" then
        ...
        if IsDisabledControlJustPressed(0, 24) and not _ then
            Citizen.CreateThreadNow(function()
                ...
                tOASIS.loadAnimDict("melee@small_wpn@streamed_core")
                V = PlayerPedId()
                ...
                Z = Z + 1
                if Z >= Y then
                    TriggerServerEvent("OASIS:startAtmWireCutting", j.robberyId)
                end
...
Citizen.CreateThread(function()
    for A, L in pairs(a.robberyAtms) do
        tOASIS.createArea("atmrobbery_" .. A, L.xyz, 1.5, 6, a1, a2, a9, {robberyId = A})
    end
end)
RegisterNetEvent("OASIS:setAtmHasBeenRobbed",function(ac)
    a0 = ac
end)
