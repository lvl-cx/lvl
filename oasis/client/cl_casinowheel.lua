...
RegisterNetEvent("OASIS:frameworkCleanup")
AddEventHandler("OASIS:frameworkCleanup",function()
    DeleteEntity(b)
end)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('luckywheel', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Spin the wheel of OASIS (50,000 Chips)","",{RightLabel = "→→→"},true,function(e, f, g)
                if g then
                    tOASIS.notify("~r~Biome's dad needs to fix it.")
                    --TriggerServerEvent("OASIS:requestSpinLuckyWheel")
                end
            end)
        end)
    end
...
Citizen.CreateThread(function()
    ...
    local i = tOASIS.loadModel("vw_prop_vw_luckywheel_02a")
    local j = tOASIS.loadModel(c)
...
function goToWheel()
    if not d then
        ...
        local D = false
        while not D do
            local E = GetEntityCoords(tOASIS.getPlayerPed())
        ...
        SetEntityHeading(tOASIS.getPlayerPed(), 10.9754)
        Wait(2000)
...
AddEventHandler("OASIS:onClientSpawn",function(D, E)
    if E then
        ...
        for M, N in pairs(a) do
            tOASIS.addBlip(N.entryPosition.x, N.entryPosition.y, N.entryPosition.z, 681, 0, "OASIS Wheel", 0.7, true)
            tOASIS.createArea("oasiswheel_" .. M, N.entryPosition, 1.5, 6, H, J, K, {})
        end
	end
end)

RegisterNetEvent("OASIS:syncLuckyWheel")
AddEventHandler("OASIS:syncLuckyWheel",function(O)
    Wait(900)
    actuallyRollWheel(O)
end)

RegisterNetEvent("OASIS:spinLuckyWheel")
AddEventHandler("OASIS:spinLuckyWheel",function()
    goToWheel()
end)

RegisterNetEvent("OASIS:spinLuckyWheelReaction")
AddEventHandler("OASIS:spinLuckyWheelReaction",function(P)
    local y = tOASIS.getPlayerPed()
