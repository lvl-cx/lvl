...
Citizen.CreateThread(function()
	local j = tOASIS.addBlip(-552.90362548828, 5326.1733398438, 72.599586486816, 403, 3, "Meth Trader")
	local k = tOASIS.addBlip(133.73462677002, -1305.0340576172, 29.268741607666, 403, 4, "Cocaine Trader")
	local l = tOASIS.addBlip(103.47916412354, -1937.1091308594, 19.803705215454, 140, 2, "Weed Trader")
	local m = tOASIS.addBlip(3569.8334960938, 3662.5893554688, 33.920593261718, 586, 1, "Heroin Trader")
	local n = tOASIS.addBlip(-1118.4926757813, 4926.1889648438, 218.35691833496, 150, 1, "Large Arms Trader")
	local o = tOASIS.addBlip(5065.6201171875, -4591.3857421875, 2.8652405738831, 150, 1, "Large Arms Trader")
	local p = tOASIS.addBlip(1317.0300292969, 4309.8359375, 38.005485534668, 604, 8, "LSD North Trader", 1.0)
	local q = tOASIS.addBlip(2485.7429199219, -430.13989257813, 92.992835998535, 604, 8, "LSD South Trader", 1.0)
	local r = {
		{
			title = "Weed Trader",
			type = "radius",
...
AddEventHandler("OASIS:onClientSpawn",function(v, w)
	if w then
		TriggerServerEvent("OASIS:refreshTurfOwnershipData")
		TriggerServerEvent("OASIS:requestDrugPriceUpdate")
	end
end)
RegisterNetEvent("OASIS:attackGangCapture",function(x, y)
	f = true
	a = y
	otherMembersStartGangCapture(x, y, h[x].position)
end)
RegisterNetEvent("OASIS:updateTurfOwner",function(x, z)
	turfData[x].gangOwner = z
end)
RegisterNetEvent("OASIS:defendGangCapture",function(x, y)
	a = y
	e = true
...
RegisterNetEvent("OASIS:turfDefenseInterrupt",function()
	e = false
end)
RegisterNetEvent("OASIS:initiateGangCaptureCheck",function(y, C)
	b[y] = C
end)
RegisterNetEvent("OASIS:gotTurfOwnershipData",function(D)
...
function tOASIS.isPlayerInTurf()
    for E, F in pairs(h) do
        if #(tOASIS.getPlayerCoords() - F.position) < F.distance then
            return true
        end
    end
    return false
...
Citizen.CreateThread(function()
	for B, G in pairs(h) do
		tOASIS.addMarker(G.position.x,G.position.y,G.position.z,0.7,0.7,0.5,200,50,0,200,50,24,false,false,true)
	end
	for H, I in pairs(i) do
		tOASIS.addMarker(I.position.x,I.position.y,I.position.z,0.7,0.7,0.5,0,50,200,200,50,30,false,false,true)
	end
end)
local J = -1
	...
	if w then
		...
		local O = function(M)
			if IsControlJustPressed(1, 51) then
				if not PlayerIsInGang then
					tOASIS.notify("~r~You need to be in a gang to capture a turf!")
				elseif not f then
					TriggerServerEvent("OASIS:refreshTurfOwnershipData")
					TriggerServerEvent("OASIS:checkTurfCapture", M.turfId)
				end
				Wait(10000)
			end
		end
		for B, P in pairs(h) do
			tOASIS.createArea("turf_" .. B, P.position, 1.5, 6, L, N, O, {turfId = B})
		end
		local Q = function(M)
		...
		local S = function(M)
			if M.draw then
				...
				DrawAdvancedText(0.578, 0.349, 0.005, 0.0028, 0.437, "OASIS Trader Manager", 255, 255, 255, 255, 4, 0)
						...
						if T ~= nil and tonumber(T) then
							...
							if T <= 25 and T >= 0 then
								...
								if GetGameTimer() > K + 30000 then
									...
									if B == 1 then
										TriggerServerEvent("OASIS:setNewWeedPrice", T)
									elseif B == 2 then
										TriggerServerEvent("OASIS:setNewCocainePrice", T)
									elseif B == 3 then
										TriggerServerEvent("OASIS:setNewMethPrice", T)
									elseif B == 4 then
										TriggerServerEvent("OASIS:setNewHeroinPrice", T)
									elseif B == 5 then
										TriggerServerEvent("OASIS:setNewLargeArmsCommission", T)
									elseif B == 6 then
										TriggerServerEvent("OASIS:setNewLSDNorthPrice", T)
									elseif B == 7 then
										TriggerServerEvent("OASIS:setNewLSDSouthPrice", T)
									end
								else
									tOASIS.notify("~r~Rate limit, please wait 30s to change the commission.")
								end
							end
						end
					end
		...
		for B, G in pairs(i) do
			tOASIS.createArea("turfmanager_" .. B, G.position, 1.5, 6, Q, R, S, {turfId = B, draw = false})
		end
	end
end)
RegisterNetEvent("OASIS:captureOwnershipReturned",function(B, U, V)
	if not U then
		startGangCapture(B, h[B].position, V)
	else
		tOASIS.notify("~r~Turf already owned!")
	end
end)
function otherMembersStartGangCapture(x, y, W)
    if b[y] then
        ...
        Citizen.CreateThread(function()
			local tmpMarker = tOASIS.addMarker(W.x, W.y, W.z - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
			while f do
				local X = GetEntityCoords(tOASIS.getPlayerPed(), false)
				local Y = #(X - W)
				if Y >= 37.5 then
					tOASIS.notify("~r~Turf capture cancelled, you got too far!")
					f = false
					TriggerServerEvent("OASIS:gangDefenseLocationUpdate", y, "Attackers", true)
				elseif GetEntityHealth(tOASIS.getPlayerPed()) <= 102 then
					tOASIS.notify("~r~Turf capture failed, you died")
					f = false
					TriggerServerEvent("OASIS:gangDefenseLocationUpdate", y, "Attackers", true)
				end
				Wait(1000)
			end
			tOASIS.removeMarker(tmpMarker)
		end)
        Citizen.CreateThread(function()
			while f do
				if not d[y] then
...
function startGangCapture(x, W, y)
    if turfData[x].owned then
        tOASIS.notify("~r~You already own this turf!")
        TriggerServerEvent("OASIS:failCaptureTurfOwned", x)
    else
        TriggerServerEvent("OASIS:initiateGangCapture", x, y)
        local timer = g
        while b[y] == nil do
            Wait(0)
        end
        if b[y] then
            ...
            Citizen.CreateThread(function()
				tmpMarker = tOASIS.addMarker(W.x, W.y, W.z - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
				while f do
					local X = GetEntityCoords(tOASIS.getPlayerPed(), false)
					local Y = #(X - W)
					if Y >= 37.5 then
...
function defendGangCapture(x, a, Z, _, a0)
    ...
    Citizen.CreateThread(function()
		local tmpMarker = tOASIS.addMarker(Z, _, a0 - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
		while e do
			local X = GetEntityCoords(tOASIS.getPlayerPed(), false)
			local Y = #(X - vector3(h[x].x, h[x].y, h[x].z))
			if Y < 37.5 and GetEntityHealth(tOASIS.getPlayerPed()) > 102 then
				TriggerServerEvent("OASIS:gangDefenseLocationUpdate", a, "Defenders", true)
			elseif GetEntityHealth(tOASIS.getPlayerPed()) > 102 then
				TriggerServerEvent("OASIS:gangDefenseLocationUpdate", a, "Defenders", false)
			elseif GetEntityHealth(tOASIS.getPlayerPed()) <= 102 then
				tOASIS.notify("~r~Turf defense failed, you died")
				e = false
				TriggerServerEvent("OASIS:gangDefenseLocationUpdate", a, "Defenders", false)
			else
				TriggerServerEvent("OASIS:gangDefenseLocationUpdate", a, "Defenders", false)
...
RegisterNetEvent("OASIS:setBlockedStatus",function(y, a1)
	d[y] = a1
end)
RegisterNetEvent("OASIS:gangDefenseSuccess",function()
	e = false
end)
function gangCaptureSuccess(x, a)
    turfData[x].owned = true
    TriggerServerEvent("OASIS:gangCaptureSuccess", a)
end
function gangDefenseSuccess()
    TriggerServerEvent("OASIS:gangDefenseSuccess", a)
end
