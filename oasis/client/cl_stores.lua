...
RMenu.Add('OASISStores', 'main', RageUI.CreateMenu("", "~b~Shop", tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight(), 'banners', 'shop'))
RMenu.Add("OASISStores", "confirm", RageUI.CreateSubMenu(RMenu:Get('OASISStores', 'main',  tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight())))

RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("OASISStores", "main")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for k, v in pairs(cfg.shopItems) do
                ...
                end, RMenu:Get("OASISStores", "confirm"))
            end
        end)
    end
end)
...
local Index = 1
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("OASISStores", "confirm")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Button("Confirm Purchase" , nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent("OASIS:BuyStoreItem", cHash, tonumber(Index))
                end
            end, RMenu:Get("OASISStores", "main")) 
        end) 
    end
end)
        ...
        for k, v in pairs(cfg.shops) do
            ...
            if isInArea(v, 1.0) and inMenu == false then
                ...
                if IsControlJustPressed(0, 51) then 
                    ...
                    RageUI.Visible(RMenu:Get("OASISStores", "main"), true)
                    cLoaction = v
                end
            end
