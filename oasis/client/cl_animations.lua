...
function tOASIS.GetClosestPlayer(ao)
    local ap=-1
    local aq=-1
...
RegisterCommand("umbrella",function(f,g)
    ...
    if DoesEntityExist(k)and canAnim()and not IsEntityDead(k)and d()then 
        ...
        if IsEntityPlayingAnim(k,i,"base",3)then 
			...
			DecorSetInt(prop, "OASISACVeh", 955)
...
function tOASIS.globalSurrenderring()
...
RegisterNetEvent("OASIS:startRobbingPlayer",function()
    D=GetGameTimer()
end)
RegisterNetEvent("OASIS:endRobbingPlayer",function()
    D=nil 
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        if (IsDisabledControlPressed(1,19) or IsControlPressed(1, 19)) and (IsDisabledControlPressed(1,63) or IsControlPressed(1, 63)) and d()and not sittingAtBlackjackTable then
            if not a2 and tOASIS.globalNHSOnDuty() then 
                a2=true
                ...
                detachFlag=1
                boneIndex=11816
                local aa=tOASIS.GetClosestPlayer(3)
                if aa then 
                    local ab=GetPlayerServerId(aa)
                    if ab~=-1 then 
                        TriggerServerEvent("OASIS:animationSync",aa,a3,a4,a5,a6,a7,y,ab,a9,a8,controlFlagMe,controlFlagTarget,animFlagTarget,detachFlag,boneIndex)
                    end 
                end 
            else 
                a2=false
                ...
                if ac~=0 then 
                    TriggerServerEvent("OASIS:animationSyncStop",ac)
                end 
            end
            Wait(250)
        end
...
RegisterNetEvent("OASIS:animationSyncClStop")
AddEventHandler("OASIS:animationSyncClStop",function()
    ClearPedSecondaryTask(PlayerPedId())
    DetachEntity(PlayerPedId())
end)
RegisterCommand("baseball",function(f,g)
    ...
    if aa~=nil and not c()and GetEntityHealth(GetPlayerPed(aa))>102 and d()and canAnim()then 
        TriggerServerEvent("OASIS:animationSync",aa,a3,a4,a5,a6,a7,y,ac,a9)
...
RegisterCommand("baseball9", function(f, g)
	...
	local ac = GetPlayerServerId(aa)
	if aa ~= nil and (not c()) and GetEntityHealth(GetPlayerPed(aa)) > 102 and d() and canAnim() then
		TriggerServerEvent("OASIS:animationSync", aa, a3, a4, a5, a6, a7, y, ac, a9, a8, controlFlagMe, controlFlagTarget, animFlagTarget, detachFlag)
	end
end, false)
...
RegisterNetEvent("OASIS:animationSyncTarget")
AddEventHandler("OASIS:animationSyncTarget",function(ac,ad,ae,a6,a7,y,a9,a8,af,animFlagTarget,detachFlag,boneIndex)
    local e=PlayerPedId()
...
RegisterNetEvent("OASIS:animationSyncMe")
AddEventHandler("OASIS:animationSyncMe",function(ad,ah,a9,af,ai)
    local e=PlayerPedId()
    RequestAnimDict(ad)
    while not HasAnimDictLoaded(ad)do 
        Citizen.Wait(10)
...
exports("canAnim",function()
    return tOASIS.canAnim()
end)


local ah = false
RegisterNetEvent("OASIS:startShavingPlayer",function(ai)
	...
	tOASIS.loadAnimDict("misshair_shop@hair_dressers")
	...
	tOASIS.loadModel("prop_clippers_01")
...
RegisterNetEvent("OASIS:startBeingShaved",function(aq)
...
RegisterNetEvent("OASIS:playDelayedShave",function(au)
	...
	local aw = tOASIS.getPlayerCoords()
...
RegisterNetEvent("OASIS:setAsShaved",function(ay)
	ah = true
	Citizen.Wait(ay)
	ah = false
end)
