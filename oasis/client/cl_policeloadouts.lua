...
RMenu.Add("policeloadouts","main",RageUI.CreateMenu("","Please Select Division",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight(),"banners","gunstore"))
RMenu.Add("policeloadouts","confirm",RageUI.CreateSubMenu(RMenu:Get("policeloadouts", "main"),"","Confirm Selection",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight()))
    ...
    if RageUI.Visible(RMenu:Get('policeloadouts', 'confirm')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.Button("Confirm", nil, {RightBadge = "→→→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent('OASIS:selectLoadout', selectedLoadout)
...
RegisterNetEvent('OASIS:gotLoadouts')
AddEventHandler('OASIS:gotLoadouts', function(loadoutsTable)
    ...
    if q then
        ...
        local u = function(s)
            if IsControlJustPressed(1, 38) then
                TriggerServerEvent('OASIS:getPoliceLoadouts')
        ...
        for k,v in pairs(loadoutCoords) do
            tOASIS.createArea("police_loadouts_"..k, v, 1.5, 6, r, t, u, {locationId = k})
        end
    end
end)