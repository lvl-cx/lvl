...
AddEventHandler("OASIS:onClientSpawn",function(D, E)
    if E then
		for b, c in pairs(a.stations) do
			tOASIS.addBlip(c.x, c.y, c.z, 361, 4, "Petrol Station", 0.6)
		end
	end
end)
local d = false
...
AddEventHandler("fuel:startFuelUpTick",function(y, s, j)
	...
	if y then
		TriggerServerEvent("OASIS:takeAmount", math.floor(g))
	end
	local G = GetEntityModel(j)
	local H = tOASIS.getVehicleIdFromHash(G)
	if H then
		local I = GetFuel(j)
		TriggerServerEvent("OASIS:updateFuel", H, math.floor(I * 1000) / 1000)
	end
	g = 0.0
end)
AddEventHandler("fuel:refuelFromPump",function(y, s, j)
	...
	tOASIS.loadAnimDict("timetable@gardener@filling_can")
				...
				elseif d then
					...
					if h >= a.jerryCanCost then
						if not HasPedGotWeapon(s, 883325847) then
							...
							if IsControlJustReleased(0, 38) then
								tOASIS.allowWeapon("883325847")
								GiveWeaponToPed(s, 883325847, 4500, false, true)
								TriggerServerEvent("OASIS:takeAmount", a.jerryCanCost)
								h = 50000
							end
						else
							...
							if W > 0 then
								...
								if IsControlJustReleased(0, 38) then
									TriggerServerEvent("OASIS:takeAmount", math.floor(W))
