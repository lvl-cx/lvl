...
local c
local d
AddEventHandler("OASIS:onClientSpawn",function(e, f)
    if f then
        local g = function()
            if not c then
        ...
        local i = function(j)
            if not c and IsControlJustPressed(1, 51) then
                TriggerServerEvent("OASIS:requestStartCooking", j.recipe)
        ...
        for k, l in pairs(a) do
            for m, n in pairs(l) do
                tOASIS.createArea("cooking_" .. k .. "_" .. m,n.cookingPosition,1.5,6,g,h,i,{recipe = k, recipeTable = a[k], index = m})
                tOASIS.addMarker(n.cookingPosition.x,n.cookingPosition.y,n.cookingPosition.z - 0.2,0.5,0.5,0.5,0,50,255,170,50,20,false,false,true)
            end
        end
    end
end)
...
local function t()
    for p, q in pairs(b) do
        for r, s in pairs(q) do
            local u = tOASIS.loadModel(s.model)
...
AddEventHandler("OASIS:onClientSpawn",function(e, f)
    if f then
        ...
        for w, x in pairs(b) do
            tOASIS.createArea("cooking_staticobjects_" .. w, x[1].position, 100, 5, g, h, i, {})
        end
    end
end)
...
local function B()
    CreateThread(function()
        tOASIS.startCircularProgressBar("",3000,nil,function()end)
    end)
    local C = PlayerPedId()
    tOASIS.loadAnimDict("amb@prop_human_bum_bin@enter")
    TaskPlayAnim(C, "amb@prop_human_bum_bin@enter", "enter", 8.0, 8.0, -1, 1, 0.2)
    RemoveAnimDict("amb@prop_human_bum_bin@enter")
    Wait(1500)
    tOASIS.loadAnimDict("amb@prop_human_bum_bin@exit")
...
local function D(E)
    local F = true
    local G
    tOASIS.minigameCircularProgressBar({Difficulty = E, Timeout = 100000, onComplete = function(H)
        if H then
            G = true
        else
            G = false
...
function cookBBQ()
    ...
    tOASIS.startCircularProgressBar("",10000,nil,function()end)
    local L = 0
    ...
    while L < 3 do
        ...
        if H then
            ...
            tOASIS.announceMpSmallMsg("Disaster", "You dropped the burger patty, start again!", 6, 3000)
        end
        Wait(2500)
        tOASIS.startCircularProgressBar("",10000,nil,function()end)
        Wait(2500)
    ...
    tOASIS.announceMpSmallMsg("Success", "You made a burger!", 18, 3000)
    ClearPedTasks(C)
    StopParticleFxLooped(K, true)
end
local function M()
    local C = tOASIS.getPlayerPed()
    local N = tOASIS.getPlayerCoords()
    local u = tOASIS.loadModel("prop_food_bs_tray_02")
    ...
    SetModelAsNoLongerNeeded(u)
    tOASIS.loadAnimDict("anim@heists@box_carry@")
    TaskPlayAnim(C, "anim@heists@box_carry@", "idle", 2.0, 2.0, -1, 51, 0, false, false, false)
    Citizen.CreateThread(function()
        while tOASIS.doesAreaExist("temp_foodorder_complete") do
...
local function T(U)
    ...
    local W = tOASIS.addMarker(V.x, V.y, V.z + 0.3, 0.15, 0.15, 0.15, 255, 215, 0, 170, 50, 0, false, true, true)
    local X = tOASIS.addBlip(V.x, V.y, V.z, 1, 2, "Delivery Point", 0.6)
    SetBlipFlashes(X, true)
    tOASIS.createArea("temp_foodorder_complete",V,1.5,6.0,function()
        tOASIS.removeArea("temp_foodorder_complete")
        tOASIS.removeMarker(W)
        tOASIS.removeBlip(X)
...
RegisterNetEvent("OASIS:cookingInstruction",function(Z)
    for r, _ in pairs(R) do
        tOASIS.removeArea("temp_cooking_instruction")
        tOASIS.removeMarker(_)
    end
    for r, Y in pairs(S) do
        tOASIS.removeBlip(Y)
    end
    ...
    local W = tOASIS.addMarker(V.x, V.y, V.z + 0.3, 0.15, 0.15, 0.15, 255, 215, 0, 170, 50, 0, false, true, true)
    table.insert(R, W)
    local X = tOASIS.addBlip(V.x, V.y, V.z, 1, 2, "Ingredient", 0.6)
    SetBlipFlashes(X, true)
    table.insert(S, X)
    local g = function()
    end
    ...
    local i = function()
        if IsControlJustPressed(1, 51) then
            ...
            if Z == "bbq" then
                ...
                tOASIS.removeMarker(W)
                T(function()
                    TriggerEvent("OASIS:playItemBoughtSound")
                    TriggerServerEvent("OASIS:pickupCookingIngredient", c, Z)
                    tOASIS.setCanAnim(true)
                    tOASIS.announceMpSmallMsg("Success", "You earned £4,000, now get back to work!", 18, 3000)
                end)
            else
                tOASIS.removeMarker(W)
                tOASIS.removeBlip(X)
    ...
    tOASIS.createArea("temp_cooking_instruction", V, 1.0, 6, g, h, i)
end)
RegisterNetEvent("OASIS:beginCooking",function(a0)
    TriggerEvent("OASIS:smallAnnouncement", "Cooking", "Make a " .. a0 .. ".", 15, 5000)
end)
RegisterNetEvent("OASIS:finishedCooking",function()
    c = nil
end)
