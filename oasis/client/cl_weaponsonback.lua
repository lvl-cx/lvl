local a = module("oasis-weapons", "cfg/cfg_weaponsonback")
...
local function g()
    ...
    local i = tOASIS.getWeapons()
    local j = false
    local k = globalOnPoliceDuty and tOASIS.getPlayerVehicle() ~= 0
    for l in pairs(i) do
        local m = GetHashKey(l)
        local n = a.weapons[m]
        if n then
            local o = (not k or n.type ~= "Heavy") and (tOASIS.isEmergencyService() or not isInGreenzone) and not tOASIS.isPlayerInAnimalForm()
