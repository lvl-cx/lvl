RMenu.Add("nhs","main",RageUI.CreateMenu("NHS", "Status: ~b~Available", tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight()))
RMenu.Add("nhs","equipment",RageUI.CreateSubMenu(RMenu:Get("nhs", "main"),"Equipment","Status: ~b~Available",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight()))
RMenu.Add("nhs","observations",RageUI.CreateSubMenu(RMenu:Get("nhs", "main"),"Observations","Status: ~b~Available",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight()))
RMenu.Add("nhs","lifepak",RageUI.CreateSubMenu(RMenu:Get("nhs", "main"),"Observations","Status: ~b~Available",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight()))
RMenu.Add("nhs","objects",RageUI.CreateSubMenu(RMenu:Get("nhs", "main"),"Objects","Status: ~b~Available",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight()))
RegisterKeyMapping("nhs", "NHS Menu", "keyboard", "U")
TriggerEvent("chat:addSuggestion", "/nhs", "Toggle the NHS Menu")
RegisterNetEvent("OASIS:beginRevive",function(a, b, c, d)
    print("nuser_id", b)
...
RegisterNetEvent("OASIS:attachLifepak",function(a, b, c, d)
            ...
            if e.active then
                ...
                if e.readyToRevive then
                    RageUI.Button("Shock Patient",nil,{RightLabel = ""},true,function(g, h, i)
                        if i then
                            TriggerServerEvent("OASIS:finishRevive", e.perm_id)
                            e.readyToRevive = false
                            TriggerServerEvent("OASIS:playNhsSound", "shockAdvisedCharging")
                        end
                    end)
                else
                    RageUI.Button("Detach from patient",nil,{RightLabel = ""},true,function(g, h, i)
                        if i then
                            ...
                            TriggerServerEvent("OASIS:playNhsSound", "threeBeeps")
                        end
                    end)
                end
            else
    ...
    if RageUI.Visible(RMenu:Get('nhs', 'lifepak')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                if e.readyToRevive then
                    RageUI.Button("Shock Patient",nil,{RightLabel = ""},true,function(g, h, i)
                        if i then
                            TriggerServerEvent("OASIS:finishRevive", e.perm_id)
                            e.readyToRevive = false
                            TriggerServerEvent("OASIS:playNhsSound", "carryOutShock")
                        end
                    end)
                end
            end
            RageUI.Button("Enable Metronome",nil,{RightLabel = "100-120bpm"},true,function(g, h, i)
                if i then
                    TriggerServerEvent("OASIS:playNhsSound", "metronome")
                end
            end)
...
function carryLifePak()
    ...
    f.active = true
    local l = tOASIS.loadModel("prop_lifepak")
...
function changeUpObs()
    Citizen.CreateThread(function()
        ...
        if not e.dead then
            ...
            if n == 3 then
                Wait(3000)
                TriggerServerEvent("OASIS:playNhsSound", "checkPatientAlarm")
            end
...
function beginObs(a, u, v, w, d)
    TriggerServerEvent("OASIS:playNhsSound", "threeBeeps")
    Wait(3000)
    TriggerServerEvent("OASIS:playNhsSound", "analysingHeartRythm")
    e = {}
    ...
    if e.coma then
        ...
        if IsEntityDead(GetPlayerPed(u)) then
            ...
            if e.readyToRevive then
                ...
                5000)
                TriggerServerEvent("OASIS:playNhsSound", "pushToShock")
            end
        end
    end
end
function connectObsToPatient()
    TriggerServerEvent("OASIS:attachLifepakServer")
end
RegisterNetEvent("OASIS:clientPlayNhsSound",function(x, y)
...
RegisterNetEvent("OASIS:returnRevive",function()
    TriggerServerEvent("OASIS:playNhsSound", "checkPatientAlarm")
    e.bloodPressure1 = math.random(20, 199)
    e.bloodPressure2 = math.floor(e.bloodPressure1 - e.bloodPressure1 / 2.9)
    if e.bloodPressure2 > e.bloodPressure1 then
    ...
    e.coma = false
    Wait(3000)
    TriggerServerEvent("OASIS:playNhsSound", "safeToTouch")
    Wait(12000)
    TriggerServerEvent("OASIS:playNhsSound", "threeBeeps")
end)
    ...
    if globalNHSOnDuty or globalLFBOnDuty then
        ...
        if E ~= GetHashKey("WEAPON_UNARMED") then
            if not table.has(B, E) then
                tOASIS.setWeapon(D, "WEAPON_UNARMED", true)
            end
        end
    end
end
tOASIS.createThreadOnTick(C)

local K = false
RegisterNetEvent("OASIS:attemptCPR",function()
    K = true
    tOASIS.loadAnimDict("missheistfbi3b_ig8_2")
    ...
    while K do
        if not IsEntityPlayingAnim(PlayerPedId(), "missheistfbi3b_ig8_2", "cpr_loop_paramedic", 3) then
            K = false
            TriggerServerEvent("OASIS:cancelCPRAttempt")
            break
        end
        Citizen.Wait(0)
    end
end)
RegisterNetEvent("OASIS:cancelCPRAttempt",function()
    K = false
    if tOASIS.canAnim() then
