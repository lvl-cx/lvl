...
RegisterNetEvent("OASIS:receiveBarbs",function(e)
    c = true
    d = e
end)
RegisterNetEvent("OASIS:receiveActivation",function()
    if not tOASIS.isStaffedOn() then
        displayTaserNotification("You are currently being ~b~activated~w~.")
        SetPedToRagdoll(tOASIS.getPlayerPed(), 5000, 5000, 0, 0, 0, 0)
    end
end)
RegisterNetEvent("OASIS:arcSuccessful",function()
    ...
    TriggerServerEvent("OASIS:playTaserSound", GetEntityCoords(tOASIS.getPlayerPed()), "arcsound")
    b = false
...
RegisterNetEvent("playTaserSoundClient",function(f, g)
    local h = GetEntityCoords(tOASIS.getPlayerPed(), true)
    local i = #(h - f)
    if i <= 15 then
        SendNUIMessage({transactionType = g})
    end
end)
RegisterNetEvent("OASIS:barbsNoLonger",function()
    c = false
    d = 0
    displayTaserNotification("Your taser ~b~barbs ~w~have been ~b~disconnected~w~.")
end)
RegisterNetEvent("OASIS:barbsRippedOut",function(e)
...
RegisterNetEvent("OASIS:reloadTaser",function(j)
    if GetSelectedPedWeapon(tOASIS.getPlayerPed()) == GetHashKey(a.hash) then
        MakePedReload(tOASIS.getPlayerPed())
        if a.cartridges == 0 then
            a.cartridges = 2
            if a.reactivate1.active then
                TriggerServerEvent("OASIS:barbsNoLongerServer", a.reactivate1.serverId)
            end
            if a.reactivate2.active then
                TriggerServerEvent("OASIS:barbsNoLongerServer", a.reactivate2.serverId)
            end
            a.reactivate1 = {active = false, ped = 0, serverId = 0}
            a.reactivate2 = {active = false, ped = 0, serverId = 0}
        ...
        if k.weapon == GetHashKey(a.hash) then
            if IsControlJustPressed(1, 308) then
                if a.reactivate1.active == true then
                    ...
                    TriggerServerEvent("OASIS:playTaserSound", k.playerCoords, "reactivate")
                    TriggerServerEvent("OASIS:reactivatePed", a.reactivate1.serverId)
                end
            elseif IsControlJustPressed(1, 307) then
                if a.reactivate2.active == true then
                    ...
                    TriggerServerEvent("OASIS:playTaserSound", k.playerCoords, "reactivate")
                    TriggerServerEvent("OASIS:reactivatePed", a.reactivate2.serverId)
                end
            elseif IsControlJustPressed(1, 121) and b then
                TriggerServerEvent("OASIS:arcTaser")
            end
            if a.cartridges < 1 then
                ...
                if IsPedShooting(k.playerPed) then
                    TriggerServerEvent("OASIS:playTaserSound", k.playerCoords, "taser")
                    a.cartridges = a.cartridges - 1
                    local l, m = GetEntityPlayerIsFreeAimingAt(k.playerId)
                    if l and IsEntityAPed(k.playerPed) then
                        local n = tOASIS.getPedServerId(m)
                        TriggerServerEvent("OASIS:receiveBarbsServer", n)
                        if n ~= nil then
                            if a.cartridges == 1 then
...
tOASIS.createThreadOnTick(func_taserHandler)
...
Citizen.CreateThread(function()
    SetPedMinGroundTimeForStungun(tOASIS.getPlayerPed(), 15000)
    while true do
        if IsPedBeingStunned(tOASIS.getPlayerPed()) then
            stunGun()
        end
        Wait(100)
    end
        ...
        if c then
            while c do
                ...
                if IsControlJustPressed(1, 38) then
                    c = false
                    if d and d > 0 then
                        TriggerServerEvent("OASIS:barbsRippedOutServer", d)
                    end
                    d = 0
...
function tOASIS.setStunExitsEarly()
    p = true
end
