    ...
    if tOASIS.globalOnPoliceDuty() then 
        if m[1]~=nil then 
            if tonumber(m[1])>100 then 
                d=tonumber(m[1])
                tOASIS.notify(string.format("~g~Maximum speed set to %smph",tonumber(m[1])))
            else 
                d=101
                tOASIS.notify("~r~Minimum speed you can set is 101mph!")
            end 
        end 
    else 
        tOASIS.notify("~r~Speed gun is not enabled!")
    end 
end)
...
tOASIS.createThreadOnTick(func_drawRadar)
Citizen.CreateThread(function()
    while true do 
        if tOASIS.globalOnPoliceDuty() and not usingDelgun then
            if GetSelectedPedWeapon(tOASIS.getPlayerPed())==a then 
                b=true 
            else 
                b=false 
            end
            if b and tOASIS.getPlayerVehicle()==0 then 
                local n=tOASIS.getPlayerId()
                if IsPlayerFreeAiming(n)then 
                    ...
                    if p~=0 and GetPedInVehicleSeat(p,-1)==e and not IsPedInAnyPlane(e) and not IsPedInAnyHeli(e) then 
                        ...
                        if k[f]~=nil then 
                            ...
                            tOASIS.notify(string.format("~h~~r~Vehicle Flagged:~s~~n~Plate %s is flagged for:~n~%s",f,r))
                            soundPlayed=true
                        ...
                        if h>d and h>101 then 
                            ...
                            if not j[q]then 
                                ...
                                TriggerServerEvent("OASIS:speedGunFinePlayer",GetPlayerServerId(NetworkGetPlayerIndexFromPed(e)),h-d)
                                Citizen.Wait(3000)
                            else 
                                tOASIS.notify("~r~This vehicle has been fined recently!")
                            end 
                            else i="~w~"
                        end 
                    end 
...
RegisterNetEvent("OASIS:speedGunPlayerFined")
AddEventHandler("OASIS:speedGunPlayerFined",function()
    PlaySoundFrontend(-1,"ScreenFlash","MissionFailedSounds",1)
    StartScreenEffect("FocusOut",0,false)
    Wait(2000)
    StopScreenEffect("FocusOut")
end)
AddEventHandler("OASIS:setFlagVehicles",function(s)
    k=s 
end)