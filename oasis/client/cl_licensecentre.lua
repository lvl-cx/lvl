...
RMenu.Add('LicenseCentre', 'main', RageUI.CreateMenu("", "~b~" .. "Job Centre", tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight(), "banners", "groups"))
RMenu.Add("LicenseCentre", "confirm", RageUI.CreateSubMenu(RMenu:Get('LicenseCentre', 'main',  tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight())))
    ...
    if q then
        ...
        local u = function(s)
            if IsControlJustPressed(1, 38) then
                TriggerServerEvent('OASIS:getOwnedLicenses')
                Wait(500)
            ...
            tOASIS.addMarker(licensecentre.location.x, licensecentre.location.y,licensecentre.location.z-0.98,0.7,0.7,0.5,255, 255,255,250,50,27,false,false)
        end
        tOASIS.createArea("licensecentre", licensecentre.location, 1.5, 6, r, t, u, {})
        tOASIS.addBlip(licensecentre.location.x, licensecentre.location.y, licensecentre.location.z, 457, 2, "Job Centre", 0.6, true)
    end
end)
...
function func_licenseMenuControl()
    if IsControlJustPressed(0, 167) then
        if a then
            ...
            TriggerServerEvent("OASIS:GetLicenses")
        end
    end
    if a then
        ...
        DrawAdvancedText(0.595, 0.213, 0.005, 0.0028, 1.0, "OASIS Licenses", 255, 255, 255, 255, 1, 0)
...
if not tOASIS.isPurge() then
    tOASIS.createThreadOnTick(func_licenseMenuControl)
end
RegisterNetEvent("OASIS:RecievedLicenses",function(i)
    b = i
    a = true
end)
RegisterNetEvent("OASIS:gotOwnedLicenses",function(i)
    for k,v in pairs(licensecentre.licenses) do
        v.notOwned = true
