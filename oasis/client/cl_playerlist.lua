...
function func_playerlistControl()
    if IsUsingKeyboard(2) then
        if IsControlJustPressed(0, 212) then
            a = not a
            TriggerServerEvent("OASIS:getPlayerListData")
            Wait(100)
...
tOASIS.createThreadOnTick(func_playerlistControl)
RegisterNUICallback("closeOASISPlayerList",function(j, k)
    SetNuiFocus(false, false)
end)
AddEventHandler("OASIS:onClientSpawn",function(h, l)
    if l then
        TriggerServerEvent("OASIS:getPlayerListData")
    end
end)

RegisterNetEvent("OASIS:gotFullPlayerListData",function(staff, police, nhs, lfb, hmp, civillians)
...
local u,v,x
RegisterNetEvent("OASIS:playerListMetaUpdate",function(t)
...
function sendFullPlayerListData()
    ...
    SendNUIMessage({setServerMetaData = '<img src="playerlist_images/oasis.png" align="top" width="20px",height="20px"><span class="staff">' ..tostring(H) .. "</span>"})
...
Citizen.CreateThread(function()
    while true do
        ...
        if u ~= nil and v ~= nil and x ~= nil then
            ...
            SetDiscordRichPresenceAssetText("discord.gg/oasisv") 
            SetDiscordRichPresenceAssetSmall('mainlogo')
            SetDiscordRichPresenceAssetSmallText('OASIS')
            SetRichPresence("[ID:" .. tostring(tOASIS.getUserId()) .. "] | " .. tostring(v) .. "/" .. tostring(x))
        end
        Wait(15000)
    end
end)