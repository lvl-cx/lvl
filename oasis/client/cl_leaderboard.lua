...
RegisterCommand("leaderboardtest",function()
    if tOASIS.isDev() then
        local b = tOASIS.createLeaderboard(a)
        Wait(1000)
        tOASIS.closeLeaderboard(b)
    end
end,false)
local b = 1
local c = {}
...
function tOASIS.closeLeaderboard(b)
    c[b] = false
end
function tOASIS.createLeaderboard(m)
    local n = Scaleform("SC_LEADERBOARD")
    n.RunFunction("SET_DISPLAY_TYPE", {1})
    n.RunFunction("SET_MULTIPLAYER_TITLE", {"OASIS RP Event"})
...
RegisterNetEvent(
    "OASIS:initScoreboard",
    function(t)
        d = t
    end
)
function tOASIS.updateScoreboard(u, v, w)
    local x
    ...
    if u == -1 then
        ...
        if x then
            ...
            print("error: OASIS:updateScoreboard couldn't find row with values", u, v, w)
        end
    end
end
RegisterNetEvent(
    "OASIS:updateScoreboard",
    function(u, v, w)
        tOASIS.updateScoreboard(u, v, w)
    end
)
function tOASIS.setPlayerCanOpenLeaderboard(F)
    e = F
end
function tOASIS.clearLeaderboardData()
    d = {columns = {}, rows = {}}
end
function tOASIS.isLeaderboardSetup()
    return table.count(d.rows) > 0
end
function tOASIS.removePlayerFromLeaderboard(u)
