RMenu.Add("oasisscenemenu","main",RageUI.CreateMenu("","~b~OASIS Traffic Scene Menu",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight(),"banners","scenemenu"))
RMenu.Add("oasisscenemenu","objects",RageUI.CreateSubMenu(RMenu:Get("oasisscenemenu", "main"),"","~b~Spawn Objects",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight(),"banners","scenemenu"))
RMenu.Add("oasisscenemenu","speedZone",RageUI.CreateSubMenu(RMenu:Get("oasisscenemenu", "main"),"","~b~Set a speed zone",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight(),"banners","scenemenu"))
RMenu.Add("oasisscenemenu","spikes",RageUI.CreateSubMenu(RMenu:Get("oasisscenemenu", "main"),"","~b~Set a spike trap",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight(),"banners","scenemenu"))
local b = {}
local c = {}
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('oasisscenemenu', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            E()
            RageUI.ButtonWithStyle("Object Menu",nil,{RightLabel = "→→→"},true,function(K, L, M)
            end,RMenu:Get("oasisscenemenu", "objects"))
            RageUI.ButtonWithStyle("Speed Zone",nil,{RightLabel = "→→→"},true,function(K, L, M)
            end,RMenu:Get("oasisscenemenu", "speedZone"))
            if tOASIS.globalOnPoliceDuty() then
                RageUI.ButtonWithStyle("Spike Strip Menu",nil,{RightLabel = "→→→"},true,function(K, L, M)
                end,RMenu:Get("oasisscenemenu", "spikes"))
            end
            RageUI.ButtonWithStyle("Close Menu",nil,{RightLabel = "→→→"},true,function(K, L, M)
                if M then
                    RageUI.Visible(RMenu:Get("oasisscenemenu", "main"), false)
                end
            end)
        end)
    end
    if RageUI.Visible(RMenu:Get('oasisscenemenu', 'objects')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
    ...
    if RageUI.Visible(RMenu:Get('oasisscenemenu', 'speedZone')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("Create Speedzone",nil,{},true,function(K, L, M)
                if M then
                    if p == 0 then
                        tOASIS.notify("~r~Please set a radius")
                    elseif q == 0 then
                        tOASIS.notify("~r~Please set a speed")
                    else
                        TriggerServerEvent("OASIS:createSpeedZone", tOASIS.getPlayerCoords(), q + 0.0, p + 0.0)
                        tOASIS.notify("~b~Speed zone created")
                    end
                end
            end,nil)
            RageUI.ButtonWithStyle("Delete Speedzone",nil,{},true,function(K, L, M)
                if M then
                    local Q = tOASIS.getPlayerCoords()
                    local R = nil
                    local S = 250.0
                    ...
                    if R then
                        TriggerServerEvent("OASIS:deleteSpeedZone", R)
                        tOASIS.notify("~r~Speed zone removed")
                    end
                end
            end,nil)
        end)
    end
    if RageUI.Visible(RMenu:Get('oasisscenemenu', 'spikes')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
...
RegisterNetEvent("OASIS:toggleTrafficMenu",function()
    RageUI.Visible(RMenu:Get("oasisscenemenu", "main"), not RageUI.Visible(RMenu:Get("oasisscenemenu", "main")))
    if globalOnPoliceDuty and tOASIS.hasPoliceCallsign() then
        RMenu:Get("oasisscenemenu", "main"):SetSubtitle("~b~MPD~w~: " .. tOASIS.getPoliceCallsign() .. " - " .. tOASIS.getPoliceRank() .. " - " .. tOASIS.getPoliceName())
    end
end)
...
function deleteObject()
    for n, o in pairs(k) do
        ...
        local Y, Z, _ = table.unpack(GetEntityCoords(tOASIS.getPlayerPed(), true))
        if DoesObjectOfTypeExistAtCoords(Y, Z, _, 0.9, X, true) then
            ...
            if NetworkGetEntityIsNetworked(a0) and not NetworkHasControlOfEntity(a0) then
                local a1 = tOASIS.getNetId(a0, "deleteObject()")
                if a1 ~= 0 then
                    TriggerServerEvent("OASIS:requestSceneObjectDelete", a1)
                end
            else
                DeleteObject(a0)
            end
        ...
        for n, a2 in pairs(b) do
            if DoesEntityExist(a2) then
                if NetworkGetEntityIsNetworked(a2) then
                    ...
                    if a3 ~= 0 then
                        TriggerServerEvent("OASIS:requestSceneObjectDelete", a3)
                    end
                else
                    DeleteEntity(a2)
                end
...
function deleteSpike()
    local Q = GetEntityCoords(tOASIS.getPlayerPed())
    local a8 = getClosetSpike(Q)
    if a8 then
        TriggerServerEvent("OASIS:removeSpike", a8[2])
    end
end
RegisterNetEvent("OASIS:addSpike",function(Q, a7)
    c[a7] = {Q, a7}
end)
RegisterNetEvent("OASIS:deleteSpike",function(a7)
    c[a7] = nil
end)
local a9 = 1
Citizen.CreateThread(function()
...
Citizen.CreateThread(function()
    while true do
        local a4, ah = tOASIS.getPlayerVehicle()
        if a4 ~= 0 and ah then
            local ai = false
            local aj = 1000.0
            local A = tOASIS.getPlayerCoords()
...
RegisterNetEvent("OASIS:createSpeedZone",function(T, ak, q, p)
...
RegisterNetEvent("OASIS:deleteSpeedZone",function(T)
    local U = j[T]
    if U then
        RemoveBlip(U.blip)
