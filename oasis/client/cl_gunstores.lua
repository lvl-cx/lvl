local weapons = module("oasis-weapons", "cfg/weapons")
local a=false
local b
local c
...
RMenu.Add("OASISGunstore","mainmenu",RageUI.CreateMenu("","",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight(),"banners", "gunstore"))
RMenu:Get("OASISGunstore","mainmenu"):SetSubtitle("~b~GUNSTORE")
RMenu.Add("OASISGunstore","type",RageUI.CreateSubMenu(RMenu:Get("OASISGunstore","mainmenu"),"","~b~Purchase Weapon or Ammo",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight(),"banners", "gunstore"))
RMenu.Add("OASISGunstore","confirm",RageUI.CreateSubMenu(RMenu:Get("OASISGunstore","type"),"","~b~Purchase confirm your purchase",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight(),"banners", "gunstore"))
RMenu.Add("OASISGunstore","vip",RageUI.CreateSubMenu(RMenu:Get("OASISGunstore","mainmenu"),"","~b~Purchase Weapon or Ammo",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight(),"banners", "gunstore"))

RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("OASISGunstore", "mainmenu")) then
        ...
        if b~=nil and i~=nil then 
            if tOASIS.isPlatClub() then 
                if b=="VIP"then 
                    ...
                    end,RMenu:Get("OASISGunstore","vip"))
                end 
            end
                    ...
                    for o,p in pairs(sortedKeys(n))do 
                        ...
                        if p~="_config"then 
                            ...
                            if p=="item|fillUpArmour"then 
                                local z=GetPedArmour(tOASIS.getPlayerPed())
                                local A=100-z
                                y=A*1000
                                x=true 
                            end
                            ...
                            end,RMenu:Get("OASISGunstore","type"))
                        end 
                    end 
                end 
            end 
    ...
    if RageUI.Visible(RMenu:Get("OASISGunstore", "confirm")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            RageUI.ButtonWithStyle("Yes","",{RightLabel="→→→"},true,function(j,k,l)
            if l then 
                if string.sub(d.model,1,4)=="item"then 
                    TriggerServerEvent("OASIS:buyWeapon",d.model,d.price,d.name,d.weaponShop,"armour")
                else 
                    if g=="ammo"then 
                        if HasPedGotWeapon(tOASIS.getPlayerPed(),GetHashKey(d.model),false)then 
                            TriggerServerEvent("OASIS:buyWeapon",d.model,d.price,d.name,d.weaponShop,"ammo")
                        else 
                            tOASIS.notify("~r~You do not have the body of this weapon to purchase ammo.")
                        end 
                    else 
                        TriggerServerEvent("OASIS:buyWeapon",d.model,d.price,d.name,d.weaponShop,"weapon",h)
                    end 
                end 
            end 
            end,RMenu:Get("OASISGunstore","confirm"))
            RageUI.ButtonWithStyle("No","",{RightLabel="→→→"},true,function(j,k,l)
            end,RMenu:Get("OASISGunstore","mainmenu"))
        end) 
    end
    if RageUI.Visible(RMenu:Get("OASISGunstore", "vip")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
...
RegisterNetEvent("OASIS:refreshGunStorePermissions",function()
    TriggerServerEvent("OASIS:requestNewGunshopData")
end)
local G = false
RegisterNetEvent("OASIS:recieveFilteredGunStoreData")
AddEventHandler("OASIS:recieveFilteredGunStoreData",function(F)
    i=F 
...
RegisterNetEvent("OASIS:recalculateLargeArms")
AddEventHandler("OASIS:recalculateLargeArms",function(G)
    for m,n in pairs(i)do 
...
local function J(m,K)
    ...
    c=K
    if m=="Rebel"then 
        RMenu:Get('OASISGunstore','mainmenu'):SetSpriteBanner("banners", "rebel")
    else 
        RMenu:Get('OASISGunstore','mainmenu'):SetSpriteBanner("banners", "gunstore")
    end
    RageUI.ActuallyCloseAll()
    RageUI.Visible(RMenu:Get('OASISGunstore','mainmenu'),true)
end
local function L(m)
    b=nil
    c=nil
    e=nil
    RageUI.ActuallyCloseAll()
    RageUI.Visible(RMenu:Get('OASISGunstore','mainmenu'),false)
end
Citizen.CreateThread(function()
    while true do 
        ...
        if not G and GetSelectedPedWeapon(R) == GetHashKey("WEAPON_MP5TAZER") then
            tOASIS.setWeapon(R, "WEAPON_UNARMED", true)
        end
        Wait(0)
    end 
end)
    ...
    if E then
        ...
        for m,n in pairs(i)do 
            ...
            for K,U in pairs(P)do 
                if T then 
                    tOASIS.addBlip(U.x,U.y,U.z,Q,R,S)
                end
                tOASIS.addMarker(U.x,U.y,U.z,1.0,1.0,1.0,255,0,0,170,50,27)
                local V=function()
                    if GetVehiclePedIsIn(tOASIS.getPlayerPed(),false)==0 then 
                        J(m,K)
                    else 
                        tOASIS.notify("~r~Exit your vehicle to access the gun store.")
                    end 
                end
                local W=function()
                    L(m)
                ...
                tOASIS.createArea("gunstore_"..m.."_"..K,U,1.5,6,V,W,X,{})
            end 
        end 
    end
end)
...
local Y={}
function tOASIS.createGunStore(Z,_,a0)
    local V=function()
        if GetVehiclePedIsIn(tOASIS.getPlayerPed(),false)==0 then 
            J(_)
        else 
    ...
    tOASIS.createArea(a1,a0,1.5,6,V,W,function()
    end)
    local a2=tOASIS.addMarker(a0.x,a0.y,a0.z,1.0,1.0,1.0,255,0,0,170,50,27)
    Y[Z]={area=a1,marker=a2}
end
function tOASIS.deleteGunStore(Z)
    local a3=Y[Z]
    if a3 then 
        tOASIS.removeMarker(a3.marker)
        tOASIS.removeArea(a3.area)
        Y[Z]=nil 
    end 
end