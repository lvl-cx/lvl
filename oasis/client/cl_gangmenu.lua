...
AddEventHandler("playerSpawned",function()
    local h = true
    if h then 
        TriggerServerEvent("OASIS:GetGangData")
    end 
end)
RegisterNetEvent("OASIS:GotGangData")
AddEventHandler("OASIS:GotGangData",function(i,j,k,PO)
    if i==nil then 
        PlayerIsInGang=false 
    else 
        PlayerIsInGang=true
...
RegisterNetEvent("OASIS:disbandedGang")
AddEventHandler("OASIS:disbandedGang",function()
    PlayerIsInGang=false
    showGangUI=false
    showNoGangUI=true
    ...
    TriggerEvent("OASIS:ForceRefreshData")
end)
RegisterNetEvent("OASIS:ForceRefreshData")
AddEventHandler("OASIS:ForceRefreshData",function()
    TriggerServerEvent("OASIS:GetGangData")
end)
RegisterNetEvent("OASIS:InviteRecieved")
AddEventHandler("OASIS:InviteRecieved",function(o,Q)
    OASISGangInvites[OASISGangInviteIndex]=Q
    OASISGangInviteIndex=OASISGangInviteIndex+1
    tOASIS.notify(o)
end)
RegisterNetEvent("OASIS:gangNameNotTaken")
AddEventHandler("OASIS:gangNameNotTaken",function()
    showNoGangUI=false
    showGangUI=true
    PlayerIsInGang=true 
end)
function func_drawGangUI()
    if showNoGangUI then 
        ...
        DrawAdvancedText(0.558,0.303,0.005,0.0028,0.539,"OASIS Gangs",255,255,255,255,7,0)
        ...
        for p,q in pairs(OASISGangInvites)do 
        ...
        if CursorInArea(0.35,0.415,0.415,0.46)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if createGangName~=nil and createGangName~="null"and createGangName~=""then 
                    TriggerServerEvent("OASIS:CreateGang",createGangName)
                else 
                    tOASIS.notify("~r~No gang name entered!")
                end 
            end 
        else 
            CreateGangSelectionRed=0
        ...
        if CursorInArea(0.435,0.51,0.415,0.46)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if selectedGangInvite~=nil then 
                    selectedGangInvite=OASISGangInvites[selectedGangInvite]
                    local perm1s = tOASIS.getUserId()
                    TriggerServerEvent("OASIS:addUserToGang",selectedGangInvite,perm1s)
                    OASISGangInvites={}
                    showNoGangUI=false
                    OASISGangInviteIndex=0
                    showGangUI=true
                    ...
                    tOASIS.notify("~r~No gang invite selected")
                end 
            end 
        else 
            JoinGangSelectionRed=0
    ...
    if showFundsGangUI then 
        ...
        DrawAdvancedText(0.591,0.378,0.005,0.0028,0.48,"OASIS Gang - Funds",255,255,255,255,7,0)
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount ~= nil then
                    if closeToBarclays() then
                        TriggerServerEvent("OASIS:depositGangBalance", amount)
                    end
                else
                    tOASIS.notify("~r~No amount entered!")
                end
            end
        else
        ...
        if CursorInArea(0.4083, 0.4718, 0.5490, 0.5999) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if closeToBarclays() then
                    TriggerServerEvent("OASIS:depositAllGangBalance")
                end
            end
        else
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount ~= nil then
                    if gangPermission >= 3 then
                        if closeToBarclays() then
                            TriggerServerEvent("OASIS:withdrawGangBalance", amount)
                        end
                    else
                        tOASIS.notify("~r~You don't have a high enough rank to withdraw")
                    end
                else
                    tOASIS.notify("~r~No amount entered!")
                end
        ...
        if CursorInArea(0.6088, 0.6739, 0.5481, 0.6018) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if gangPermission >= 3 then
                    if closeToBarclays() then
                        TriggerServerEvent("OASIS:withdrawAllGangBalance")
                    end
                else
                    tOASIS.notify("~r~You don't have a high enough rank to withdraw")
                end
    ...
    if showMembersGangUI then 
        ...
        DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"OASIS Gang - Members",255,255,255,255,7,0)
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if selectedMember~=nil and PlayerIsInGang and gangID~=nil then 
                    if gangPermission>=4 then 
                        TriggerServerEvent("OASIS:PromoteUser",gangID,tonumber(selectedMember))
                    else 
                        tOASIS.notify("~r~You don't have permission to promote!")
                    end 
                else 
                    tOASIS.notify("~r~No gang member selected")
                end 
            end 
        else 
        ...
        if CursorInArea(0.6182,0.6822,0.4138,0.4694)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if selectedMember~=nil and PlayerIsInGang and gangID~=nil then 
                    if gangPermission>=4 then 
                        TriggerServerEvent("OASIS:DemoteUser",gangID,selectedMember)
                    else 
                        tOASIS.notify("~r~You don't have permission to demote!")
                    end 
                else 
                    tOASIS.notify("~r~No gang member selected")
                end 
                ...
                if selectedMember~=nil then 
                    if gangPermission>=3 then 
                        if YesNoConfirm()then 
                            TriggerServerEvent("OASIS:kickMemberFromGang",gangID,selectedMember)
                        end 
                    else 
                        tOASIS.notify("~r~You don't have permission to kick!")
                    end 
                else 
                    tOASIS.notify("~r~No gang member selected")
                end 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if s~=nil then 
                    if gangPermission>=2 then 
                        TriggerServerEvent("OASIS:InviteUserToGang",gangID,s)
                    else 
                        tOASIS.notify("~r~You don't have permission to invite players")
                    end 
                else 
                    tOASIS.notify("No player name entered")
                end 
            end 
        else 
        ...
        if CursorInArea(0.3281,0.3723,0.6768,0.7074)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                PlaySound(-1,"SELECT","HUD_FRONTEND_DEFAULT_SOUNDSET",0,0,1)
                if e<=1 then 
                    tOASIS.notify("~r~Lowest page reached")
                else 
                    e=e-1 
                end 
            end 
        ...
        if CursorInArea(0.4130,0.4572,0.6712,0.7064)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                PlaySound(-1,"SELECT","HUD_FRONTEND_DEFAULT_SOUNDSET",0,0,1)
                if e>=f then 
                    tOASIS.notify("~r~Max page reached")
                else 
                    e=e+1 
                end 
            end 
    ...
    if showLogsGangUI then 
        DrawRect(0.501,0.525,0.421,0.387,0,0,0,150)
        DrawRect(0.501,0.308,0.421,0.047,18,82,228,248)
        DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"OASIS Gang - Logs",255,255,255,255,7,0)
    ...
    if showSettingsGangUI then 
        DrawRect(0.501,0.525,0.421,0.387,0,0,0,150)
        DrawRect(0.501,0.308,0.421,0.047,18,82,228,248)
        DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"OASIS Gang - Settings",255,255,255,255,7,0)
        ...
        if CursorInArea(0.3187,0.3937,0.4712,0.5462)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                PlaySound(-1,"SELECT","HUD_FRONTEND_DEFAULT_SOUNDSET",0,0,1)
                if YesNoConfirm()then 
                    TriggerServerEvent("OASIS:memberLeaveGang",gangID)
                    setCursor(0)
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if gangPermission>=4 then 
                    if YesNoConfirm()==true and gangID~=nil then 
                        TriggerServerEvent("OASIS:DeleteGang",gangID)
                    end 
                else 
                    tOASIS.notify("~r~You don't have permission to disband!")
                end 
            end 
        else 
    ...
    if showTurfsGangUI then 
        ...
        DrawRect(0.501,0.308,0.421,0.047,18,82,228,248)
        DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"OASIS Gang - Turfs",255,255,255,255,7,0)
    ...
    if showGangUI then 
        ...
        DrawAdvancedText(0.591,0.399,0.005,0.0028,0.51,"OASIS Gangs",255,255,255,255,7,0)
...
RegisterNetEvent("OASIS:Notify",function(u)
    tOASIS.notify(u)
end)
...
function closeToBarclays()
    ...
    if w<10.0 or x<10.0 or z<10.0 then 
        ...
        tOASIS.notify("~r~You must be near Barclays to access gang funds.")
        return false 
    end 
end
