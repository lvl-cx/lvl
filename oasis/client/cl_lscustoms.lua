...
local function DriveOutOfGarage(pos)
	Citizen.CreateThread(function()
		...
		local ok, veh_type, model = tOASIS.getNearestOwnedVehicle(1)
		myveh.model = model
...
AddEventHandler("OASIS:onClientSpawn",function(D, E)
    if E then
        ...
        for q,h in pairs(garages)do 
            tOASIS.addBlip(h.inside.position.x,h.inside.position.y,h.inside.position.z,72, nil, "LS Customs")
        end 
	end
end)
...
function tOASIS.isInsideLsCustoms()
    return inside
end
