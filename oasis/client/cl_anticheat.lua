...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(500)
		--if tOASIS.isPlayerInBankHeistSetup and not tOASIS.isPlayerInBankHeistSetup() then
			c(function(f)
				if DecorGetInt(f, "OASISACVeh") ~= 955 then
...
AddEventHandler("onClientResourceStart",function(q)
	...
	if r >= 15 and q ~= "screenshot-basic" then
		TriggerServerEvent("OASIS:acType18", q)
	end
	--TriggerServerEvent("OASIS:resourceStarted", q)
end)
	...
	while true do
		...
		for _,theWeapon in ipairs(WeaponBL) do
			...
			if HasPedGotWeapon(PlayerPedId(),GetHashKey(theWeapon),false) == 1 then
				...
				TriggerServerEvent("OASIS:acType2", theWeapon)
				return
			end
		end
		local _, z = StatGetInt(GetHashKey("mp0_shooting_ability"), true)
		if z > 100 then
			TriggerServerEvent("OASIS:acType19")  
			return
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	local _ = 0
	while true do
		if _ >= 100 and not tOASIS.isInComa() then
			TriggerServerEvent("OASIS:acType6")
			return
		end
		...
		for i, data in pairs(DetectableTextures) do
			if data.x and data.y then
				if GetTextureResolution(data.txd, data.txt).x == data.x and GetTextureResolution(data.txd, data.txt).y == data.y then
					TriggerServerEvent("OASIS:acType7", data.name..' Menu')
					return
				end
			else 
...
Citizen.CreateThread(function()
	while true do
		...
		if j ~= 0 then
			...
			if E > 1.0 then
				TriggerServerEvent("OASIS:acType8", "GetVehicleTopSpeedModifier "..E)
			end
		end
		local F = GetWeaponDamageModifier(GetCurrentPedWeapon(i))
		local G = GetPlayerMeleeWeaponDamageModifier()
		if y > 1.0 then
			TriggerServerEvent("OASIS:acType8", "PlayerWeaponDamageModifier "..y)
			return
		end
		if z > 1.0 then
			TriggerServerEvent("OASIS:acType8", "PlayerWeaponDefenseModifier "..z)
			return
		end
		if A > 1.0 then
			TriggerServerEvent("OASIS:acType8", "PlayerWeaponDefenseModifier_2 "..A)
			return
		end
		if B > 1.0 then
			TriggerServerEvent("OASIS:acType8", "PlayerVehicleDamageModifier "..B)
			return
		end
		if C > 1.0 then
			TriggerServerEvent("OASIS:acType8", "PlayerVehicleDefenseModifier "..C)
			return
		end
		if F > 1.0 then
			TriggerServerEvent("OASIS:acType8", "GetWeaponDamageModifier "..F)
			return
		end
		if G > 1.0 then
			TriggerServerEvent("OASIS:acType8", "GetPlayerMeleeWeaponDamageModifier "..G)
			return
		end
		...
		if IsPedShooting(h) then
			if not table.has(X, k) then
				...
				if Z == GetMaxAmmoInClip(h, k) then
					TriggerServerEvent("OASIS:acType8", "Infinite Ammo")
					Wait(60000)
				end
			end
		end
...
Citizen.CreateThread(function()
	while true do
		local f = tOASIS.getPlayerVehicle()
		...
		if GetVehicleHasParachute(f) or GetCanVehicleJump(f) or (GetHasRocketBoost(f) and be ~= GetHashKey("voltic2")) then
			if not table.has(b, be) then
				TriggerServerEvent("OASIS:acType12", globalVehicleModelHashMapping[be])
			end
		end
		Wait(1000)
	end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		local j = tOASIS.getPlayerId()
		local k = tOASIS.getPlayerVehicle()
		if k == 0 then
			...
			if not h and GetUsingseethrough() and not tOASIS.isPlayerInPoliceHeli() and not tOASIS.isPlayerInDrone() then
				TriggerServerEvent("OASIS:acType13")
				h = true
			end
		end
		SetPedInfiniteAmmoClip(i, false)
		for e, m in pairs(tOASIS.getWeapons()) do
...
Citizen.CreateThread(function()
	...
	if min.y < -0.29 or max.z > 0.98 then
		TriggerServerEvent("OASIS:acType14")
	end
end)
...
Citizen.CreateThread(function()
	Wait(10000)
	while true do
		if GetPlayerInvincible(PlayerId()) and not isInGreenzone and not tOASIS.isInsideLsCustoms() and not tOASIS.isStaffedOn() and not noclipActive and not tOASIS.isInComa() and not tOASIS.isInSpectate() and not tOASIS.getInRPZone() and not tOASIS.isInTutorial() then
			TriggerServerEvent("OASIS:acType15")
			Citizen.Wait(60000)
		end
		Citizen.Wait(1000)
	end
...
print('[OASIS] - Anti-Cheat initialised (Credits: c)')
