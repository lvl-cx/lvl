...
local e = false
local f = false
function tOASIS.isKnockedOut()
    return a
end
function tOASIS.putInNearestVehicleAsPassenger(j)
    local k = tOASIS.getClosestVehicle(j)
    if IsEntityAVehicle(k) then
        for l = 1, math.max(GetVehicleMaxNumberOfPassengers(k), 3) do
            if IsVehicleSeatFree(k, l) then
                SetPedIntoVehicle(tOASIS.getPlayerPed(), k, l)
                return true
            end
        end
    end
...
function tOASIS.putInNetVehicleAsPassenger(m)
    local k = tOASIS.getObjectId(m)
    if IsEntityAVehicle(k) then
...
function tOASIS.putInVehiclePositionAsPassenger(n, o, p)
    local k = tOASIS.getVehicleAtPosition(n, o, p)
    if IsEntityAVehicle(k) then
        for l = 1, GetVehicleMaxNumberOfPassengers(k) do
            if IsVehicleSeatFree(k, l) then
                SetPedIntoVehicle(tOASIS.getPlayerPed(), k, l)
...
RegisterNetEvent("OASIS:knockOut",function()
    if not a and not tOASIS.isPurge() then
        tOASIS.setCanAnim(false)
        a = true
        b = r()
    end
end)
RegisterNetEvent("OASIS:knockOutDisable",function()
    if a then
        local s = PlayerPedId()
...
RegisterNetEvent("OASIS:drag")
AddEventHandler("OASIS:drag",function(t)
    d = t
    e = not e
end)
RegisterNetEvent("OASIS:undrag")
AddEventHandler("OASIS:undrag",function(t)
    e = false
end)
RegisterNetEvent("OASIS:clearPoliceStuff",function()
...
local u = {}
RegisterNetEvent("OASIS:addS60",function(v, w, x)
...
RegisterNetEvent("OASIS:removeS60",function(x)
    if u[t] == nil then
        return
    else
        local D = u[x]
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(0, 323) or IsControlPressed(0, 27) and not IsUsingKeyboard(2) then
            if not globalSurrenderring and not tOASIS.isInComa() and not tOASIS.isHandcuffed() and (tOASIS.canAnim() or tOASIS.isTazedByRevive()) then
                DisablePlayerFiring(tOASIS.getPlayerPed(), true)
                DisableControlAction(0, 22, true)
                DisableControlAction(0, 25, true)
                DisableControlAction(0, 154, true)
                if not IsEntityDead(tOASIS.getPlayerPed()) then
                    if not c and not GetIsTaskActive(tOASIS.getPlayerPed(), 298) then
                        c = true
                        tOASIS.loadAnimDict("missminuteman_1ig_2")    
                        TaskPlayAnim(tOASIS.getPlayerPed(),"missminuteman_1ig_2","handsup_enter",7.0,1.0,-1,50,0,false,false,false)
                        RemoveAnimDict("missminuteman_1ig_2")
                    end
                end
            end
        end
        if (IsControlJustReleased(1, 323) or IsControlJustReleased(1, 27)) and not globalSurrenderring and c and not tOASIS.isInComa() and not tOASIS.isHandcuffed() and tOASIS.canAnim() then
            c = false
        ...
        if a then
            if tOASIS.isStaffedOn() then
                TriggerEvent("OASIS:knockOutDisable")
            elseif not tOASIS.isInComa() then
                local s = PlayerPedId()
                if not IsEntityPlayingAnim(s, b[1], b[2], 3) then
                    tOASIS.loadAnimDict(b[1])
                ...
                tOASIS.notify("~r~You have been knocked out!")
            end
        end
        Wait(0)
    end
...
RMenu.Add("policehandbook","main",RageUI.CreateMenu("Police Handbook", "~b~Officer Handbook", tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight()))
...
RegisterNetEvent("OASIS:breathTestResult",function(M,N)
    local O = N
    ...
    TaskPlayAnim(tOASIS.getPlayerPed(),"weapons@first_person@aim_rng@generic@projectile@shared@core","idlerng_med",1.0,-1,10000,50,0,false,false,false)
...
RegisterNetEvent("OASIS:beingBreathalysed",function()
    RageUI.Text({message = "~w~You are currently being ~b~breathalysed ~w~by a police officer."})
end)
RegisterNetEvent("OASIS:breathalyserCommand",function()
    local E = tOASIS.getPlayerPed()
        ...
        for Q, R in pairs(P) do
            if GetPlayerPed(R) ~= E then
                ...
                if T < 3.0 then
                    ...
                    TriggerServerEvent("OASIS:breathalyserRequest", U)
                    break
                end
            end
        end
...
RegisterNetEvent("OASIS:flashWarrantCard",function()
    local E = PlayerPedId()
    local V = tOASIS.loadModel("prop_fib_badge")
...
RegisterNetEvent("OASIS:startSearchingSuspect",function()
    tOASIS.setCanAnim(false)
    tOASIS.loadAnimDict("custom@police")
    ...
    while GetGameTimer() - a3 < 10000 do
        if IsDisabledControlJustPressed(0, 73) then
            TriggerServerEvent("OASIS:cancelPlayerSearch")
            return
        end
        Citizen.Wait(0)
    end
...
local a4 = false
RegisterNetEvent("OASIS:startBeingSearching",function(a5)
    ...
    a4 = true
    tOASIS.setCanAnim(false)
    tOASIS.loadAnimDict("custom@suspect")
    local s = tOASIS.getPlayerPed()
...
RegisterNetEvent("OASIS:cancelPlayerSearch",function()
    a4 = false
    tOASIS.setCanAnim(true)
    local s = tOASIS.getPlayerPed()
    DetachEntity(s)
...
local ae = ""
local af = false
RegisterNetEvent("OASIS:receivePoliceCallsign",function(ag, ah, ai)
    a8 = ah
    a9 = ag
    aa = ai
    ab = true
...
RegisterNetEvent("OASIS:receiveHmpCallsign",function(ag, ah, ai)
    ac = ah
    ad = ag
    ae = ai
    af = true
...
function tOASIS.getPoliceCallsign()
    return a9
end
function tOASIS.getPoliceRank()
    return a8
end
function tOASIS.getPoliceName()
    return aa
end
function tOASIS.hasPoliceCallsign()
    return ab
end
function tOASIS.getHmpCallsign()
    return ad
end
function tOASIS.getHmpRank()
    return ac
end
function tOASIS.getHmpName()
    return ae
end
function tOASIS.hasHmpCallsign()
    return af
end
...
tOASIS.createThreadOnTick(func_drawCallsign)
local aj = 0
local function ak()
    local s = tOASIS.getPlayerPed()
...
tOASIS.createThreadOnTick(ak)
function tOASIS.hasRecentlyShotGun()
    return aj ~= 0 and GetGameTimer() - aj < 600000
end

AddEventHandler("OASIS:onClientSpawn",function()
    aj = 0
end)

RMenu.Add("trainingWorlds","mainmenu",RageUI.CreateMenu("Training Worlds", "Main Menu", tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight()))
local ap = {}
local aq = false
RegisterNetEvent("OASIS:trainingWorldOpen",function(ar)
    aq = ar
    ...
    if RageUI.Visible(RMenu:Get('trainingWorlds', 'mainmenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            for x, at in pairs(ap) do
                ...
                local av = at.bucket == tOASIS.getPlayerBucket()
                ...
                RageUI.ButtonWithStyle(at.name,au,aw,true,function(J, K, L)
                    if K and aq then
                        ...
                        if IsControlJustPressed(0, 214) then
                            TriggerServerEvent("OASIS:trainingWorldRemove", x)
                        end
                    end
                    if L then
                        TriggerServerEvent("OASIS:trainingWorldJoin", x)
                    end
                end)
                if av then
                    as = av
            ...
            if as then
                RageUI.ButtonWithStyle("~r~Leave Training World",nil,{},true,function(J, K, L)
                    if L then
                        TriggerServerEvent("OASIS:trainingWorldLeave")
                    end
                end)
            end
            if aq then
                RageUI.ButtonWithStyle("~b~Create Training World",nil,{},true,function(J, K, L)
                    if L then
                        TriggerServerEvent("OASIS:trainingWorldCreate")
                    end
                end)
            end
        end)
...
RegisterNetEvent("OASIS:trainingWorldSend",function(x, ax)
    ap[x] = ax
end)
RegisterNetEvent("OASIS:trainingWorldSendAll",function(ax)
    ap = ax
end)
RegisterNetEvent("OASIS:trainingWorldRemove",function(x)
    ap[x] = nil
end)
