...
AddEventHandler("OASIS:onClientSpawn",function(p, q)
    if q then
        tOASIS.addMarker(b.x, b.y, b.z, 0.7, 0.7, 0.5, 0, 125, 255, 125, 150, 20, false, false, true)
        tOASIS.addBlip(b.x, b.y, b, 378, 3, "Organ Heist Police")
        tOASIS.addMarker(c.x, c.y, c.z, 0.7, 0.7, 0.5, 255, 0, 0, 125, 150, 20, false, false, true)
        tOASIS.addBlip(c.x, c.y, c.z, 378, 1, "Organ Heist Criminals")
    end
end)
...
Citizen.CreateThread(function()
    while true do
        if f then
            ...
            if IsControlJustPressed(0, 38) then
                if not globalOnPoliceDuty then
                    if not g then
                        TriggerServerEvent("OASIS:joinOrganHeist")
                        Wait(100)
                    else
                        tOASIS.notify("~r~Already joined the Organ Heist!")
                    end
                else
                    tOASIS.notify("~r~You are police, please use the other entrance!")
                end
            end
        elseif e then
            ...
            if IsControlJustPressed(0, 38) then
                if globalOnPoliceDuty then
                    ...
                    tOASIS.notify("~r~You are a civillian, please use the other entrance!")
                end
            end
        end
        ...
        if inGamePhase or inWaitingStage then
            ...
            RemoveWeaponFromPed(tOASIS.getPlayerPed(), "WEAPON_MOLOTOV")
            RemoveWeaponFromPed(tOASIS.getPlayerPed(), "WEAPON_FLASHBANG")
            RemoveWeaponFromPed(tOASIS.getPlayerPed(), "WEAPON_SMOKEGRENADE")
            RemoveWeaponFromPed(tOASIS.getPlayerPed(), "WEAPON_BZGAS")
            RemoveWeaponFromPed(tOASIS.getPlayerPed(), "WEAPON_STUNGUN")
            local r = PlayerPedId()
...
RegisterNetEvent("OASIS:teleportToOrganHeist",function(v, w, x, y)
    local r = PlayerPedId()
    d = w
    canPlayAnim = false
    h = x
    ...
    if x == "civ" then
        ...
        tOASIS.createAtm("Organ Heist", k.atmLocation)
    elseif x == "police" then
        SetPedRelationshipGroupHash(PlayerPedId(), m)
    end
    for l, z in pairs(k.gunStores[h]) do
        tOASIS.createGunStore(z[1], z[2], z[3])
    end
    tOASIS.setFriendlyFire(false)
    DoScreenFadeOut(1000)
...
RegisterNetEvent("OASIS:addOrganHeistPlayer",function(A, x)
    if x == "civ" then
        j[A] = true
    elseif x == "police" then
        i[A] = true
...
RegisterNetEvent("OASIS:organHeistKillConfirmed",function(B)
    PlaySoundFrontend(-1, "Weapon_Upgrade", "DLC_GR_Weapon_Upgrade_Soundset", true)
    tOASIS.playScreenEffect("MP_Celeb_Win", 0.25)
    tOASIS.notify("~g~Killed " .. B .. " received £25,000")
end)
    ...
    while true do
        if inWaitingStage then
            ...
            if #(s - k.pastGate) < 3.0 then
                ...
                NetworkFadeInEntity(PlayerPedId(), 0)
                DoScreenFadeIn(1000)
                tOASIS.notify("~r~You got too far from the organ heist and have been teleported back.")
            end
        end
        if inGamePhase then
            local s = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent("OASIS:startOrganHeist",function()
...
RegisterNetEvent("OASIS:removeFromOrganHeist",function(F)
    if i[F] then
        i[F] = nil
    end
    if j[F] then
...
RegisterNetEvent("OASIS:endOrganHeist",function()
    ...
    inGamePhase = false
    d = 600
    tOASIS.deleteAtm("Organ Heist")
    for l, z in pairs(k.gunStores[h]) do
        tOASIS.deleteGunStore(z[1])
    end
    h = ""
    k = nil
...
RegisterNetEvent("OASIS:endOrganHeistWinner",function(x)
    Wait(10000)
    winOrganHeist(x)
    local r = PlayerPedId()
    tOASIS.setFriendlyFire(true)
...
function tOASIS.setDeathInOrganHeist()
    inOrganHeist = false
    inWaitingStage = false
    inGamePhase = false
end
