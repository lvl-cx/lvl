...
local d
RegisterNetEvent("OASIS:crateDrop",function(e, f, g)
    -- e = vector3
    -- f = crate id
    -- g (if it's rig)
    ...
    DecorSetInt(a[f].crate, "OASISACVeh", 955)
    ...
    DecorSetInt(a[f].parachute, "OASISACVeh", 955)
    ...
    --AddOwnedExplosion(tOASIS.getPlayerPed(), y.x, y.y, y.z, 1, 0.0, true, false, 3.0)
    FreezeEntityPosition(a[f].crate, true)
    for h, i in pairs(c) do
...
RegisterNetEvent("OASIS:removeLootcrate",function(f)
    if a[f] then
...
RegisterNetEvent("OASIS:addCrateDropRedzone",function(f, e)
    tOASIS.loadModel("xs_prop_arena_crate_01a")
    a[f] = {}
...
RegisterNetEvent("OASIS:removeCrateRedzone",function()
    SetTimeout(300000,function()
        RemoveBlip(radiusBlip)
    end)
end)
