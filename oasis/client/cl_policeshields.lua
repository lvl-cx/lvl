    ...
    while true do
        if a.active and a.id ~= 0 then
            ...
            if h ~= GetHashKey("WEAPON_UNARMED") and (not b[a.id].allowPistol or not table.has(c, h)) then
                tOASIS.setWeapon(g, "WEAPON_UNARMED", true)
            end
...
RMenu.Add("policeshields","main",RageUI.CreateMenu("Police Shields", "~b~Shield Management", tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight()))
...
RegisterNetEvent("OASIS:toggleShieldMenu",function(l)
...
function removeShield(m)
    if DoesEntityExist(a.prop) then
        ...
        if m then
            tOASIS.notify("~g~Success~w~: Shield removed")
        end
    end
end
function createShield(n)
    ...
    local o = false
    local g = tOASIS.getPlayerPed()
    local p = tOASIS.getPlayerCoords()
    if a.active then
        removeShield(false)
    end
    a.id = n
    local q = tOASIS.loadModel(b[n].model)
    ...
    tOASIS.loadAnimDict(b[n].animDict)
