...
RegisterCommand("togglemelee",function()
    if tOASIS.getUserId()==1 or tOASIS.getUserId()==2 then 
        c = not c
        tOASIS.notify('Melee Combat: '..(c and "~g~Enabled" or "~r~Disabled"))
    end 
end)
function tOASIS.enablePunching(d)
    c = d
end
...
AddEventHandler("OASIS:onClientSpawn",function(h, i)
    if i then
        ...
        for n, o in pairs(a) do
            tOASIS.createArea("boxing_" .. n, o.pos, o.radius, 6, j, l, m, {})
        end
    end
end)
...
AddEventHandler("OASIS:onClientSpawn",function(h, i)
    if i then
        ...
        for n, u in pairs(t) do
            tOASIS.addMarker(u.x, u.y, u.z, 1.0, 1.0, 1.0, 0, 0, 255, 100, 50, 27, false, false, true)
            tOASIS.createArea(n .. "_boxingGloves", u, 1.5, 6, j, l, m, {})
        end
    end
end)
local v = {}
function putGloves()
    local w = tOASIS.getPlayerPed()
    local x = tOASIS.loadModel("prop_boxing_glove_01")
