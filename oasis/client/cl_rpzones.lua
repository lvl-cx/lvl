...
local function f()
    ...
    g.position=tOASIS.getPlayerCoords()
    g.radius=g.radius+0.0
    g.vehicles.speed=g.vehicles.speed+0.0
    TriggerServerEvent("OASIS:createRPZone",g)
end
...
local function h()
    ...
    if i then 
        TriggerServerEvent("OASIS:removeRPZone",i)
    end 
end
local inRPZone = false
RMenu.Add("rpzones","mainmenu",RageUI.CreateMenu("","~b~Main Menu",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight(),"banners","admin"))
RMenu.Add("rpzones","weapons",RageUI.CreateSubMenu(RMenu:Get("rpzones","mainmenu"),"","~b~Weapons",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight(),"banners","admin"))
RMenu.Add("rpzones","vehicles",RageUI.CreateSubMenu(RMenu:Get("rpzones","mainmenu"),"","~b~Vehicles",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight(),"banners","admin"))
...
RegisterNetEvent("OASIS:createRPZone",function(g)
...
RegisterNetEvent("OASIS:removeRPZone",function(r)
...
local function G(g)
    local l,u=GetCurrentPedWeapon(tOASIS.getPlayerPed())
    if u~=0 then 
        t(g,u)
    end
    local y,H=tOASIS.getPlayerVehicle()
    if y~=0 and H then 
        A(g,y)
    end
...
tOASIS.createThreadOnTick(I)

function tOASIS.GetRPZoneInfo()
    local K=tOASIS.getPlayerCoords()
    for l,g in ipairs(d)do 
        if#(K-g.position)<g.radius then
            return g
