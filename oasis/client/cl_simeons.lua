...
RegisterNetEvent("OASIS:gotCarDealerInstances",function(m)
    a = m
end)
RegisterNetEvent("OASIS:gotCarDealerCategories",function(n)
    b = n
end)
RegisterNetEvent("OASIS:carDealerPurchased",function()
    DeleteVehicle(f)
    f = 0
    g = 0
    h = false
    local o = a[d].posVector
    SetEntityCoords(tOASIS.getPlayerPed(), o.x, o.y, o.z)
end)
local function p(q)
    if q ~= nil then
        d = q
...
local function s(t, u, v, w)
    ...
    if HasModelLoaded(x) then
        local z = CreateVehicle(x, u, v, w, GetEntityHeading(tOASIS.getPlayerPed()), false, false)
        DecorSetInt(z, "OASISACVeh", 955)
        ...
        TaskWarpPedIntoVehicle(tOASIS.getPlayerPed(), z, -1)
        ...
        tOASIS.notify("~r~Could not load vehicle")
        return -1
    end
end
CreateThread(function()
    for A, B in pairs(l.simeonsInstances) do
        ...
        tOASIS.addBlip(C.x, C.y, C.z, D, E)
        tOASIS.addMarker(C.x, C.y, C.z, 0.7, 0.7, 0.5, G[1], G[2], G[3], 125, 50, F, true, true)
    end
end)
...
function isNearPreviewVector(J)
    if #(GetEntityCoords(tOASIS.getPlayerPed()) - J) < 5.0 then
        return true
    end
    return false
end
...
Citizen.CreateThread(function()
    while true do
        if a ~= nil then
            for A, B in pairs(a) do
                local C, D, E, F, G, H, I =B.posVector,B.blipId,B.blipColour,B.markerId,B.markerColourTable,B.permissionsTable,B.categorys
                c[A] = #(GetEntityCoords(tOASIS.getPlayerPed()) - C)
            end
        end
        Wait(250)
    end
end)
RMenu.Add("cardealer","mainmenu",RageUI.CreateMenu("", "", tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight(), "banners",  "dealership"))
RMenu:Get("cardealer", "mainmenu"):SetSubtitle("~b~Categories")
RMenu.Add("cardealer","categories",RageUI.CreateSubMenu(RMenu:Get("cardealer", "mainmenu"),"","~b~Vehicles",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight()))
RMenu.Add("cardealer","vehicle",RageUI.CreateSubMenu(RMenu:Get("cardealer", "categories"),"","~b~Options",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight()))
RMenu.Add("cardealer","confirm",RageUI.CreateSubMenu(RMenu:Get("cardealer", "vehicle"),"","~b~Are you sure?",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight()))
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if d ~= nil then
                ...
                RageUI.ButtonWithStyle("Test Drive " .. i[1],"",{RightLabel = "→→→"},true,function(N, O, P)
                    if P then
                        DeleteVehicle(GetVehiclePedIsIn(tOASIS.getPlayerPed()))
                        if not k then
                            RageUI.ActuallyCloseAll()
                            hash = GetHashKey(j)
                            ...
                            if HasModelLoaded(hash) then
                                ...
                                testDriveSeconds = 60
                                local S = GetEntityCoords(tOASIS.getPlayerPed())
                                DecorSetInt(testDriveCar, "OASISACVeh", 955)
                                SetModelAsNoLongerNeeded(hash)
                                TaskWarpPedIntoVehicle(tOASIS.getPlayerPed(), testDriveCar, -1)
                                setVehicleFuel(testDriveCar, 100)
                                tOASIS.notify("~g~You have 1 minute to test drive this vehicle!")
                                for T = 0, 24 do
                                ...
                                SetTimeout(60000,function()
                                    if k then
                                        ...
                                        SetEntityCoords(tOASIS.getPlayerPed(), S.x, S.y, S.z)
                                        tOASIS.notify("~b~Test drive over!")
                                        p()
                                    end
                                end)
                                Citizen.CreateThread(function()
                                ...
                                Citizen.CreateThread(function()
                                    while k do
                                        local U = GetVehiclePedIsIn(tOASIS.getPlayerPed(), false)
                                        if U ~= nil then
    ...
    if RageUI.Visible(RMenu:Get('cardealer', 'confirm')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Yes","",{RightLabel = "→→→"},true,function(N, O, P)
                if P then
                    TriggerServerEvent("OASIS:purchaseCarDealerVehicle", e, j)
                    TriggerServerEvent('OASIS:FetchCars', 'Standard Garage')
                end
            end,RMenu:Get("cardealer", "mainmenu"))
            RageUI.ButtonWithStyle("No","",{RightLabel = "→→→"},true,function(N, O, P)
                if P then
                    tOASIS.notify("~y~Cancelled!")
                end
            end,
...
tOASIS.createThreadOnTick(func_previewSimeonsVehicle)
