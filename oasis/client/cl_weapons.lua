...
local aZ = false
function tOASIS.giveWeapons(d,e)
    local f=PlayerPedId()
    if e then 
...
function tOASIS.hasWeapon(hash)
...
function tOASIS.getWeapons()
    ...
    for b,c in pairs(a.weapons)do 
        if HasPedGotWeapon(f,c.hash)then
            ...
            --h.attachments=tOASIS.getAllWeaponAttachments(b)
            d[b]=h 
        end 
    end
    return d 
end
function tOASIS.removeAllWeapons()
    RemoveAllPedWeapons(tOASIS.getPlayerPed())
end
...
RegisterCommand("storecurrentweapon",function()
    if s+3000<GetGameTimer()then 
        ...
        if HasPedGotWeapon(PlayerPedId(),'WEAPON_PISTOL50')or HasPedGotWeapon(PlayerPedId(),'WEAPON_MACHINEPISTOL')then 
            ...
            TriggerServerEvent("OASIS:forceStoreSingleWeapon",g)
        end 
    else 
        tOASIS.notify("~r~Store weapons cooldown, please wait.")
    end 
end)
            ...
            if not table.has(X, k) then
                for k,v in pairs(a.weapons) do
                    if i == GetHashKey(k) then
                        TriggerServerEvent('OASIS:acType17', v.name)
                        Wait(60000)
                    end
                end
            end
