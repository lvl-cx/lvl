    ...
    if E then
        ...
        local f = function()
            if IsControlJustPressed(1, 51) then
                TriggerEvent("OASIS:openBarbershop")
            end
        end
        for g, h in pairs(a) do
            tOASIS.createArea("barbershop_" .. g, h, 1.5, 6, d, e, f)
            tOASIS.addMarker(h.x, h.y, h.z - 0.2, 0.5, 0.5, 0.5, 0, 50, 255, 170, 50, 20, false, false, true)
            tOASIS.addBlip(h.x, h.y, h.z, 71, 13, "Barber Shop")
        end
    end
end)