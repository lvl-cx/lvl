RMenu.Add("cardev","mainmenu",RageUI.CreateMenu("", "", tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight(), "banners", "cardev"))
RMenu:Get("cardev", "mainmenu"):SetSubtitle("~b~Car Dev Menu")
RMenu.Add("cardev","vehiclemods",RageUI.CreateSubMenu(RMenu:Get("cardev", "mainmenu"),"","~b~Vehicle Mods",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight()))
RMenu.Add("cardev","vehiclemodindexes",RageUI.CreateSubMenu(RMenu:Get("cardev", "vehiclemods"),"","~b~Vehicle Mod Indexes",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight()))
RMenu.Add("cardev","extras",RageUI.CreateSubMenu(RMenu:Get("cardev", "mainmenu"),"","~b~Extras",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight()))
RMenu.Add("cardev","colours",RageUI.CreateSubMenu(RMenu:Get("cardev", "mainmenu"),"","~b~Colours",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight()))
local a = false
local b = false
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('cardev', 'mainmenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if b and tOASIS.getPlayerBucket() == 333 then
                RageUI.ButtonWithStyle("Spawn Vehicle (No Mods)","",{RightLabel = "→→→"},true,function(h, i, j)
                    if j then
                        tOASIS.clientPrompt("Spawncode:","",function(k)
                            if k ~= "" and tOASIS.getPlayerBucket() == 333 then
                                local l = tOASIS.loadModel(k)
                                TriggerServerEvent("OASIS:logVehicleSpawn", k, "/cardev")
                                local m = tOASIS.getPlayerCoords()
                                local n = tOASIS.spawnVehicle(l,m.x,m.y,m.z,GetEntityHeading(tOASIS.getPlayerPed()),true,true,true)
                                DecorSetInt(n, "OASISACVeh", 955)
                                SetVehicleOnGroundProperly(n)
                                SetEntityInvincible(n, false)
                                SetPedIntoVehicle(tOASIS.getPlayerPed(), n, -1)
                ...
                RageUI.ButtonWithStyle("Delete Vehicle","",{RightLabel = "→→→"},true,function(h, i, j)
                    if j then
                        local o = GetVehiclePedIsIn(tOASIS.getPlayerPed(), false)
                        if NetworkGetEntityIsNetworked(o) then
                            local p = NetworkGetNetworkIdFromEntity(o)
                            if p ~= 0 then
                                DeleteEntity(aT)
                                --TriggerServerEvent("OASIS:cardevDeleteVehicle", p)
                            end
                        else
                            DeleteEntity(o)
                        end
                ...
                RageUI.Checkbox("Return to normal Universe","",b,{},function()
                    ...
                    b = true
                    TriggerServerEvent("OASIS:setCarDevMode", b)
                end,function()
                    b = false
                    TriggerServerEvent("OASIS:setCarDevMode", b)
                end)
    ...
    if RageUI.Visible(RMenu:Get('cardev', 'vehiclemods')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            for w, x in pairs(f) do
                if GetNumVehicleMods(tOASIS.getPlayerVehicle(), w) > 0 then
    ...
    if RageUI.Visible(RMenu:Get('cardev', 'vehiclemodindexes')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if GetNumVehicleMods(tOASIS.getPlayerVehicle(), selectedModType) == 0 then
                RageUI.Text({message = "~r~No available mod indexes for this mod type for this vehicle."})
            else
                for w = 0, GetNumVehicleMods(tOASIS.getPlayerVehicle(), selectedModType) do
                    RageUI.ButtonWithStyle("Mod " .. w,"",{RightLabel = "→→→"},true,function(h, i, j)
                        if j then
                            SetVehicleModKit(tOASIS.getPlayerVehicle(), 0)
                            SetVehicleMod(tOASIS.getPlayerVehicle(), selectedModType, w)
                        end
                    end,RMenu:Get("cardev", "vehiclemodindexes"))
                end
            end
    ...
    if RageUI.Visible(RMenu:Get('cardev', 'extras')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            local r = tOASIS.getPlayerVehicle()
            local y = false
            if r ~= 0 then
                for w = 1, 12 do
                        ...
                        if IsVehicleExtraTurnedOn(r, w) then
                            ...
                            RageUI.Button("Enable Extra " .. w,nil,{RightLabel = tostring(w)},true,function(h, i, j)
                                if j then
                                    ...
                                    TriggerEvent("OASIS:staffFixVehicle")
                                end
                            end)
                        end
                    end
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            for z, A in pairs(g) do
                RageUI.Button(A.name,nil,{},true,function(h, i, j)
                    if j then
                        SetVehicleColours(tOASIS.getPlayerVehicle(), A.colorindex, A.colorindex)
                    end
                end)
            end
        end)
...
RegisterCommand("cardev",function(C)
    if a and not tOASIS.isPurge() then
        B()
    end
end)
RegisterNetEvent("OASIS:setCarDev",function()
    a = true
end)
RegisterNetEvent("OASIS:setBucket",function(D)
    if b and D ~= 333 then
...
local function ae()
    if not b or tOASIS.getPlayerBucket() ~= 333 then
        if J then
            ac(false)
        end
        return
    ...
    L()
    local af = tOASIS.getFontId("Akrobat-ExtraBold")
    ...
    local t = tOASIS.getPlayerVehicle()
