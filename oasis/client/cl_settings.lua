RMenu.Add('SettingsMenu', 'MainMenu', RageUI.CreateMenu("", "~b~Settings Menu", tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight(), "banners","settings")) 
RMenu.Add("SettingsMenu", "graphicpresets", RageUI.CreateSubMenu(RMenu:Get("SettingsMenu", "MainMenu"), "", '~b~Graphics Presets',tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight(),"banners","settings"))
RMenu.Add("SettingsMenu", "changediscord", RageUI.CreateSubMenu(RMenu:Get("SettingsMenu", "MainMenu"), "", '~b~Link Discord',tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight(),"banners","settings"))
RMenu.Add("SettingsMenu", "killeffects", RageUI.CreateSubMenu(RMenu:Get("SettingsMenu", "MainMenu"), "", '~b~Kill Effects',tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight(),"banners","settings"))
RMenu.Add("SettingsMenu", "bloodeffects", RageUI.CreateSubMenu(RMenu:Get("SettingsMenu", "MainMenu"), "", '~b~Blood Effects',tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight(),"banners","settings"))
RMenu.Add("SettingsMenu", "weaponswhitelist", RageUI.CreateSubMenu(RMenu:Get("SettingsMenu", "MainMenu"), "", '~b~Custom Weapons Owned',tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight(),"banners","settings"))
RMenu.Add("SettingsMenu", "generateaccesscode", RageUI.CreateSubMenu(RMenu:Get("SettingsMenu", "weaponswhitelist"), "", '~b~Generate Access Code',tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight(),"banners","settings"))
RMenu.Add("SettingsMenu", "viewwhitelisted", RageUI.CreateSubMenu(RMenu:Get("SettingsMenu", "generateaccesscode"), "", '~b~View Whilelisted Users',tOASIS.getRageUIMenuWidth(), tOASIS.getRageUIMenuHeight(),"banners","settings"))
...
Citizen.CreateThread(function()
    local f = GetResourceKvpString("oasis_diagonalweapons") or "false"
    if f == "false" then
        b = false
        TriggerEvent("OASIS:setVerticalWeapons")
    else
        b = true
        TriggerEvent("OASIS:setDiagonalWeapons")
    end
    local g = GetResourceKvpString("oasis_frontars") or "false"
    if g == "false" then
        c = false
        TriggerEvent("OASIS:setBackAR")
    else
        c = true
        TriggerEvent("OASIS:setFrontAR")
    end
    local h = GetResourceKvpString("oasis_hitmarkersounds") or "false"
    if h == "false" then
        d = false
        TriggerEvent("OASIS:hsSoundsOff")
    else
        d = true
        TriggerEvent("OASIS:hsSoundsOn")
    end
    local m = GetResourceKvpString("oasis_hideeventannouncement") or "false"
    if m == "false" then
        g = false
    else
        g = true
    end
    local n = GetResourceKvpString("oasis_healthpercentage") or "false"
    if n == "false" then
        h = false
    else
        h = true
...
function tOASIS.setDiagonalWeaponSetting(i)
    SetResourceKvp("oasis_diagonalweapons", tostring(i))
end
function tOASIS.setFrontARSetting(i)
    SetResourceKvp("oasis_frontars", tostring(i))
end
function tOASIS.setHitMarkerSetting(i)
    SetResourceKvp("oasis_hitmarkersounds", tostring(i))
end
function tOASIS.setCODHitMarkerSetting(i)
    SetResourceKvp("oasis_codhitmarkersounds", tostring(i))
end
function tOASIS.setHideEventAnnouncementFlag(p)
    SetResourceKvp("oasis_hideeventannouncement", tostring(p))
end
function tOASIS.getHideEventAnnouncementFlag()
    return g
end
function tOASIS.setShowHealthPercentageFlag(p)
    SetResourceKvp("oasis_healthpercentage", tostring(p))
end
function tOASIS.getShowHealthPercentageFlag()
    return h
end
local function j(k)
    RageUI.CloseAll()
...
local r
local s
RegisterNetEvent("OASIS:gotCustomWeaponsOwned",function(t)
    print("gotCustomWeaponsOwned", dump(t))
    o = t
end)
RegisterNetEvent("OASIS:generatedAccessCode",function(u)
    print("got accessCode", u)
    r = u
end)
RegisterNetEvent("OASIS:getWhitelistedUsers",function(v)
    s = v
end)
local w = {}
...
local function G(y, z, H)
    ...
    SetResourceKvp("oasis_graphic_presets", I)
end
local J = {
    "0%",
    "5%",
...
local function U()
    ...
    SetResourceKvp("oasis_kill_effects", V)
end
local W = {head = 1, body = 1, arms = 1, legs = 1}
local function X()
    local Y = json.encode(W)
    SetResourceKvp("oasis_blood_effects", Y)
end
Citizen.CreateThread(function()
    Citizen.Wait(1000)
    local I = GetResourceKvpString("oasis_graphic_presets")
    if I and I ~= "" then
        w = json.decode(I) or {}
    end
    ...
    local V = GetResourceKvpString("oasis_kill_effects")
    if V and V ~= "" then
    ...
    local Y = GetResourceKvpString("oasis_blood_effects")
    if Y and Y ~= "" then
    ...
    if RageUI.Visible(RMenu:Get('SettingsMenu', 'MainMenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            local function a4()
                TriggerEvent("OASIS:setDiagonalWeapons")
                b = true
                tOASIS.setDiagonalWeaponSetting(b)
            end
            local function a5()
                TriggerEvent("OASIS:setVerticalWeapons")
                b = false
            ...
            local function a4()
                TriggerEvent("OASIS:hsSoundsOn")
                d = true
                tOASIS.setHitMarkerSetting(d)
                tOASIS.notify("~y~Experimental Headshot sounds now set to " .. tostring(d))
            end
            local function a5()
                TriggerEvent("OASIS:hsSoundsOff")
                d = false
            ...
            RageUI.ButtonWithStyle("Weapon Whitelists","Sell your custom weapon whitelists here.",{RightLabel = "→→→"},true,function(a0, a1, a2)
                if a2 then
                    ...
                    q = nil
                    s = nil
                    TriggerServerEvent("OASIS:getCustomWeaponsOwned")
                end
            ...
            RageUI.Checkbox("Streetnames","",tOASIS.isStreetnamesEnabled(),{RightBadge = RageUI.CheckboxStyle.Car},function(a0, a2, a1, a6)
            end,
            function()
                tOASIS.setStreetnamesEnabled(true)
            end,
            function()
                tOASIS.setStreetnamesEnabled(false)
            end)
            RageUI.Checkbox("Compass","",tOASIS.isCompassEnabled(),{RightBadge = RageUI.CheckboxStyle.Car},function(a0, a2, a1, a6)
            end,
            function()
                tOASIS.setCompassEnabled(true)
            end,
            function()
                tOASIS.setCompassEnabled(false)
            end)
            local function a4()
                tOASIS.hideUI()
                hideUI = true
            ...
            RageUI.Checkbox("Show Health Percentage","Displays the health and armour percentage on the bars.",h,{},function()
                ...
                h = true
                tOASIS.setShowHealthPercentageFlag(true)
            end,function()
                h = false
                tOASIS.setShowHealthPercentageFlag(false)
            end)
            RageUI.Checkbox("Hide Event Announcements","Hides the big scaleform from displaying across your screen, will still announce in chat.",g,{},function()
                ...
                g = true
                tOASIS.setHideEventAnnouncementFlag(true)
            end,function()
                g = false
                tOASIS.setHideEventAnnouncementFlag(false)
            end)
            RageUI.ButtonWithStyle("Change Linked Discord","Begins the process of changing your linked Discord. Your linked discord is used to sync roles with the server.",{RightLabel = "→→→"},true,function(a0, a1, a2)
                if a2 then
                    TriggerServerEvent('OASIS:changeLinkedDiscord')
                end
            end)
    ...
    if RageUI.Visible(RMenu:Get('SettingsMenu', 'changediscord')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("Enter Code","",{RightLabel = "→→→"},true,function(a0, a1, a2)
                if a2 then
                    TriggerServerEvent('OASIS:enterDiscordCode')
                end
            end)
        end)
    end
    ...
    if RageUI.Visible(RMenu:Get('SettingsMenu', 'generateaccesscode')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                if a2 then
                    ...
                    if tonumber(a9) then
                        ...
                        if a9 > 0 then
                            ...
                            TriggerServerEvent("OASIS:generateWeaponAccessCode", q, a9)
                        end
                    end
                end
            end)
            RageUI.ButtonWithStyle("View whitelisted users","",{RightLabel = "→→→"},true,function(a0, a1, a2)
                if a2 then
                    TriggerServerEvent("OASIS:requestWhitelistedUsers", q)
                end
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.List("~y~Particles~w~",P,S.particle,"",{},true,function(a0, a1, a2, a3)
                if a1 and S.particle ~= a3 then
                    if not tOASIS.isPlusClub() and not tOASIS.isPlatClub() then
                        notify("~y~You need to be a subscriber of OASIS Plus or OASIS Platinum to use this feature.")
                        notify("~y~Available @ store.oasisv.co.uk")
                    end
                    S.particle = a3
                    U()
                end
            ...
            if GetGameTimer() - T > ad + 1000 then
                tOASIS.addKillEffect(PlayerPedId(), true)
                T = GetGameTimer()
            end
    ...
    if RageUI.Visible(RMenu:Get('SettingsMenu', 'bloodeffects')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.List("~y~Head",P,W.head,"Effect that displays when you hit the head.",{},true,function(a0, a1, a2, a3)
                ...
                if a2 then
                    tOASIS.addBloodEffect("head", 0x796E, PlayerPedId())
                end
            end)
            RageUI.List("~y~Body",P,W.body,"Effect that displays when you hit the body.",{},true,function(a0, a1, a2, a3)
                ...
                if a2 then
                    tOASIS.addBloodEffect("body", 0x0, PlayerPedId())
                end
            end)
            RageUI.List("~y~Arms",P,W.arms,"Effect that displays when you hit the arms.",{},true,function(a0, a1, a2, a3)
                ...
                if a2 then
                    tOASIS.addBloodEffect("arms", 0xBB0, PlayerPedId())
                    tOASIS.addBloodEffect("arms", 0x58B7, PlayerPedId())
                end
            end)
            RageUI.List("~y~Legs",P,W.legs,"Effect that displays when you hit the legs.",{},true,function(a0, a1, a2, a3)
                ...
                if a2 then
                    tOASIS.addBloodEffect("legs", 0x3FCF, PlayerPedId())
                    tOASIS.addBloodEffect("legs", 0xB3FE, PlayerPedId())
                end
            end)
        end)
    end
...
AddEventHandler("OASIS:enteredCity",function()end)
AddEventHandler("OASIS:leftCity",function()end)
...
function tOASIS.addKillEffect(au, av)
    if S.lightning then
        ForceLightningFlash()
    end
    if S.pedFlash then
    ...
    if S.particle ~= 1 and (tOASIS.isPlatClub() or av) then
        Citizen.CreateThreadNow(function()
            ar(au)
        end)
    end
...
function tOASIS.addBloodEffect(aw, ax, ag)
    local ay = W[aw]
    if ay > 1 then
        local as = Q[ay]
        tOASIS.loadPtfx(as[1])
...
AddEventHandler("OASIS:onPlayerKilledPed",function(az)
    tOASIS.addKillEffect(az, false)
end)
local aA = {
    [0x0] = "body",
    [0x2E28] = "body",
...
AddEventHandler("OASIS:onPlayerDamagePed",function(az)
    if not tOASIS.isPlusClub() and not tOASIS.isPlatClub() then
        return
    end
    ...
    if aB then
        ...
        tOASIS.addBloodEffect(aE, ax, az)
    end
end)

RegisterNetEvent("OASIS:gotDiscord")
AddEventHandler("OASIS:gotDiscord",function()
