local a=module("oasis-vehicles","cfg_vehiclemaxspeeds")
isInGreenzone = false
local cityCap = true
local b = false
local c = false
...
function tOASIS.areGreenzonesDisabled()
    return i
end
function tOASIS.setGreenzonesDisabled(p)
    i = p
end
n(
    function()
        for q, r in pairs(m) do
            if not r[5] and not tOASIS.isPurge() or r[5] and tOASIS.isPurge() then
...
n(
    function()
        local purgeActive = tOASIS.isPurge()
        while true do
            local t = tOASIS.getPlayerPed()
            local u = tOASIS.getPlayerCoords()
            for v, w in pairs(l) do
                if not w.isPurge and not purgeActive or w.isPurge and purgeActive then
                    ...
                    SetPlayerInvincible(tOASIS.getPlayerId(), false)
                    ...
                    SetNetworkVehicleAsGhost(tOASIS.getPlayerVehicle(), false)
                end
            end
            o(250)
        end
...
AddEventHandler("OASIS:onClientSpawn",function(y, z)
    if z then
        local A = function(B)
            inCityZone = true
        end
        ...
        tOASIS.createArea("cityzone",vector3(-225.30703735352, -916.74755859375, 31.216938018799),750.0,100,A,C,D,{})
    end
end)
local function E()
    local F = tOASIS.getPlayerCoords()
    local G = nil
    for H = 1, 25 do
    ...
    if G then
        local K, L = tOASIS.getPlayerVehicle()
        if K ~= 0 then
    ...
    while true do
        ...
        if not tOASIS.areGreenzonesDisabled() then
            ...
            if b or c then
                ...
                ClearPedBloodDamage(t)
                if usingDelgun then
                    tOASIS.setWeapon(t, "WEAPON_STAFFGUN", true)
                else
                    tOASIS.setWeapon(t, "WEAPON_UNARMED", true)
                end
                ResetPedVisibleDamage(t)
                ClearPedLastWeaponDamage(t)
            ...
            if b then
                ...
                local U, V = tOASIS.getPlayerCombatTimer()
                if U > 0 and V then
                    E()
                end
            end
...
RegisterCommand('togglecitycap', function()
    if tOASIS.getUserId() == 1 then
        cityCap = not cityCap
        tOASIS.notify('City Cap: '..(cityCap and "~g~Enabled" or "~r~Disabled"))
    end
end)