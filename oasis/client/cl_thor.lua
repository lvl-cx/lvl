...
RegisterNetEvent("OASIS:useTheForceTarget")
AddEventHandler("OASIS:useTheForceTarget",function()
    FreezeEntityPosition(PlayerPedId(),true)
    Wait(2500)
    tOASIS.loadAnimDict("ragdoll@human")
    TaskPlayAnim(tOASIS.getPlayerPed(),"ragdoll@human","electrocute",3.0,1.0,-1,01,0,0,0,0)
    Wait(5000)
    SetEntityHealth(tOASIS.getPlayerPed(),0)
    FreezeEntityPosition(PlayerPedId(),false)
end)

RegisterNetEvent("OASIS:useTheForceSync")
AddEventHandler("OASIS:useTheForceSync",function(q,r,s)
    if#(q-GetEntityCoords(PlayerPedId()))<25.0 then 
        TriggerEvent("oasis:PlaySound", "unlimitedpower")
        local s={}
        local a=0
    ...
    if tOASIS.getUserId() == 1 then
        ...
        if j then
            ...
            if A ~= -1 then
                ...
                local t = {}
                local s = r - q
                TriggerServerEvent("OASIS:theForceSync", A, q, r, s)
                for u = 1, 10, 1 do
                    table.insert(t, q + vector3(s.x / 10 * u, s.y / 10 * u, s.z / 10 * u))
                end
                TriggerEvent("oasis:PlaySound", "unlimitedpower")
                FreezeEntityPosition(PlayerPedId(), true)
                tOASIS.loadAnimDict("anim@amb@clubhouse@bar@drink@idle_a")
                TaskPlayAnim(tOASIS.getPlayerPed(),"anim@amb@clubhouse@bar@drink@idle_a","idle_a_bartender",3.0,1.0,-1,01,0,0,0,0)
