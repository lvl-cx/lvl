...
function tOASIS.setTime(f, g, h)
    b = {h = f, m = g, s = h}
end
function tOASIS.overrideTime(f, g, h)
    b.h = f
    b.m = g
    b.s = h
    c = true
end
function tOASIS.cancelOverrideTimeWeather()
    c = false
end
function tOASIS.setWeather(i)
    a = i
end
...
tOASIS.createThreadOnTick(func_manageTimeAndWeather)
RegisterNetEvent("OASIS:setTime",function(f, g, h)
    tOASIS.setTime(f, g, h)
end)
RegisterNetEvent("OASIS:setWeather",function(i)
    tOASIS.setWeather(i)
end)
...
function voteWeather(C)
    if j then
        if not k then
            TriggerServerEvent("OASIS:vote", C)
            k = true
            tOASIS.notify("~g~Vote sent!")
        else
            tOASIS.notify("~r~You have already voted!")
        end
    else
        tOASIS.notify("~r~Vote not in progress, start a vote with /voteweather!")
    end
end
function resetVotes()
    m = {
...
RegisterNetEvent("OASIS:startWeatherVote",function()
    j = true
...
RegisterNetEvent("OASIS:voteStateChange",function(D)
    m[D] = m[D] + 1
end)
RegisterCommand("voteweather",function()
    TriggerServerEvent("OASIS:tryStartWeatherVote")
end,false)
