RMenu.Add('OASISclothing','mainMenu',RageUI.CreateMenu("","Clothing Store",tOASIS.getRageUIMenuWidth(),tOASIS.getRageUIMenuHeight(),"banners","cstore"))
RMenu.Add('OASISclothing','changePed',RageUI.CreateSubMenu(RMenu:Get('OASISclothing','mainMenu'),"","Change Gender"))
RMenu.Add('OASISclothing','changeClothing',RageUI.CreateSubMenu(RMenu:Get('OASISclothing','mainMenu'),"","Change your Clothes"))
RMenu.Add('OASISclothing','clearProps',RageUI.CreateSubMenu(RMenu:Get('OASISclothing','mainMenu'),"","Clear Props"))
...
Citizen.CreateThread(function()
    while true do 
        local G=GetEntityCoords(tOASIS.getPlayerPed())
        for H=1,#r do 
            if#(G-r[H])<=2.5 and not t then 
                ...
                h=true
                RageUI.Visible(RMenu:Get('OASISclothing','mainMenu'),true)
            elseif s==H then 
                ...
                if not RageUI.Visible(RMenu:Get('OASISclothing','changeClothing'))then 
                    F()
                end 
            end 
        end
    ...
    if not n then
        if RageUI.Visible(RMenu:Get('OASISclothing', 'mainMenu')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                RageUI.Button('Apply Outfit Code','Applys an outfit code given by a player with ~y~OASIS Plus~w~ or ~y~OASIS Platinum~w~.',{RightLabel=">>>"},true,function(L,M,N)
                    if N then 
                        tOASIS.clientPrompt("Outfit Code:","",function(Q)
                            local R = string.gsub(Q, "%s+", "")
                            if #R == 5 then
                                TriggerServerEvent("OASIS:applyOutfitCode", string.upper(R))
                            else
                ...
                end,RMenu:Get('OASISclothing','clearProps'))
            end)
        end
        if RageUI.Visible(RMenu:Get('OASISclothing', 'changePed')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                RageUI.Button('MP Male','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            tOASIS.loadCustomisationPreset("DefaultMale")
                        else
                            tOASIS.notify("~r~Cannot change gender, you're dead.")
                        end
                    end 
                end,function()
                end)
                RageUI.Button('MP Female','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            tOASIS.loadCustomisationPreset("DefaultFemale")
                        else
                            tOASIS.notify("~r~Cannot change gender, you're dead.")
                        end
                    end 
        ...
        if RageUI.Visible(RMenu:Get('OASISclothing', 'changeClothing')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                m=true
                if h then 
                    for H=1,#a,1 do 
                        ...
                        RageUI.List(P.name,get_drawables(P.type,P.index),a[H].currentListIndex,"Current Texture Index: "..k.textureN.."/"..j-1,{},true,function(L,M,N,Q)
                            ...
                            if N then 
                                ...
                                if k.index==2 then 
                                    ...
                                    TriggerServerEvent("OASIS:saveClothingHairData",GetPedDrawableVariation(tOASIS.getPlayerPed(),2),GetPedHairColor(tOASIS.getPlayerPed()))
...
function current_clothing(R,i,S)
    if R=="prop"then 
        if S then 
            if GetPedPropTextureIndex(tOASIS.getPlayerPed(),i)~=nil then 
                return GetPedPropTextureIndex(tOASIS.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedPropIndex(tOASIS.getPlayerPed(),i)~=nil then 
                if GetPedPropIndex(tOASIS.getPlayerPed(),i)==-1 then 
                    return GetPedPropIndex(tOASIS.getPlayerPed(),i)+2 
                else 
                    return GetPedPropIndex(tOASIS.getPlayerPed(),i)+1 
                end 
            else 
                return 0 
            end 
    ...
    elseif R=="drawable"then 
        ...
        elseif S then 
            if GetPedTextureVariation(tOASIS.getPlayerPed(),i)~=nil then 
                return GetPedTextureVariation(tOASIS.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedDrawableVariation(tOASIS.getPlayerPed(),i)~=nil then 
                return GetPedDrawableVariation(tOASIS.getPlayerPed(),i)+1 
            else 
                return 0 
            end 
        end 
...
function get_textures(T,R,i)
    if T=="drawable"then 
        ...
        j=GetNumberOfPedTextureVariations(tOASIS.getPlayerPed(),R,i-1)
        return j 
    else 
        j=GetNumberOfPedPropTextureVariations(tOASIS.getPlayerPed(),R,i-1)
        return j 
    end 
end
local function Y()
...
Citizen.CreateThread(function()
    while true do 
        if k.index~=nil and RageUI.Visible(RMenu:Get('OASISclothing','changeClothing'))then 
...
AddEventHandler("OASIS:onClientSpawn",function(D, E)
    if E then
		for a7,a8 in pairs(r)do 
            tOASIS.addBlip(a8.x,a8.y,a8.z,73,3,"Clothing Store",0.7)
            tOASIS.addMarker(a8.x,a8.y,a8.z,0.6,0.6,0.6,10,255,81,170,50,9,false,false,true,"dp_clothing","top",90.0,90.0,0.0)
        end 
	end
end)
