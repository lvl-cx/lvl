...
function IsMenuOpen()
	return (ArmaTattoo.IsMenuOpened('tattoo') or string.find(tostring(ArmaTattoo.CurrentMenu() or ""), "ZONE_"))	
end
...
Citizen.CreateThread(function()
	ArmaTattoo.CreateMenu("tattoo", "Tattoo Shop", function()
        return CloseTattooShop()
    end)
    ArmaTattoo.SetSubTitle('tattoo', "Tattoo Shop")
	for k, v in ipairs(Config.TattooCats) do
		ArmaTattoo.CreateSubMenu(v[1], "tattoo", v[2])
		ArmaTattoo.SetSubTitle(v[1], v[2])
	end
    while true do 
        Citizen.Wait(0)
		local CanSleep = true
			...
			for _,interiorId in ipairs(Config.interiorIds) do
				if GetInteriorFromEntity(PlayerPedId()) == interiorId then
					...
					if not IsPedInAnyVehicle(PlayerPedId(), false) then
						alert('Press ~INPUT_VEH_HORN~ to access ~b~Tattoo Shop')
        ...
        if ArmaTattoo.IsMenuOpened('tattoo') then
			CanSleep = false
            for k, v in ipairs(Config.TattooCats) do
				ArmaTattoo.MenuButton(v[2], v[1])
			end
			ClearPedDecorations(PlayerPedId())
			for k,v in pairs(currentTattoos) do
				if v.Count ~= nil then
		...
		for k, v in ipairs(Config.TattooCats) do
			if ArmaTattoo.IsMenuOpened(v[1]) then
				CanSleep = false
					...
					if tattoo.Zone == v[1] then
						if GetEntityModel(PlayerPedId()) == `mp_m_freemode_01` then
							if tattoo.HashNameMale ~= '' then
								...
								if found then
									local clicked, hovered = ArmaTattoo.SpriteButton(GetLabelText(tattoo.Name), "commonmenu", "shop_tattoos_icon_a", "shop_tattoos_icon_b")
									if clicked then
