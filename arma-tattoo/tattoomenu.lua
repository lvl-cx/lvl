...
local function debugPrint(text)
    if ArmaTattoo.debug then
        Citizen.Trace('[ArmaTattoo] '..tostring(text))
    end
end
...
-- API
function ArmaTattoo.CreateMenu(id, title, closeCallback)
    -- Default settings
    menus[id] = { }
    ...
    menus[id].maxOptionCount = 18

    menus[id].titleFont = 2
    menus[id].titleColor = {255, 255, 255, 225}
...
function ArmaTattoo.CreateSubMenu(id, parent, subTitle, closeCallback)
    if menus[parent] then
        ArmaTattoo.CreateMenu(id, menus[parent].title)
...
function ArmaTattoo.CurrentMenu()
    return currentMenu
end

function ArmaTattoo.MenuTable()
    return menus
end


function ArmaTattoo.OpenMenu(id)
    if id and menus[id] then
...
function ArmaTattoo.IsMenuOpened(id)
    return isMenuVisible(id)
end


function ArmaTattoo.IsAnyMenuOpened()
...
function ArmaTattoo.IsAnyMenuWithTitleOpened(subtitle)
...
function ArmaTattoo.IsMenuAboutToBeClosed()
    if menus[currentMenu] then
...
function ArmaTattoo.IsThisMenuAboutToBeClosed(id)
    if menus[id] then
...
function ArmaTattoo.Button(text, subText)
...
function ArmaTattoo.DisabledButton(text, subText)
    local buttonText = text
    if subText then
        buttonText = '{ '..tostring(buttonText)..', '..tostring(subText)..' }'
    end
...
function ArmaTattoo.SpriteButton(text, textDict, sprite, focusSprite)
    local buttonText = text
...
function ArmaTattoo.SpriteMenuButton(text, textDict, sprite, focusSprite, id)
    if menus[id] then
        local clicked, hovered = ArmaTattoo.SpriteButton(text, textDict, sprite, focusSprite)
        if clicked then
            setMenuVisible(currentMenu, false)
...
function ArmaTattoo.ComboBox(text, items, currentIndex, selectedIndex, callback, displaycb)
    ...
    if ArmaTattoo.Button(text, selectedItem) then
...
function ArmaTattoo.MenuButton(text, id, secondtext)
    if menus[id] then
		local clicked, hovered = ArmaTattoo.Button(text, (secondtext and secondtext or "→")) 
		if clicked then
            setMenuVisible(currentMenu, false)
            setMenuVisible(id, true, true)
...
function ArmaTattoo.SwitchMenu(id)
	setMenuVisible(currentMenu, false)
    setMenuVisible(id, true, true)
end

function ArmaTattoo.SetDescription(text)
	currentDesc = tostring(text)
end

function ArmaTattoo.CheckBox(text, bool, callback)
    --[[local checked = '~r~Off'
    if bool then
        checked = '~g~On'
    end]]
    ...
    if ArmaTattoo.SpriteButton(text, "commonmenu", sprite, focusSprite) then
        bool = not bool
...
function ArmaTattoo.SetMenuWidth(id, width)
    setMenuProperty(id, 'width', width)
end


function ArmaTattoo.SetMenuX(id, x)
    setMenuProperty(id, 'x', x)
end


function ArmaTattoo.SetMenuY(id, y)
    setMenuProperty(id, 'y', y)
end


function ArmaTattoo.SetMenuMaxOptionCountOnScreen(id, count)
    setMenuProperty(id, 'maxOptionCount', count)
end


function ArmaTattoo.SetTitleColor(id, r, g, b, a)
    setMenuProperty(id, 'titleColor', { r, g, b, a or  menus[id].titleColor[4] })
end


function ArmaTattoo.SetTitleBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, 'titleBackgroundColor', { r, g, b, a or menus[id].titleBackgroundColor[4] })
end


function ArmaTattoo.UseSpriteAsBackground(id, textDict, sprite, r, g, b, a, stillDrawText)
