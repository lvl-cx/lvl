...
local function play_drink(player)
  ...
  ATMclient.playAnim(player,{true,seq,false})
end
...
pills_choices["Take"] = {function(player,choice)
  ...
  if user_id ~= nil then
    ATMclient.isInComa(player,{}, function(in_coma)    
        if not in_coma then
          if ATM.tryGetInventoryItem(user_id,"pills",1) then
            ATMclient.varyHealth(player,{25})
            ATMclient.notify(player,{"~g~ Taking pills."})
            play_drink(player)
...
local function play_smoke(player)
  ...
  ATMclient.playAnim(player,{true,seq2,false})
end
...
smoke_choices["Take"] = {function(player,choice)
  ...
  if user_id ~= nil then
    if ATM.tryGetInventoryItem(user_id,"weed",1) then
	  ATM.varyHunger(user_id,(20))
      ATMclient.notify(player,{"~g~ smoking weed."})
      play_smoke(player)
      ATM.closeMenu(player)
    end
  end
...
local function play_smell(player)
  ...
  ATMclient.playAnim(player,{true,seq3,false})
end
...
smell_choices["Take"] = {function(player,choice)
  ...
  if user_id ~= nil then
    if ATM.tryGetInventoryItem(user_id,"cocaine",1) then
	  ATM.varyThirst(user_id,(20))
      ATMclient.notify(player,{"~g~ smell cocaine."})
      play_smell(player)
      ATM.closeMenu(player)
    end
  end
...
local function play_lsd(player)
  ...
  ATMclient.playAnim(player,{true,seq4,false})
end
...
lsd_choices["Take"] = {function(player,choice)
  ...
  if user_id ~= nil then
    if ATM.tryGetInventoryItem(user_id,"lsd",1) then
	  ATM.varyThirst(user_id,(20))
      ATMclient.notify(player,{"~g~ Taking lsd."})
      play_lsd(player)
      ATM.closeMenu(player)
    end
  end
