...
local function tr_remove_player(tr,player) -- remove player from transforming
  ...
  ATMclient.removeProgressBar(player,{"ATM:tr:"..tr.name})
...
local function tr_add_player(tr,player,recipe) -- add player to transforming
  ...
  ATM.closeMenu(player)
  ATMclient.setProgressBar(player,{"ATM:tr:"..tr.name,"center",recipe.."...",tr.itemtr.r,tr.itemtr.g,tr.itemtr.b,0})
...
local function tr_tick(tr) -- do transformer tick
  for k,v in pairs(tr.players) do
    local user_id = ATM.getUserId(tonumber(k))
    local player = ATM.getUserSource(tonumber(user_id))
    if v and user_id ~= nil then -- for each player transforming
      
     ATMclient.getPosition(player,{},function(x,y,z)
	    ATMclient.getDistanceBetweenCoords(player,{x,y,z,tr.itemtr.x,tr.itemtr.y,tr.itemtr.z},function(distance)
      ...
      if tr.units > 0 and recipe then -- check units
        ...
        for l,w in pairs(recipe.reagents) do
          reagents_ok = reagents_ok and (ATM.getInventoryItemAmount(user_id,l) >= w)
        end

        -- check money
        local money_ok = (ATM.getMoney(user_id) >= recipe.in_money)
        ...
        local new_weight = ATM.getInventoryWeight(user_id)+ATM.computeItemsWeight(out_witems)-ATM.computeItemsWeight(in_witems)

        local inventory_ok = true
        if new_weight > ATM.getInventoryMaxWeight(user_id) then
          inventory_ok = false
          ATMclient.notify(tonumber(k), {lang.inventory.full()})
        end
        ...
        if money_ok and reagents_ok and inventory_ok then -- do transformation
          ...
          -- consume reagents
          if recipe.in_money > 0 then ATM.tryPayment(user_id,recipe.in_money) end
          for l,w in pairs(recipe.reagents) do
            ATM.tryGetInventoryItem(user_id,l,w,true)
          end

          -- produce products
          if recipe.out_money > 0 then ATM.giveMoney(user_id,recipe.out_money) end
          for l,w in pairs(recipe.products) do
            ATM.giveInventoryItem(user_id,l,w,true)
          end
          ...
          for l,w in pairs(recipe.aptitudes or {}) do
            ...
            if #parts == 2 then
              ATM.varyExp(user_id,parts[1],parts[2],w)
            end
          end
  ...
  for k,v in pairs(tr.players) do
    ATMclient.setProgressBarValue(k,{"ATM:tr:"..tr.name,math.floor(tr.units/tr.itemtr.max_units*100.0)})
    
    if tr.units > 0 then -- display units left
      ATMclient.setProgressBarText(k,{"ATM:tr:"..tr.name,v.."... "..tr.units.."/"..tr.itemtr.max_units})
    else
      ATMclient.setProgressBarText(k,{"ATM:tr:"..tr.name,"empty"})
    end
  end
end

local function bind_tr_area(player,tr) -- add tr area to client
  ATM.setArea(player,"ATM:tr:"..tr.name,tr.itemtr.x,tr.itemtr.y,tr.itemtr.z,tr.itemtr.radius,tr.itemtr.height,tr.enter,tr.leave)
end

local function unbind_tr_area(player,tr) -- remove tr area from client
  ATM.removeArea(player,"ATM:tr:"..tr.name)
end
...
function ATM.setItemTransformer(name,itemtr)
  ATM.removeItemTransformer(name) -- remove pre-existing transformer
  ...
  tr.enter = function(player,area)
    ...
    if user_id ~= nil and ATM.hasPermissions(user_id,itemtr.permissions or {}) then
      ATM.openMenu(player, tr.menu) -- open menu
    end
  end
...
function ATM.removeItemTransformer(name)
...
SetTimeout(5000,function()
  ...
  for k,v in pairs(cfg.item_transformers) do
    ATM.setItemTransformer("cfg:"..k,v)
  end
end)
...
local function hidden_placement_tick()
  ATM.getSData("ATM:hidden_trs", function(data)
    ...
    for k,v in pairs(cfg.hidden_transformers) do
      ...
      if tonumber(os.time())-htr.timestamp >= cfg.hidden_transformer_duration*60 then
        ...
        ATM.removeItemTransformer("cfg:"..k)
      ...
      if transformers["cfg:"..k] == nil then
        ...
        ATM.setItemTransformer("cfg:"..k, v.def)
      end
    end

    ATM.setSData("ATM:hidden_trs",json.encode(hidden_trs)) -- save hidden transformers
  end)
...
local function ch_informer_buy(player,choice)
  ...
  if user_id ~= nil and tr ~= nil then
    if ATM.tryPayment(user_id, price) then
      ATMclient.setGPS(player, {tr.itemtr.x,tr.itemtr.y}) -- set gps marker
      ATMclient.notify(player, {lang.money.paid({price})})
      ATMclient.notify(player, {lang.itemtr.informer.bought()})
    else
...
local function informer_enter()
  ...
  if user_id ~= nil then
    ATM.openMenu(source,informer_menu) 
  end
end
...
local function informer_placement_tick()
  ...
  for k,v in pairs(ATM.rusers) do
    ...
    ATMclient.setNamedBlip(player,{"ATM:informer",x,y,z,cfg.informer.blipid,cfg.informer.blipcolor,lang.itemtr.informer.title()})
    ATMclient.setNamedMarker(player,{"ATM:informer",x,y,z-1,0.7,0.7,0.5,0,255,125,125,150})
    ATM.setArea(player,"ATM:informer",x,y,z,1,1.5,informer_enter,informer_leave)
  end
  ...
  SetTimeout(cfg.informer.duration*60000, function()
    for k,v in pairs(ATM.rusers) do
      local player = ATM.getUserSource(tonumber(k))
      ATMclient.removeNamedBlip(player,{"ATM:informer"})
      ATMclient.removeNamedMarker(player,{"ATM:informer"})
      ATM.removeArea(player,"ATM:informer")
    end
  end)
