...
function ATM.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  ATMclient.openMenuData(source,{menudata})
end

-- force close player menu
function ATM.closeMenu(source)
  ATMclient.closeMenu(source,{})
end
...
function ATM.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  ATMclient.prompt(source,{title,default_text})
end
...
function ATM.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  ATMclient.request(source,{id,text,time}) -- send request to client
...
function ATM.registerMenuBuilder(name, builder)
...
function ATM.buildMenu(name, data, cbr)
...
function ATM.openMainMenu(source)
  ATM.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    ATM.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tATM.closeMenu(id)
...
function tATM.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tATM.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tATM.requestResult(id,ok)
...
function tATM.openMainMenu()
  ATM.openMainMenu(source)
end
...
function ATM.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for ATM.addStaticMenuChoices calls
...
AddEventHandler("ATM:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    ATMclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
