...
function ATM.getUserGroups(user_id)
...
function ATM.addUserGroup(user_id, group)
    if not ATM.hasGroup(user_id, group) then
        local user_groups = ATM.getUserGroups(user_id)
        ...
        if ngroup then
            ...
            TriggerEvent("ATM:playerJoinGroup", user_id, group, gtype)
        end
    end
end
...
function ATM.getUserGroupByType(user_id, gtype)
    local user_groups = ATM.getUserGroups(user_id)
    for k, v in pairs(user_groups) do
        local kgroup = groups[k]
        if kgroup then
...
function ATM.getUsersByGroup(group)
    local users = {}

    for k, v in pairs(ATM.rusers) do
        if ATM.hasGroup(tonumber(k), group) then
...
function ATM.getUsersByPermission(perm)
    local users = {}

    for k, v in pairs(ATM.rusers) do
        if ATM.hasPermission(tonumber(k), perm) then
            table.insert(users, tonumber(k))
        end
    end
...
function ATM.removeUserGroup(user_id, group)
    local user_groups = ATM.getUserGroups(user_id)
    ...
    TriggerEvent("ATM:playerLeaveGroup", user_id, group, gtype)

    user_groups[group] = nil -- remove reference
end

-- check if the user has a specific group
function ATM.hasGroup(user_id, group)
    local user_groups = ATM.getUserGroups(user_id)
...
-- check if the user has a specific permission
function ATM.hasPermission(user_id, perm)
    local user_groups = ATM.getUserGroups(user_id)
    ...
    if fchar == "@" then -- special aptitude permission
        ...
        if #parts == 3 then -- decompose group.aptitude.operator
            ...
            local alvl = math.floor(ATM.expToLevel(ATM.getExp(user_id, group, aptitude)))
    ...
    elseif fchar == "#" then -- special item permission
        ...
        if #parts == 2 then -- decompose item.operator
            ...
            local amount = ATM.getInventoryItemAmount(user_id, item)
...
function ATM.hasPermissions(user_id, perms)
...
local function ch_select(player, choice)
    ...
    if user_id ~= nil then
        ATM.addUserGroup(user_id, choice)
        ...
        for k, v in pairs(selectors) do
            ...
            if gcfg and menu then
                ...
                local function selector_enter()
                    local user_id = ATM.getUserId(source)
                    if user_id ~= nil and ATM.hasPermissions(user_id, gcfg.permissions or {}) then
                ...
                ATMclient.addBlip(source, {x, y, z, gcfg.blipid, gcfg.blipcolor, k})
                ATMclient.addMarker(source, {x, y, z - 1, 0.7, 0.7, 0.5, 255, 154, 24, 125, 150})

                ATM.setArea(source, "ATM:gselector:" .. k, x, y, z, 1, 1.5, selector_enter, selector_leave)
            end
        end
    end
end
    ...
    if first_spawn then
        ...
        if user ~= nil then
            for k, v in pairs(user) do
                ATM.addUserGroup(user_id, v)
            end
        end

        -- add default group user
        ATM.addUserGroup(user_id, "user")
    end
