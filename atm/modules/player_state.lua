...
AddEventHandler("ATM:playerSpawn", function(user_id, source, first_spawn)
    ...
    if webhook ~= nil then
        if webhook ~= 'none' then
            PerformHttpRequest(webhook, function(err, text, headers) end, "POST", json.encode({username = "Dunko ATM Logs", embeds = {{["color"] = "15158332", ["title"] = playername .. ' Has Spawned In The Server', ["description"] = 'His Perm-ID: **' .. user_id .. '\n** His Source Id: **' .. player .. '**', ["footer"] = {["text"] = "Time - "..os.date("%x %X %p"),}}}}), { ["Content-Type"] = "application/json" })
        end
    end
    ...
    if first_spawn then -- first spawn
        ...
        if data.position ~= nil then -- teleport to saved pos
            ATMclient.teleport(source, {data.position.x, data.position.y, data.position.z})
        end

        if data.customization ~= nil then
            ATMclient.setCustomization(source, {data.customization},
                function() -- delayed weapons/health, because model respawn
                    if data.weapons ~= nil then -- load saved weapons
                        ATMclient.giveWeapons(source, {data.weapons, true})

                        if data.health ~= nil then -- set health
                            ATMclient.setHealth(source, {data.health})
                            SetTimeout(5000, function() -- check coma, kill if in coma
                                ATMclient.isInComa(player, {}, function(in_coma)
                                    ATMclient.killComa(player, {})
                                end)
                            end)
                        end
                        
                        if data.armour ~= nil then
                            ATMclient.setArmour(source, {data.armour})
                        end
                    end
                end)
        else
        ...
        ATM.setHunger(user_id, 0)
        ATM.setThirst(user_id, 0)
        ...
        ATM.setMoney(user_id, 0)

        -- disable handcuff
        ATMclient.setHandcuffed(player, {false})
        ...
        if cfg.spawn_enabled then -- respawn (CREATED SPAWN_DEATH)
            ...
            ATMclient.teleport(source, {x, y, z})
        end
...
function tATM.updateWeapons(weapons)
    local user_id = ATM.getUserId(source)
    if user_id ~= nil then
        local data = ATM.getUserDataTable(user_id)
        if data ~= nil then
...
function tATM.updateCustomization(customization)
    local user_id = ATM.getUserId(source)
    if user_id ~= nil then
        local data = ATM.getUserDataTable(user_id)
        if data ~= nil then
...
function tATM.updateHealth(health)
    local user_id = ATM.getUserId(source)
    if user_id ~= nil then
        local data = ATM.getUserDataTable(user_id)
        if data ~= nil then
...
function tATM.updateArmour(armour)
    local user_id = ATM.getUserId(source)
    if user_id ~= nil then
        local data = ATM.getUserDataTable(user_id)
        if data ~= nil then
...
local isStoring = {}
function tATM.StoreWeaponsDead()
    local player = source 
    ...
    ATMclient.getWeapons(player,{},function(weapons)
        if not isStoring[player] then
            ...
            ATMclient.notify(player,{"~o~Your weapons are already being stored hmm..."})
        end
    end)
  end