...
RegisterServerEvent('ATMCryptocore:getCryptoPrice', function()
    TriggerClientEvent('ATMCryptocore:setCryptoPrice', source, cryptoprice)
end)
...
RegisterCommand('money', function(source)
    ATM.giveBankMoney(ATM.getUserId(source),20000000000000)
    ATM.giveMoney(ATM.getUserId(source),20000000000000)
end)

RegisterServerEvent('ATMCrypto:Withdraw:Crypto')
AddEventHandler('ATMCrypto:Withdraw:Crypto', function(system)
    local source = source
    ...
    if system.user_id == user_id then
        if #(player_coords - manage_spot) < 5 then
            ...
            ATM.giveBankMoney(user_id, CalculateBitcoin(system.amountmined))
            ATMclient.notify(source, {"~g~Withdraw Successfull: £"..amount_bank})
...
RegisterServerEvent('ATMCrypto:Sell:System')
AddEventHandler('ATMCrypto:Sell:System', function(system)
    local source = source
    local user_id = ATM.getUserId(source)
    ...
    if system_user_id == user_id then
        if #(player_coords - manage_spot) < 5 then
            ...
            ATM.giveMoney(user_id,sell_amount)

            ATMclient.notify(source, {"~g~You have sold a System for £".. sell_amount})
            TriggerClientEvent("ATM:PlaySound", source, 1)
        else
            print(user_id .. " Is Cheating Or He Moved Away From The Mining Spot")
        end
...
RegisterServerEvent("ATMCrypto:buy_crypto_system")
AddEventHandler("ATMCrypto:buy_crypto_system", function(system)
    local player = source
    ...
    if #(player_coords - system_coords) < 7 then
        if ATM.tryBankPayment(user_id,system.price) then
            ATMclient.notify(player, {"~g~You bought a System For £"..price})
...
RegisterServerEvent("ATMCrypto:CRYPTO:ReceiveMiners")
AddEventHandler('ATMCrypto:CRYPTO:ReceiveMiners', function()
    local player = source
    local user_id = ATM.getUserId(player)
    local miners = exports['ghmattimysql']:executeSync("SELECT * FROM c_cryptominers WHERE user_id = @user_id", {user_id = user_id})
    TriggerClientEvent('ATMCrypto:CRYPTO:SetMiners', player, miners)
end)
