...
AddEventHandler("Infinite:CarryRequest", function(targetSrc)
	...
	userid = ATM.getUserId(senderSrc)
	if ATM.hasPermission(userid, 'staffon.perm') then 
		TriggerClientEvent("Infinite:StartCarry",senderSrc,targetSrc)
	else
		ATMclient.notify(targetSrc,{"Player: ~b~"..senderSrcName.."~w~ is trying to carry you, press ~g~=~w~ to accept or ~r~-~w~ to refuse"})
		ATMclient.notify(senderSrc,{"Sent carry request to Temp ID: "..targetSrc})
...
AddEventHandler("Infinite:CarryAccepted", function(senderSrc)
	...
	ATMclient.notify(targetSrc,{"~g~Carry request accepted."})
	ATMclient.notify(senderSrc,{"~g~Your carry request to "..targetSrcName.." has been accepted."})
	TriggerClientEvent("Infinite:StartCarry",senderSrc,targetSrc)
end)
...
AddEventHandler("Infinite:CarryDeclined", function(senderSrc)
	...
	local targetSrc = source
	local targetSrcName = GetPlayerName(targetSrc)
	ATMclient.notify(senderSrc,{"~r~Your carry request to "..targetSrcName.." has been declined."})
	ATMclient.notify(targetSrc,{"~r~Carry request denied."})
end)