        ...
        ATMclient.getNearestPlayer(player, {10}, function(nplayer)
            ...
            if nuser_id ~= nil then
                ...
                SetTimeout(10000, function()
                    ATMclient.varyHealth(nplayer, 100) -- heal 100
                    ATMclient.notify(nplayer,{"~g~You have been healed by an NHS Member, free of charge"})
                    ATMclient.notify(player,{"~g~You healed someone, as a reward, here is £5,000 into your bank"})
                    ATM.giveBankMoney(player,5000)
                end)
            else
                ATMclient.notify(player, {"~r~There is no player nearby"})
            end
