    ...
    if #(playerCoords - boxCoords) < 2.0 then
        if not used then
                ...
                if lootrandom == 1 then -- [Legendary]
                    -- [Legendary]
                    ATM.giveInventoryItem(user_id, "wbody|" .. 'WEAPON_MOSIN', 1, true)
                    ATM.giveInventoryItem({user_id, '7.62 Bullets', 250, true})

                    ATM.giveInventoryItem(user_id, "wbody|" .. 'WEAPON_HK45', 1, true)
                    ATM.giveInventoryItem(user_id, '9mm Bullets', 250, true)
                    ATM.giveInventoryItem(user_id, "body_armor", 3, true)
                    ATMclient.notify(source,{'Received ~g~£200,000 ~w~Cash.'})
                    ATM.giveMoney(user_id,200000)
...
RegisterServerEvent('updateLoot', function(source, item, amount)
    ...
    if #currentLoot == 0 then
        if not used then
            ...
            TriggerClientEvent('Infinite:SendSecondaryInventoryData', source, currentLoot, ATM.computeItemsWeight({currentLoot}), 30)
end) 
