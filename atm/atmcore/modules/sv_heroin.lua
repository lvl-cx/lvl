...
RegisterNetEvent('ATM:GatherHeroin')
AddEventHandler('ATM:GatherHeroin', function()
      local source = source
      Citizen.Wait(5000)
      ...
      if #(playerCoords - vector3(Drugs.Heroin.Gather.x,Drugs.Heroin.Gather.y,Drugs.Heroin.Gather.z)) <= Drugs.Heroin.Gather.radius then 
        if user_id ~= nil and ATM.hasPermission(user_id, "heroin.job") then
          local amount = 5
          local item = 1.00
          ...
          if new_weight > ATM.getInventoryMaxWeight(user_id) then
            ATMclient.notify(source,{"~r~Not enough space in inventory."})
              ...
              ATM.giveInventoryItem(user_id, 'opium', 5, true)
          end
        else
...
RegisterNetEvent('ATM:ProcessHeroin')
AddEventHandler('ATM:ProcessHeroin', function()
    local source = source
    Citizen.Wait(5000)
    local ped = GetPlayerPed(source)
    local playerCoords = GetEntityCoords(ped)
    ...
    if #(playerCoords - vector3(Drugs.Heroin.Process.x,Drugs.Heroin.Process.y,Drugs.Heroin.Process.z)) <= Drugs.Heroin.Process.radius then 
      if ATM.hasPermission(user_id, "heroin.job") then 
          if ATM.getInventoryItemAmount(user_id, 'opium') >= 5 then

            if ATM.getInventoryItemAmount(user_id, 'opium') >= 5 then
              ATM.tryGetInventoryItem(user_id, 'opium', 5, true)
              ATM.giveInventoryItem(user_id, 'heroin', 1, true)

            end
          else
            ATMclient.notify(source, {"~r~You do not have that much Opium."})
          end
      else
        ATMclient.notify(source,{"~r~You do not have the correct license."})
      end
...
local commision = 0
local finalID = nil
RegisterNetEvent('ATM:SellHeroin')
AddEventHandler('ATM:SellHeroin', function()
    local user_id = ATM.getUserId(source)
    local ped = GetPlayerPed(source)
    local playerCoords = GetEntityCoords(ped)
    if #(playerCoords - vector3(Drugs.Heroin.Sell.x,Drugs.Heroin.Sell.y,Drugs.Heroin.Sell.z)) <= 5.0 then 
      if ATM.tryGetInventoryItem(user_id,'heroin', 1) then
        if user_id ~= nil then
          ...
          ATMclient.notify(source, {"~g~Sold 1 heroin for £" .. tostring(price - finalCommision) .. " ~w~+" .. commision .. "% Commision!"})
