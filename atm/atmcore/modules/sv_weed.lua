...
RegisterNetEvent('ATM:GatherWeed')
AddEventHandler('ATM:GatherWeed', function()
      local source = source
      Citizen.Wait(5000)
      ...
      if #(playerCoords - vector3(Drugs.Weed.Gather.x,Drugs.Weed.Gather.y,Drugs.Weed.Gather.z)) <= Drugs.Weed.Gather.radius then 
        if user_id ~= nil and ATM.hasPermission(user_id, "weed.job") then
          local amount = 5
          local item = 1.00
          ...
          if new_weight > ATM.getInventoryMaxWeight(user_id) then
            ATMclient.notify(source,{"~r~Not enough space in inventory."})
              ...
              ATM.giveInventoryItem(user_id, 'cannabis', 5, true)
          end
        else
...
RegisterNetEvent('ATM:ProcessWeed')
AddEventHandler('ATM:ProcessWeed', function()
    local source = source
    Citizen.Wait(5000)
    local ped = GetPlayerPed(source)
    local playerCoords = GetEntityCoords(ped)
    ...
    if #(playerCoords - vector3(Drugs.Weed.Process.x,Drugs.Weed.Process.y,Drugs.Weed.Process.z)) <= Drugs.Weed.Process.radius then 
      if ATM.hasPermission(user_id, "weed.job") then 
          if ATM.getInventoryItemAmount(user_id, 'cannabis') >= 5 then

            if ATM.getInventoryItemAmount(user_id, 'cannabis') >= 5 then
              ATM.tryGetInventoryItem(user_id, 'cannabis', 5, true)
              ATM.giveInventoryItem(user_id, 'weed', 1, true)

            end
          else
            ATMclient.notify(source, {"~r~You do not have that much Cannabis Sativa."})
          end
      else
        ATMclient.notify(source,{"~r~You do not have the correct license."})
      end
...
local commision = 0
local finalID = nil
RegisterNetEvent('ATM:SellWeed')
AddEventHandler('ATM:SellWeed', function()
    local user_id = ATM.getUserId(source)
    local ped = GetPlayerPed(source)
    local playerCoords = GetEntityCoords(ped)
    if #(playerCoords - vector3(Drugs.Weed.Sell.x,Drugs.Weed.Sell.y,Drugs.Weed.Sell.z)) <= 5.0 then 
      if ATM.tryGetInventoryItem(user_id,'weed', 1) then
        if user_id ~= nil then
        ...
        ATMclient.notify(source, {"~g~Sold 1 weed for £" .. tostring(price - finalCommision) .. " ~w~+" .. commision .. "% Commision!"})
