...
RMenu.Add('ATMStores', 'main', RageUI.CreateMenu("", "~g~ATM Shop", 1300, 50, 'shop', 'shop'))
RMenu.Add("ATMStores", "confirm", RageUI.CreateSubMenu(RMenu:Get('ATMStores', 'main',  1300, 50)))

RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("ATMStores", "main")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for k, v in pairs(cfg.shopItems) do
                ...
                end, RMenu:Get("ATMStores", "confirm"))
            end
        end)
    end
end)
...
local Index = 1
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("ATMStores", "confirm")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
        ...
        RageUI.Button("Confirm Purchase" , nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected)
            if Selected then
                ...
                TriggerServerEvent("ATM:BuyStoreItem", cHash, cPrice * Index, tonumber(Index), cLoaction)

            end
        end, RMenu:Get("ATMStores", "main"))
        ...
        for k, v in pairs(cfg.shops) do
            ...
            if isInArea(v, 1.0) and inMenu == false then
                ...
                if IsControlJustPressed(0, 51) then 
                    ...
                    RageUI.Visible(RMenu:Get("ATMStores", "main"), true)
                    cLoaction = v
                end
            end
