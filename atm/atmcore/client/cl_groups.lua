...
RMenu.Add('GroupMenu', 'groups',  RageUI.CreateMenu("", "~g~ATM License Menu", 1300, 50, "groups", "groups"))
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            if SelectedGroup == 'Heroin' then 
                ...
                RageUI.Button('Refund License - ~g~[£' .. '2,500,000' ..']', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        TriggerServerEvent('ATM:RefundLicense', SelectedGroup)
            ...
            RageUI.Button('Sell License to Nearest Player', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                if Selected then 
                   TriggerServerEvent('ATM:SellLicense', SelectedGroup)
                end
            end)
    end) 
end
