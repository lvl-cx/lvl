          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                TriggerServerEvent('ATM:GatherDiamond')
                
                exports.rprogress:Start("", 5000)
                ClearPedTasksImmediately(ped)
                Action = false
              else
                ATM.notify({"~r~You cant gather Diamond while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TriggerServerEvent('ATM:ProcessDiamond')
...
-- [RageUI Menu]
RMenu.Add('SellerMenu', 'main', RageUI.CreateMenu("", "~g~ATM Seller",1300, 50, "seller", "seller"))
  ...
  if RageUI.Visible(RMenu:Get('SellerMenu', 'main')) then
    ...
    RageUI.Button("Sell Gold" , nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected)
        if Selected then
            ...
            TriggerServerEvent('ATM:SellGold')
        end
    end)
    ...
    RageUI.Button("Sell Diamond" , nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected)
      if Selected then
          ...
          TriggerServerEvent('ATM:SellDiamond')
      end
  end)
