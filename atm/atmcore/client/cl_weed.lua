          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                TriggerServerEvent('ATM:GatherWeed')
                
                exports.rprogress:Start("", 5000)
                ClearPedTasksImmediately(ped)
                Action = false
              else
                ATM.notify({"~r~You cant gather Weed while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TriggerServerEvent('ATM:ProcessWeed')
...
-- [RageUI Menu]
RMenu.Add('WeedMenu', 'Weed Seller', RageUI.CreateMenu("", "~g~ATM Weed Seller",1300, 50, "weed", "weed"))
  ...
  if RageUI.Visible(RMenu:Get('WeedMenu', 'Weed Seller')) then
    ...
    RageUI.Button("Sell Weed" , nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected)
        if Selected then
            ...
            TriggerServerEvent('ATM:SellWeed')
        end
    end)
