...
RMenu.Add('SandySmallArms', 'main', RageUI.CreateMenu("", "~g~ATM " .. sandysmall.name, 1300, 50, sandysmall.banner, sandysmall.banner))
...
AddEventHandler("ATM:PlaySound", function(status)
    if status == 1 then 
...
local clothingID = 4
local index = 0
RegisterNetEvent('ATM:SetVest')
AddEventHandler('ATM:SetVest', function()
    player = GetPlayerPed(-1)
    SetPedComponentVariation(player, 9, clothingID, index, 0) 
end)


RegisterNetEvent('ATM:ChangeArmour2')
AddEventHandler('ATM:ChangeArmour2', function(id, indexz)
  clothingID = id
  index = indexz
end)
