          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                TriggerServerEvent('ATM:GatherCocaine')
                
                exports.rprogress:Start("", 5000)
                ClearPedTasksImmediately(ped)
                Action = false
              else
                ATM.notify({"~r~You cant gather Cocaine while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TriggerServerEvent('ATM:ProcessCocaine')
...
-- [RageUI Menu]
RMenu.Add('CocaineMenu', 'Cocaine Seller', RageUI.CreateMenu("", "~g~ATM Cocaine Seller",1300, 50, "cocaine", "cocaine"))
  ...
  if RageUI.Visible(RMenu:Get('CocaineMenu', 'Cocaine Seller')) then
    ...
    RageUI.Button("Sell Cocaine" , nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected)
        if Selected then
            ...
            TriggerServerEvent('ATM:SellCocaine')
        end
    end)
