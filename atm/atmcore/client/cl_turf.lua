...
RegisterNetEvent('ATM:TakenTurf')
AddEventHandler('ATM:TakenTurf', function(isnTurf)
...
RegisterNetEvent('ATM:OutOfZone')
AddEventHandler('ATM:OutOfZone', function(isnTurf)
	inTurf = false
...
RegisterNetEvent('ATM:PlayerDied')
...
RegisterNetEvent('ATM:TurfComplete')
AddEventHandler('ATM:TurfComplete', function(reward, name)
	inTurf = false
	secondsRemaining = 0
...
RegisterNetEvent("ATM:DontIt")
AddEventHandler("ATM:DontIt", function(bool)
	TakenTurf = bool 
end)

RegisterNetEvent("ATM:DontItWeed")
AddEventHandler("ATM:DontItWeed", function(bool)
	TakenTurfWeed = bool 
end)

RegisterNetEvent("ATM:DontItCocaine")
AddEventHandler("ATM:DontItCocaine", function(bool)
	TakenTurfCocaine = bool 
end)


RegisterNetEvent("ATM:DontItLSD")
AddEventHandler("ATM:DontItLSD", function(bool)
	TakenTurfLSD = bool 
end)

RegisterNetEvent("ATM:DontItHeroin")
AddEventHandler("ATM:DontItHeroin", function(bool)
...
function changeComision()
    ...
    if (GetOnscreenKeyboardResult()) then
		...
		if  resultID then
            TriggerServerEvent("ATM:ChangeCommision", tonumber(resultID))
...
function changeComision2()
    ...
    if (GetOnscreenKeyboardResult()) then
        local resultID = GetOnscreenKeyboardResult()
		if  resultID then
            TriggerServerEvent("ATM:ChangeCommisionWeed", tonumber(resultID))
...
function changeComision3()
    ...
    if (GetOnscreenKeyboardResult()) then
        local resultID = GetOnscreenKeyboardResult()
		if  resultID then
            TriggerServerEvent("ATM:ChangeCommisionCocaine", tonumber(resultID))
...
function changeComision4()
    ...
    if (GetOnscreenKeyboardResult()) then
        local resultID = GetOnscreenKeyboardResult()
		if  resultID then
            TriggerServerEvent("ATM:ChangeCommisionLSD", tonumber(resultID))
...
function changeComision5()
    ...
    if (GetOnscreenKeyboardResult()) then
        local resultID = GetOnscreenKeyboardResult()
		if  resultID then
            TriggerServerEvent("ATM:ChangeCommisionHeroin", tonumber(resultID))
				...
				if not inTurf then
					...
					if (Vdist(pos.x, pos.y, pos.z, pos3.x, pos3.y, pos3.z) < 1.4) then
						...
						if (IsControlJustReleased(1, 51)) then
							TriggerServerEvent('ATM:TakeTurf', k)
...
RegisterNetEvent("ATM:MakeTurfTrue")
AddEventHandler("ATM:MakeTurfTrue", function(isit)
	if isit then
		inTurf = true
	else 
		inTurf = false 
