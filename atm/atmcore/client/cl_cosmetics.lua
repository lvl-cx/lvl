...
RMenu.Add("CosmeticMenu", "main",  RageUI.CreateMenu("", "~g~ATM Cosmetic Menu", 1300, 50, "cosmetics", "cosmetics"))
    ...
    if RageUI.Visible(RMenu:Get("CosmeticMenu", "main")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Button('Your Cosmetics', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                if Selected then 
                    TriggerServerEvent('ATM:GetCosmetic')
                end
            ...
            for i,v in pairs(cosmetics.shop) do 
                if CosmeticItem == v.item then
                    ...
                    RageUI.Button('Confirm', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                        if Selected then 
                            TriggerServerEvent('ATM:BuyCosmetic', v.item)
                        end
                    end, RMenu:Get("CosmeticMenu", "standard"))
                ...
                if SelectedCosmetic == v.item then 
                            ...
                            if Selected then 
                                if not VestEquip then
                                    if GetPedArmour(PlayerPedId()) ~= 0 then 
                                        ...
                                        TriggerEvent('ATM:ChangeArmour', v.clothingid, v.index)
                                        TriggerEvent('ATM:ChangeArmour2', v.clothingid, v.index)
                                        VestEquip = true
                                    else
                    ...
                    RageUI.Button('Sell Cosmetic to player', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                        if Selected then
                            ...
                            TriggerServerEvent('ATM:SellCosmeticToPlayer', v.item)
                    ...
                    RageUI.Button('List on ATM Market Place', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
            ...
            for i,v in pairs(cosmetics.cfg) do 
                if SelectedCosmetic == v.item then
                    ...
                    RageUI.Button('Confirm', 'Listing Price: ~g~£' .. listingprice .. '\n~w~Listing Message: ~g~' .. listingmessage .. '\n~w~Item Name: ~g~' .. v.item, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                        if Selected then 
                            if cooldown <= 0 then 
                                TriggerServerEvent('ATM:CosmeticMarketPlace', v.item, tonumber(listingprice), listingmessage)
                                TriggerServerEvent('ATM:GetCosmetic')
                                cooldown = 300
                            else
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for i,v in pairs(cosmetics.cfg) do 
                if SelectedCosmetic == v.item then
                    ...
                    RageUI.Button('Confirm', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                        if Selected then 
                            TriggerServerEvent('ATM:RefundCosmetic', v.item)
                            TriggerServerEvent('ATM:GetCosmetic')
                        end
                    end, RMenu:Get("CosmeticMenu", "cosmetics"))
...
-- [Functions/ Events]
RegisterNetEvent('ATM:ResetCosmetics')
AddEventHandler('ATM:ResetCosmetics', function(type2) -- [Remove All of Ped Cosmetics]
    if type2 == 'Vest' then 
        SetPedComponentVariation(PlayerPedId(), 9, 0, 0, 0) 
...
RegisterNetEvent('ATM:ReturnCosmetic')
AddEventHandler('ATM:ReturnCosmetic', function(rdata)
    data = rdata
end)
