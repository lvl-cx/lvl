          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                TriggerServerEvent('ATM:GatherLSD')
                
                exports.rprogress:Start("", 5000)
                ClearPedTasksImmediately(ped)
                Action = false
              else
                ATM.notify({"~r~You cant gather LSD while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TriggerServerEvent('ATM:ProcessLSD')
...
-- [RageUI Menu]
RMenu.Add('LSDMenu', 'LSD Seller', RageUI.CreateMenu("", "~g~ATM LSD Seller",1300, 50, "lsd", "lsd"))
  ...
  if RageUI.Visible(RMenu:Get('LSDMenu', 'LSD Seller')) then
    ...
    RageUI.Button("Sell LSD" , nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected)
        if Selected then
            ...
            TriggerServerEvent('ATM:SellLSD')
        end
    end)
