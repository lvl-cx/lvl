...
RMenu.Add('callmanager', 'main', RageUI.CreateMenu("", '~g~ATM Call Manager', 1300, 50, 'manager', 'manager'))
                        ...
                        if v[2] == GetPlayerServerId(PlayerId()) then
                            ...
                            if not isInTicket then
                                    savedCoords = GetEntityCoords(PlayerPedId())
                                        ...
                                        TriggerServerEvent("ATM:returnMe", v[1], v[2], v[3])
...
RegisterCommand("return", function()
    if takenticket then
        if savedCoords == nil then return notify("~r~Couldn't get Last Position") end
            ...
            TriggerEvent("ATM:OMioDioMode",false)
            isInTicket = false
...
AddEventHandler("staffon", function()
    ...
    TriggerEvent("ATM:OMioDioMode",true)
    isInTicket = true
end)
