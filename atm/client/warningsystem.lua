...
RegisterNetEvent("atm:showWarningsOfUser")
AddEventHandler("atm:showWarningsOfUser",function(atmwarningstables)
	showWarningSystem = true
	atmWarnings = atmwarningstables
end)

RegisterNetEvent("atm:recievedRefreshedWarningData")
AddEventHandler("atm:recievedRefreshedWarningData",function(atmwarningstables)
	atmWarnings = atmwarningstables
end)
...
RegisterCommand('warnings', function()
    ...
    if showWarningSystem then
        TriggerServerEvent("atm:refreshWarningSystem")
    end
end)
...
Citizen.CreateThread(function()
	while true do
		if showWarningSystem then
			...
			DrawAdvancedText(0.59, 0.198, 0.005, 0.0028, 0.619, 'ATM' .. ' Warnings', 255, 255, 255, 255, 7, 0)
			...
			for warningID,warningTable in pairs(atmWarnings) do
...
RegisterCommand("warn",function()
	...
	TriggerServerEvent("atm:warnPlayer",tonumber(userIDtoWarn),userWarningMessage)
end)
