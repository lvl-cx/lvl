...
Tunnel.bindInterface("ATM",tATM)

-- get server interface
ATMserver = Tunnel.getInterface("ATM","ATM")

-- add client proxy interface (same as tunnel interface)
Proxy.addInterface("ATM",tATM)

-- functions


function tATM.teleport(x,y,z)
  tATM.unjail() -- force unjail before a teleportation
  SetEntityCoords(GetPlayerPed(-1), x+0.0001, y+0.0001, z+0.0001, 1,0,0,1)
  ATMserver.updatePos({x,y,z})
end

-- return x,y,z
function tATM.getPosition()
  local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
  return x,y,z
end

--returns the distance between 2 coordinates (x,y,z) and (x2,y2,z2)
function tATM.getDistanceBetweenCoords(x,y,z,x2,y2,z2)
...
function tATM.addPlayer(player)
  players[player] = true
end

function tATM.removePlayer(player)
  players[player] = nil
end

function tATM.getNearestPlayers(radius)
  local r = {}

  local ped = GetPlayerPed(i)
  local pid = PlayerId()
  local px,py,pz = tATM.getPosition()
...
function tATM.getNearestPlayer(radius)
  local p = nil

  local players = tATM.getNearestPlayers(radius)
...
function tATM.notify(msg)
  SetNotificationTextEntry("STRING")
  AddTextComponentString(msg)
  DrawNotification(true, false)
end

function tATM.notifyPicture(icon, type, sender, title, text)
    SetNotificationTextEntry("STRING")
...
function tATM.playScreenEffect(name, duration)
...
-- name, see https://wiki.fivem.net/wiki/Screen_Effects
function tATM.stopScreenEffect(name)
  StopScreenEffect(name)
end
...
-- looping: if true, will ATMly loop the first element of the sequence until stopAnim is called
function tATM.playAnim(upper, seq, looping)
  if seq.task ~= nil then -- is a task (cf https://github.com/ImagicTheCat/ATM/pull/118)
    tATM.stopAnim(true)
...
function tATM.playSpatializedSound(dict,name,x,y,z,range)
  PlaySoundFromCoord(-1,name,x+0.0001,y+0.0001,z+0.0001,dict,0,range+0.0001,0)
end

-- play sound
function tATM.playSound(dict,name)
  PlaySound(-1,name,dict,0,0,1)
end

function tATM.playFrontendSound(dict, name)
  PlaySoundFrontend(-1, dict, name, 0)
end

--[[
-- not working
function tATM.setMovement(dict)
  if dict then
...
AddEventHandler("playerSpawned",function()
  TriggerServerEvent("ATMcli:playerSpawned")
end)

AddEventHandler("onPlayerDied",function(player,reason)
  TriggerServerEvent("ATMcli:playerDied")
end)
...
TriggerServerEvent('ATM:CheckID')

RegisterNetEvent('ATM:CheckIdRegister')
AddEventHandler('ATM:CheckIdRegister', function()
