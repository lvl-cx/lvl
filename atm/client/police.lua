...
function tATM.toggleHandcuff()
  ...
  if handcuffed then
    ...
    tATM.playAnim(true,{{"mp_arresting","idle",1}},true)
  else
    DeleteEntity(cuffs)
    tATM.stopAnim(true)
...
function tATM.setHandcuffed(flag)
  if handcuffed ~= flag then
    tATM.toggleHandcuff()
  end
end

function tATM.isHandcuffed()
  return handcuffed
end

-- (experimental, based on experimental getNearestVehicle)
function tATM.putInNearestVehicleAsPassenger(radius)
...
function tATM.putInNetVehicleAsPassenger(net_veh)
...
function tATM.putInVehiclePositionAsPassenger(x,y,z)
  local veh = tATM.getVehicleAtPosition(x,y,z)
  if IsEntityAVehicle(veh) then
    for i=1,GetVehicleMaxNumberOfPassengers(veh) do
      if IsVehicleSeatFree(veh,i) then
        SetPedIntoVehicle(GetPlayerPed(-1),veh,i)
...
function tATM.jail(x,y,z,radius)
  tATM.teleport(x,y,z) -- teleport to center
  jail = {x+0.0001,y+0.0001,z+0.0001,radius+0.0001}
end

-- unjail the player
function tATM.unjail()
  jail = nil
end

function tATM.isJailed()
  return jail ~= nil
end
...
function tATM.applyWantedLevel(new_wanted)
  Citizen.CreateThread(function()
...
Citizen.CreateThread(function()
  while true do
    ...
    if nwanted_level ~= wanted_level then
      ...
      ATMserver.updateWantedLevel({wanted_level})
    end
  end
end)
