...
-- pause
AddEventHandler("ATM:pauseChange", function(paused)
  SendNUIMessage({act="pause_change", paused=paused})
end)

-- MENU

function tATM.openMenuData(menudata)
  SendNUIMessage({act="open_menu", menudata = menudata})
end

function tATM.closeMenu()
  SendNUIMessage({act="close_menu"})
end

-- PROMPT

function tATM.prompt(title,default_text)
...
function tATM.request(id,text,time)
...
RegisterNUICallback("menu",function(data,cb)
  if data.act == "close" then
    ATMserver.closeMenu({data.id})
  elseif data.act == "valid" then
    ATMserver.validMenuChoice({data.id,data.choice,data.mod})
  end
end)
...
RegisterNUICallback("prompt",function(data,cb)
  if data.act == "close" then
    SetNuiFocus(false)
    SetNuiFocus(false)
    ATMserver.promptResult({data.result})
  end
end)
...
RegisterNUICallback("request",function(data,cb)
  if data.act == "response" then
    ATMserver.requestResult({data.id,data.ok})
  end
end)
...
function tATM.announce(background,content)
...
-- try to fix missing cfg issue (cf: https://github.com/ImagicTheCat/ATM/issues/89)
for i=1,5 do
...
function tATM.setProgressBar(name,anchor,text,r,g,b,value)
...
function tATM.setProgressBarValue(name,value)
  SendNUIMessage({act="set_pbar_val", name = name, value = value})
end

-- set progress bar text
function tATM.setProgressBarText(name,text)
  SendNUIMessage({act="set_pbar_text", name = name, text = text})
end

-- remove a progress bar
function tATM.removeProgressBar(name)
...
Citizen.CreateThread(function()
  while true do
    ...
    -- open general menu
    if IsControlJustPressed(table.unpack(cfg.controls.phone.open)) and (not tATM.isInComa() or not cfg.coma_disable_menu) and (not tATM.isHandcuffed() or not cfg.handcuff_disable_menu) and GetLastInputMethod(0) then 
      ATMserver.openMainMenu({})
    end
    ...
    if pause_menu and not paused then
      paused = true
      TriggerEvent("ATM:pauseChange", paused)
