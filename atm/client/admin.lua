...
Citizen.CreateThread(function()
    while true do
        ...
        if noclip then
            ...
            local dx, dy, dz = tATM.getCamDirection()
...
local function teleportToWaypoint()
    ...
    Citizen.CreateThread(function()
        ...
        if success then
            ...
            DoScreenFadeIn(250)
            tATM.notify('~g~Teleported success!')
        end
    end)
end
...
RegisterNetEvent("ATM:OMioDioMode")
AddEventHandler("ATM:OMioDioMode",function(DioMode)
	...
	if not OMioDioMode then
		...
		tATM.setCustomization(adminTicketSavedCustomization)
		tATM.giveWeapons(savedAdminTicketGuns,true)
        TriggerServerEvent("hello", false)

	else
        adminTicketSavedCustomization = tATM.getCustomization()
		savedAdminTicketGuns = tATM.getWeapons()
