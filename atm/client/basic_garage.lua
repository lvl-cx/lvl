...
function tATM.spawnGarageVehicle(vtype, name, pos) -- vtype is the vehicle type (one vehicle per type allowed at the same time)
    ...
    if AllowMoreThenOneCar or vehicle == nil then
        ...
        if HasModelLoaded(mhash) then
            ...
            TriggerServerEvent("ATM:PayVehicleTax")
        ...
        tATM.notify("You can only have one Veh type: " .. vtype .. " of vehicle out.")
    end
end

function tATM.despawnGarageVehicle(vtype, max_range)
    local vehicle = vehicles[vtype]
    if vehicle then
        ...
        if GetDistanceBetweenCoords(x, y, z, px, py, pz, true) < max_range then -- check distance with the vehicule
            ...
            Citizen.InvokeNative(0xEA386986E786A54F, Citizen.PointerValueIntInitialized(vehicle[3]))
            vehicles[vtype] = nil
            tATM.notify("Vehicle stored.")
        else
            tATM.notify("Too far away from the vehicle.")
        end
    end
end
...
Citizen.CreateThread(function()
  ...
  for k,v in pairs(vehicles) do
    ...
    elseif v.pos then -- not valid, respawn if with a valid position
      print("[ATM] invalid vehicle "..v[1]..", respawning...")
      tATM.spawnGarageVehicle(v[1], v[2], v.pos)
    end
  end
end)
--]]
...
function tATM.getNearestVehicle(radius)
    local x, y, z = tATM.getPosition()
...
function tATM.fixeNearestVehicle(radius)
    local veh = tATM.getNearestVehicle(radius)
    if IsEntityAVehicle(veh) then
        SetVehicleFixed(veh)
    end
end

function tATM.replaceNearestVehicle(radius)
    local veh = tATM.getNearestVehicle(radius)
    if IsEntityAVehicle(veh) then
...
function tATM.getVehicleAtPosition(x, y, z)
    x = x + 0.0001
    y = y + 0.0001
    z = z + 0.0001
...
function tATM.getNearestOwnedVehicle(radius)
    local px, py, pz = tATM.getPosition()
    for k, v in pairs(vehicles) do
...
-- return ok,x,y,z
function tATM.getAnyOwnedVehiclePosition()
    for k, v in pairs(vehicles) do
...
-- return x,y,z
function tATM.getOwnedVehiclePosition(vtype)
    local vehicle = vehicles[vtype]
    local x, y, z = 0, 0, 0
...
function tATM.getOwnedVehicleId(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
...
-- vehicle commands
function tATM.vc_openDoor(vtype, door_index)
    local vehicle = vehicles[vtype]
    if vehicle then
...
function tATM.vc_closeDoor(vtype, door_index)
    local vehicle = vehicles[vtype]
    if vehicle then
...
function tATM.vc_detachTrailer(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
...
function tATM.vc_detachTowTruck(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
...
function tATM.vc_detachCargobob(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
...
function tATM.vc_toggleEngine(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
...
function tATM.vc_toggleLock(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
        local veh = vehicle[3]
        ...
        if locked then -- unlock
            ...
            tATM.notify("~r~Vehicle unlocked.")
        else -- lock
            SetVehicleDoorsLocked(veh, 2)
            SetVehicleDoorsLockedForAllPlayers(veh, true)
            tATM.notify("~g~Vehicle locked.")
        end
    end
end
