...
RMenu.Add('CryptoMiner', 'main', RageUI.CreateMenu("", "~g~ATM Crypto Systems",1300, 50, 'crypto', 'crypto'))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('CryptoMiner', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            RageUI.Button("Buy Systems", nil, {RightLabel = '~g~→'}, true, function(Hovered, Active, Selected) 
                if Selected then end
            end, RMenu:Get("CryptoMiner", "buy_systems"))

            RageUI.Button("Manage Systems", nil, {RightLabel = '~g~→'}, true, function(Hovered, Active, Selected) 
                if Selected then 
                    TriggerServerEvent("ATMCrypto:CRYPTO:ReceiveMiners")
                end
    ...
    if RageUI.Visible(RMenu:Get('CryptoMiner', 'buy_systems')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for i=1, #cfg.systems do
                RageUI.Button(cfg.systems[i].name, nil, {RightLabel = '~g~£' .. tostring(getMoneyStringFormatted(cfg.systems[i].price))}, true, function(Hovered, Active, Selected)
                    if Selected then 
    ...
    if RageUI.Visible(RMenu:Get('CryptoMiner', 'buy_systems_manage')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Button("Buy", nil, {}, true, function(Hovered, Active, Selected)
                if Selected then 
                    TriggerServerEvent('ATMCrypto:buy_crypto_system', selected_system)
                end
    ...
    if RageUI.Visible(RMenu:Get('CryptoMiner', 'manage_systems')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for i=1, #machines_owned do
                RageUI.Button('Machine ID: '..machines_owned[i].machineid, nil, {RightLabel = '~g~' .. cfg.systems[machines_owned[i].pc_id].name}, true, function(Hovered, Active, Selected)
                    if Selected then 
    ...
    if RageUI.Visible(RMenu:Get('CryptoMiner', 'manage_systems_sub')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.Button("Withdraw BTC - ~g~[" .. '£'..roundToWholeNumber((selected_managed_system.amountmined) * cryptoprice) .. ']', nil, {}, true, function(Hovered, Active, Selected)
                if Selected then 
                    TriggerServerEvent('ATMCrypto:Withdraw:Crypto', selected_managed_system)
                end
            end, RMenu:Get("CryptoMiner", "main"))
            RageUI.Button("Sell Machine - ~g~[£" .. roundToWholeNumber(cfg.systems[selected_managed_system.pc_id].price / 4) .. ']',nil, {}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent('ATMCrypto:Sell:System', selected_managed_system)
                end
            end, RMenu:Get("CryptoMiner", "main"))
...
RegisterNetEvent('ATMCryptocore:setCryptoPrice', function(price)
    cryptoprice = price
end)

RegisterNetEvent('ATMCrypto:CRYPTO:SetMiners', function(miners)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1000)
        TriggerServerEvent('ATMCryptocore:getCryptoPrice')
    end
end)
