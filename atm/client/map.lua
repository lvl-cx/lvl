...
function tATM.addBlip(x,y,z,idtype,idcolor,text)
...
-- return native id
function tATM.setNamedBlip(name,x,y,z,idtype,idcolor,text)
  tATM.removeNamedBlip(name) -- remove old one

  named_blips[name] = tATM.addBlip(x,y,z,idtype,idcolor,text)
  return named_blips[name]
end

-- remove a named blip
function tATM.removeNamedBlip(name)
  if named_blips[name] ~= nil then
    tATM.removeBlip(named_blips[name])
...
-- return marker id
function tATM.addMarker(x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
...
-- return id
function tATM.setNamedMarker(name,x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
  tATM.removeNamedMarker(name) -- remove old marker

  named_markers[name] = tATM.addMarker(x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
  return named_markers[name]
end

function tATM.removeNamedMarker(name)
  if named_markers[name] ~= nil then
    tATM.removeMarker(named_markers[name])
...
function tATM.setArea(name,x,y,z,radius,height)
...
-- locked: boolean
-- doorswing: -1 to 1
function tATM.setStateOfClosestDoor(doordef, locked, doorswing)
...
function tATM.openClosestDoor(doordef)
  tATM.setStateOfClosestDoor(doordef, false, 0)
end

function tATM.closeClosestDoor(doordef)
  tATM.setStateOfClosestDoor(doordef, true, 0)
end