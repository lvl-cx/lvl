...
function tATM.setArmour(armour)
...
Citizen.CreateThread(function() -- coma thread
    ...
    while true do
        ...
        if IsEntityDead(GetPlayerPed(-1)) and not in_coma then --Wait for death check
            ...
            ATMserver.StoreWeaponsDead()
            ATMserver.Coma()
            TriggerEvent('ATM:IsInMoneyComa', true)
            TriggerServerEvent('ATM:InComa')
            ATMserver.MoneyDrop()
            TriggerEvent('ATM:3Seconds')
        ...
        if in_coma then
			if not calledNHS then
				if IsControlJustPressed(1, 51) then
					...
					TriggerServerEvent('ATM:recieveNHSCall2')
				end
			end
...
RegisterNetEvent("ATM:3Seconds")
AddEventHandler("ATM:3Seconds", function()
    DeathAnim = 3
    while DeathAnim <= 3 and DeathAnim >= 0 do
        Wait(1000)
        DeathAnim = DeathAnim - 1
...
function tATM.disableComa()
    in_coma = false
end

function tATM.isInComa()
    return in_coma
end

RegisterNetEvent("ATM:FixClient")
AddEventHandler("ATM:FixClient", function()
    ...
    Citizen.CreateThread(function()
        while true do 
            ...
            if resurrectspamm == true then
                ...
                tATM.stopScreenEffect(cfg.coma_effect)
                DoScreenFadeIn(500)
                Citizen.Wait(500)
            end 
        end
...
function tATM.reviveComa()
    local ped = GetPlayerPed(-1)
    SetEntityInvincible(ped,false)
    tATM.setRagdoll(false)
    tATM.stopScreenEffect(cfg.coma_effect)
    SetEntityHealth(GetPlayerPed(-1), 200) 
end

-- kill the player if in coma
function tATM.killComa()
    if in_coma then
        coma_left = 0
    end
end
...
function tATM.varyHealth(variation)
    local ped = GetPlayerPed(-1)
...
function tATM.getHealth()
    return GetEntityHealth(GetPlayerPed(-1))
end

function tATM.getArmour()
    return GetPedArmour(GetPlayerPed(-1))
end

function tATM.setHealth(health)
    local n = math.floor(health)
    SetEntityHealth(GetPlayerPed(-1),n)
end


function tATM.setFriendlyFire(flag)
    NetworkSetFriendlyFireOption(flag)
    SetCanAttackFriendly(GetPlayerPed(-1), flag, flag)
end

function tATM.setPolice(flag)
