...
Citizen.CreateThread(function()
  while true do
    ...
    if IsPlayerPlaying(PlayerId()) and state_ready then
      ...
      ATMserver.updateHealth({tATM.getHealth()})
      ATMserver.updateArmour({GetPedArmour(PlayerPedId())})
      ATMserver.updateWeapons({tATM.getWeapons()})
      ATMserver.updateCustomization({tATM.getCustomization()})
    end
  end
end)
...
function tATM.getWeaponTypes()
  return weapon_types
end

function tATM.getWeapons()
...
function tATM.giveWeapons(weapons,clear_before)
  local player = GetPlayerPed(-1)
...
function tATM.getDrawables(part)
...
function tATM.getDrawableTextures(part,drawable)
  local isprop, index = parse_part(part)
  if isprop then
...
function tATM.getCustomization()
...
function tATM.getCustomization2()
  local ped = GetPlayerPed(-1)
...
function tATM.setCustomization(custom) -- indexed [drawable,texture,palette] components or props (p0...) plus .modelhash or .model
    ...
    if custom then
      ...
      if mhash ~= nil then
        ...
        if HasModelLoaded(mhash) then
          ...
          local weapons = tATM.getWeapons()
          if GetEntityModel(PlayerPedId()) ~= GetHashKey("mp_m_freemode_01") then
            SetPlayerModel(PlayerId(), mhash)
          end
          tATM.giveWeapons(weapons,true)
...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(60000)
    if state_ready then
      local custom = tATM.getCustomization()
      custom.model = nil
      custom.modelhash = nil
      tATM.setCustomization(custom)
    end
  end
end)
--]]
