...
-- Add ATM USER_IDs and default title data
local overrides = {
    [1] =                   {prefix = {x=24,y=15}, title = "Unemployed"},
}

local user_titles = {
}

-- Add ATM USER_IDs for people who should have default access to the admin
...
function isStaff(id)
    return (ATM.hasGroup({id,"god"}) or
            ATM.hasGroup({id,"recruiter"}) or
            ATM.hasGroup({id,"mod"}) or
            ATM.hasGroup({id,"admin"}) or
            ATM.hasGroup({id,"superadmin"}))
end
...
local login_time = {}
local total_time = {}
AddEventHandler("ATM:playerJoin", function(user_id, user, name, last_login)
...
AddEventHandler("ATM:playerLeave", function(user_id, user)
				...
				if ATM.hasGroup({user_id, group}) then
					...
					if group_data.levelnames then
						...
						for level, leveldata in next, group_data.levels do
							gen_cycles = gen_cycles + 1
							if (ATM.hasPermission({user_id, "@" .. skill .. ".>" .. (level - 1)}) or
								ATM.hasPermission({user_id, "@" .. skill .. "." .. level})) and
...
function set(s,t)
    ...
    if user_id ~= nil then
        ATM.prompt({source, "User ID", "", function(player,result)
            local sel_id = tonumber(result)
            if sel_id then
                ATM.prompt({source, "Set " .. t, "", function(player,result)
                    sel_pref = result
...
function setIcon(s,t) -- user s prompted to change t icon for any user
    ...
    if user_id ~= nil then
        ATM.prompt({source, "User ID", "", function(player,result)
            local sel_id = tonumber(result)
            if sel_id then
                ATM.prompt({source, "Icon X (0-40)", "", function(player,result)
                    local sel_x = tonumber(result)
                    if sel_x == nil or result == "" then sel_x = 40 end
                    ATM.prompt({source, "Icon Y (0-39)", "", function(player,result)
...
function openTitlesMenu(player, choice, mod)
    ...
    if user_id ~= nil then
        ATM.buildMenu({"titlelist", {player = player}, function(menu)
            ...
            if CONFIG.ADD_STAFF_TITLES then
                if ATM.hasGroup({user_id,"support"}) or OVR then
                ...
                if ATM.hasGroup({user_id,"mod"}) or OVR then
                ...
                if ATM.hasGroup({user_id,"admin"}) or OVR then
                ...
                if ATM.hasGroup({user_id,"superadmin"}) or OVR then
                ...
                if ATM.hasGroup({user_id,"recruiter"}) or OVR then
            ...
            if CONFIG.ADD_VANITY_TITLES then
                ...
                if (ATM.getMoney({user_id}) >= (2.5*10^5)) or OVR then
                ...
                if (ATM.getMoney({user_id}) >= (10*10^6)) or OVR then
                ...
                if (ATM.getMoney({user_id}) >= (1*10^9)) or OVR then
...
ATM.registerMenuBuilder({"main", function(add, data)
    ...
    if user_id ~= nil then
        ...
        choices["Player List"] = {function(player,choice)
            ATM.buildMenu({"playerlist", {player = player}, function(menu)
                ...
                if ATM.hasPermission({user_id,"playerlist.hide"}) or isAdminAccount(user_id) or isStaff(user_id) then
                ...
                if ATM.hasPermission({user_id,"playerlist.icon"}) or isAdminAccount(user_id) or isStaff(user_id) then
                    menu["@Change User Icon"] = {function(p) setIcon(p,"prefix") end,"[Icon] Change a users Icon."}
                end
                if ATM.hasPermission({user_id,"playerlist.override"}) or isAdminAccount(user_id) then
