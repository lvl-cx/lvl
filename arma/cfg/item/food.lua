...
local function gen(ftype, vary_hunger, vary_thirst)
  local fgen = function(args)
    ...
    local name = ARMA.getItemName(idname)
    ...
    choices[act] = {function(player,choice)
      ...
      if user_id ~= nil then
        if ARMA.tryGetInventoryItem(user_id,idname,1,false) then
          if vary_hunger ~= 0 then ARMA.varyHunger(user_id,vary_hunger) end
          if vary_thirst ~= 0 then ARMA.varyThirst(user_id,vary_thirst) end

          if ftype == "drink" then
            ARMAclient.notify(player,{"~g~ Drinking "..name.."."})
            play_drink(player)
          elseif ftype == "eat" then
            ARMAclient.notify(player,{"~o~ Eating "..name.."."})
            play_eat(player)
          end
