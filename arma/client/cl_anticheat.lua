...
Citizen.CreateThread(function() 
    ...
    while true do
        ...
        oldPos = newPos
        if dist > 6 and not IsPedFalling(playerPed) and not IsPedInParachuteFreeFall(playerPed) and not IsPedRagdoll(playerPed) and not tARMA.isPlayerHidingInBoot() and not tARMA.isBeingDragged() and not noclipActive then
...
Citizen.CreateThread(function()
	Wait(10000)
	while true do
		if GetPlayerInvincible(PlayerId()) and not isInGreenzone and not tARMA.isInsideLsCustoms() and not tARMA.isStaffedOn() and not noclipActive and not tARMA.isInComa() then
