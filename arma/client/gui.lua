...
-- pause
AddEventHandler("ARMA:pauseChange", function(paused)
  SendNUIMessage({act="pause_change", paused=paused})
end)

-- MENU

function tARMA.openMenuData(menudata)
  SendNUIMessage({act="open_menu", menudata = menudata})
end

function tARMA.closeMenu()
  SendNUIMessage({act="close_menu"})
end

-- PROMPT

function tARMA.prompt(title,default_text)
...
function tARMA.request(id,text,time)
...
RegisterNUICallback("menu",function(data,cb)
  if data.act == "close" then
    ARMAserver.closeMenu({data.id})
  elseif data.act == "valid" then
    ARMAserver.validMenuChoice({data.id,data.choice,data.mod})
  end
end)
...
RegisterNUICallback("prompt",function(data,cb)
  if data.act == "close" then
    SetNuiFocus(false)
    SetNuiFocus(false)
    ARMAserver.promptResult({data.result})
  end
end)
...
RegisterNUICallback("request",function(data,cb)
  if data.act == "response" then
    ARMAserver.requestResult({data.id,data.ok})
  end
end)
...
function tARMA.announce(background,content)
...
-- try to fix missing cfg issue (cf: https://github.com/ImagicTheCat/ARMA/issues/89)
for i=1,5 do
...
function tARMA.setProgressBar(name,anchor,text,r,g,b,value)
...
function tARMA.setProgressBarValue(name,value)
  SendNUIMessage({act="set_pbar_val", name = name, value = value})
end

-- set progress bar text
function tARMA.setProgressBarText(name,text)
  SendNUIMessage({act="set_pbar_text", name = name, text = text})
end

-- remove a progress bar
function tARMA.removeProgressBar(name)
...
Citizen.CreateThread(function()
  while true do
    ...
    if pause_menu and not paused then
      paused = true
      TriggerEvent("ARMA:pauseChange", paused)
