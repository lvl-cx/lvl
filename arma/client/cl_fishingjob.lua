...
AddEventHandler("ARMA:onClientSpawn",function(q, r)
    if r then
        local s = {
            {
...
RegisterNetEvent("ARMA:globalFishingOnDuty",function(w)
...
AddEventHandler("ARMA:onClientSpawn",function(q, r)
    if r then
        ...
        ontick_fishing = function(y)
            if globalFishingOnDuty then
                if IsControlJustPressed(0, 51) and not g and not IsPedSwimming(PlayerPedId()) and not IsPedFalling(PlayerPedId()) then
                    TriggerServerEvent("ARMA:fishingJobChecks", y.type)
                end
        ...
        for t, A in pairs(o) do
            if o[t].type == "legal" then
                tARMA.createArea("fish_" .. t,A.coords,70,10,enter_fishing,exit_fishing,ontick_fishing,{type = o[t].type})
            else
                tARMA.createArea("fish_" .. t,A.coords,70,10,enter_fishing,exit_fishing,ontick_fishing,{type = o[t].type})
            end
        end
...
RegisterNetEvent("ARMA:fishingJobBarParams",function(B, C, D)
    e = C
    f = D
    while g do
        if c then
...
function func_fishingStatus(y)
    ...
    elseif g and c then
        if IsControlJustPressed(0, 102) then
            ...
            if d >= e and d <= f then
                TriggerServerEvent("ARMA:fishingCaughtFish", d)
            else
                TriggerServerEvent("ARMA:fishingCaughtFish", d)
...
tARMA.createThreadOnTick(func_fishingStatus)
RegisterNetEvent("ARMA:fishingJobSetCasted",function()
    g = true
...
tARMA.createThreadOnTick(func_createBar)
