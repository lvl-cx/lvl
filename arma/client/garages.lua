...
RMenu.Add('ARMAGarages', 'buy_vehicles',  RageUI.CreateSubMenu(RMenu:Get("ARMAGarages", "main")))
RMenu.Add('ARMAGarages', 'buy_vehicles_submenu',  RageUI.CreateSubMenu(RMenu:Get("ARMAGarages", "buy_vehicles")))
RMenu.Add('ARMAGarages', 'buy_vehicles_submenu_manage',  RageUI.CreateSubMenu(RMenu:Get("ARMAGarages", "buy_vehicles_submenu")))
...
-- Did you know you can toggle most things in ARMA within the arma/sharedcfg/options.lua?
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('ARMAGarages', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            DeleteCar(veh)
            RageUI.Button("Owned Vehicles", nil, {RightLabel = '~w~→'}, true, function(Hovered, Active, Selected) 
                if Selected then 
                    if Table_Type == nil or not Table_Type then 
                        Table_Type = true;
                        TriggerServerEvent('ARMA:FetchCars', true, garage_type)
                    end
                end
            end, RMenu:Get("ARMAGarages", "owned_vehicles"))
            RageUI.Button("Rented Vehicles", nil, {RightLabel = '~w~→'}, true, function(Hovered, Active, Selected) end, RMenu:Get("ARMAGarages", "rented_vehicles"))
            RageUI.Button("Store Vehicle", nil, {RightLabel = '~w~→'}, true, function(Hovered, Active, Selected) 
                if Selected then 
    ...
    if RageUI.Visible(RMenu:Get('ARMAGarages', 'buy_vehicles')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            DeleteCar(veh)
            for i,v in pairs(VehiclesFetchedTable) do 
                if garage_type == VehiclesFetchedTable[i].config.vtype then 
                    ...
                    end, RMenu:Get("ARMAGarages", "buy_vehicles_submenu"))
                end
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('ARMAGarages', 'buy_vehicles_submenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for i,v in pairs(selected_category) do 
                RageUI.Button(v[1], nil, {RightLabel = '~w~→'}, true, function(Hovered, Active, Selected)
                    if Selected then 
                        SelectedCar.spawncode = i 
                        SelectedCar.name = v[1]
                        RMenu:Get('ARMAGarages', 'buy_vehicles_submenu_manage'):SetSubtitle("~w~" .. v[1] .. ' Price: $' .. v[2])
                    end
                    if Active then 
                        Hovered_Vehicles = i
                    end
                end,RMenu:Get("ARMAGarages", "buy_vehicles_submenu_manage")) 
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('ARMAGarages', 'buy_vehicles_submenu_manage')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            RageUI.Button("Buy Vehicle", "", {}, true, function(Hovered, Active, Selected)
                if Selected then 
                    TriggerServerEvent('ARMA:BuyVehicle', SelectedCar.spawncode)
                ...
                if RentedVeh then 
                    RageUI.Button(v[1], v[2] .. " until the vehicle is returned.", {}, true, function(Hovered, Active, Selected)
                        if Selected then 
                            SelectedCar.spawncode = i 
                            SelectedCar.name = v[1]
                            RMenu:Get('ARMAGarages', 'owned_vehicles_submenu_manage'):SetSubtitle("~w~Options for " .. v[1])
                        end
                        if Active then 
                            Hovered_Vehicles = i
                        end
    ...
    if RageUI.Visible(RMenu:Get('ARMAGarages', 'owned_vehicles_submenu_manage')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            if not RentedVeh then 
                ...
                RageUI.Button('Rent out Vehicle', nil, {RightLabel = '~w~→'}, true, function(Hovered, Active, Selected)
                    if Selected then
                ...
                RageUI.Button('Sell Vehicle', nil, {RightLabel = '~w~→'}, true, function(Hovered, Active, Selected)
                    if Selected then 
                ...
                RageUI.Button('~r~[Scrap Vehicle]', nil, {RightLabel = '~r~→'}, true, function(Hovered, Active, Selected)end,RMenu:Get("ARMAGarages", "scrap_vehicle_confirmation"))
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('ARMAGarages', 'scrap_vehicle_confirmation')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Button('No', nil, {}, true, function(Hovered, Active, Selected)end,RMenu:Get("ARMAGarages", "owned_vehicles_submenu_manage"))
        end)
    end
    if RageUI.Visible(RMenu:Get('ARMAGarages', 'rented_vehicles')) then 
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            DeleteCar(veh)
            RageUI.Button('Rented Vehicles Out', nil, {RightLabel = '~w~→'}, true, function(Hovered, Active, Selected)
                if Selected then
                    Table_Type = nil;
                    TriggerServerEvent('ARMA:FetchVehiclesOut')
                end
            end,RMenu:Get("ARMAGarages", "rented_vehicles_out_manage"))
            RageUI.Button('Rented Vehicles In', nil, {RightLabel = '~w~→'}, true, function(Hovered, Active, Selected)
                if Selected then
                    Table_Type = nil;
                    RentedVeh = true;
    ...
    while true do 
        ...
        if not MenuOpen and inMarker then
            if garage_type == "VIP" then 
                MenuOpen = true
                TriggerServerEvent("ARMA:HasVIP")
...
RegisterNetEvent("ARMA:OpenVIPGarage")
AddEventHandler("ARMA:OpenVIPGarage", function()
    RageUI.Visible(RMenu:Get('ARMAGarages', 'main'), true)  
end)
