local a = false
local b = module("cfg/cfg_battleroyale")
...
tARMA.registerEventMenuItem("Battle Royal","ARMA Battlegrounds",function()
    ...
    if not currentEvent.isActive then
        RageUI.ButtonWithStyle("Start Event",nil,{RightLabel = "→→→"},true,function(i, j, k)
            if k then
                e = c.location
                TriggerServerEvent("ARMA:startEvent","Battle Royal","ARMA Battlegrounds",5,50,{locationIndex = c.location, name = d[c.location]})
            end
        end)
    end
end)
local function m()
    if a then
        ...
        DrawAdvancedText(0.591, 0.042, 0.005, 0.0028, 0.993, "ARMA BATTLEGROUNDS", 255, 255, 255, 255, 6, 0)
    end
end
...
local function n()
    if g.player.isInWinnerScreen then
        ...
        DrawAdvancedText(0.582,0.905,0.005,0.0028,0.96599999999999,"#1 " .. tARMA.getPlayerName(PlayerId()),255,255,255,255,6,0)
...
tARMA.createThreadOnTick(func_handleClicks)
RegisterNetEvent("ARMA:syncLootboxesTable",function(t, u)
    g.data.lootBoxes = table.copy(b.lootBoxes[u])
    g.data.armourPlates = table.copy(b.armourPlates[u])
    tARMA.loadModel("prop_box_ammo03a")
        ...
        if #(w.coords - g.gas.coords) < g.gas.radius / 2 then
            ...
            local x = function(y)
                if g.data.lootBoxes[y.box] then
                    ...
                    print(string.format("[ARMA] lootbox with ID: %s is nil", y.box))
                end
            end
            local z = function(y)
                if g.data.lootBoxes[y.box] then
            ...
            tARMA.createArea("armabr_lootbox_" .. v, w.coords, 200.0, 6, x, z, A, {box = v})
        end
    end
        ...
        if #(w.coords - g.gas.coords) < g.gas.radius / 2 then
            ...
            local B = function(C)
                if g.data.armourPlates[C.plateId] then
                    ...
                    print(string.format("[ARMA Events] body armour with ID of %s is nil in table (onEnter)", C.plateId))
                end
            end
            local D = function(C)
                if g.data.armourPlates[C.plateId] then
                    ...
                    print(string.format("[ARMA Events] body armour with ID of %s is nil in table (onLeave)", C.plateId))
                end
            end
            local E = function(C)
                if g.data.armourPlates[C.plateId] then
                    if C.distance <= 1.5 then
                        ...
                        tARMA.DrawText3D(vector3(F.x, F.y, F.z - 0.5), "Press [E] to pickup armour.", 0.2)
                        if IsControlJustPressed(0, 51) then
                            TriggerServerEvent("ARMA:removeArmourPlate", C.plateId)
                            SetPedComponentVariation(PlayerPedId(), 9, 15, 3, 0)
                        end
                    end
                end
            end
            tARMA.createArea("armabr_armour_" .. v, w.coords, 200.0, 6, B, D, E, {plateId = v})
            g.data.armourPlates[v].areaId = "armabr_armour_" .. v
        end
    end
...
RegisterNetEvent("ARMA:startBattlegrounds",function(I)
    e = I
    tARMA.startGas(b.locations[e].gas.initalRadius, b.locations[e].gas.centre)
    ...
    tARMA.stopEventSequence(true)
    tARMA.giveWeapons({["GADGET_PARACHUTE"] = {2}})
    SetEntityCoords(PlayerPedId(), b.locations[e].planeStart)
    tARMA.loadModel("titan")
    for o, p in pairs(GetActivePlayers()) do
        if p ~= PlayerId() then
    ...
    local J = tARMA.spawnVehicle("titan", b.locations[e].planeStart, b.locations[e].planeHeading, false, false, true)
    ...
    tARMA.loadModel("s_m_m_pilot_02")
    ...
    TaskVehicleDriveToCoord(K,J,b.locations[e].planeEnd + vector3(0.0, 0.0, 500.0),60.0,0,"titan",262144,15.0,-1.0)
    SetEntityVisible(PlayerPedId(), false)
    setCursor(1)
    g.data.plane = J
    a = not a
    inGUIARMA = not inGUIARMA
    Wait(1500)
    DoScreenFadeIn(1500)
    Citizen.CreateThread(function()
...
RegisterNetEvent("ARMA:winBattleRoyale",function()
    globalHideUi = true
    TriggerEvent("ARMA:showHUD", false)
    TriggerEvent("chat:clear")
...
function tARMA.changeGasRadius(M)
    ...
    Citizen.CreateThread(function()
        while g.gas.isActive and g.gas.radius > tonumber(M) and currentEvent.isActive do
            ...
            RemoveBlip(g.gas.blip)
            g.gas.blip = AddBlipForRadius(g.gas.coords, g.gas.radius / 2)
            SetBlipColour(g.gas.blip, 1)
            SetBlipAlpha(g.gas.blip, 155)
            Wait(0)
        end
...
tARMA.createThreadOnTick(function()
        ...
        if g.gas.isActive then
            if g.gas.timeUntilNext.seconds > 0 then
                ...
                if g.gas.timeUntilNext.seconds == 0 and g.gas.timeUntilNext.minutes == 0 then
                    tARMA.changeGasRadius(g.gas.radius - 400)
                else
...
local function Q()
    local R = sortedKeys(g.players,function(S, T) return g.players[S].kills > g.players[T].kills end)
    for h = 1, 3 do
...
RegisterNetEvent("ARMA:addBRKill",function(p, U)
    if g.players[p] then
...
RegisterNetEvent("ARMA:removePlayerFromBR",function(V)
    if g.players[V] then
...
Citizen.CreateThread(function()
    while true do
        if g.gas.isActive and g.player.hasJumped and not IsPedInParachuteFreeFall(PlayerPedId()) and GetPedParachuteState(PlayerPedId()) <= 2 and currentEvent.isActive and not tARMA.isSpectatingEvent() then
            if #(tARMA.getPlayerCoords().xy - g.gas.coords.xy) > g.gas.radius / 2.0 then
