...
RageUI.CreateWhile(1.0,RMenu:Get('dvsa','main'),nil,function()
    RageUI.IsVisible(RMenu:Get('dvsa','main'),true,false,true,function()
        ...
        RageUI.ButtonWithStyle("DVSA Alerts","View alerts received from the DVSA",{RightBadge=RageUI.BadgeStyle.Car},true,function(a8,a9,aa)
        end,RMenu:Get('dvsa','alerts'))
        if g then 
            if not currentTest.active and not a.full and a.active then 
                if not currentTest.requested then 
                    RageUI.ButtonWithStyle("Begin driving test","Begin your driving test",{RightBadge=RageUI.BadgeStyle.Alert,RightLabel="£"..getMoneyStringFormatted(i.test.price)},true,function(a8,a9,aa)
                        if aa then 
                            if a.banned then 
                    ...
                    RageUI.ButtonWithStyle("No tests available","Try again in a few minutes",{RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                        if aa then 
                ...
                if not currentTest.requested and not currentTest.active and a.full and a.active then 
                    RageUI.ButtonWithStyle("Surrender your licence","Surrender your driving licence to the DVSA",{RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                        if aa then 
                            a.full=false
                            a.active=false
    ...
    RageUI.IsVisible(RMenu:Get('dvsa','licence'),true,false,true,function(ab)
        if a.full then 
            ...
            if a.banned then 
                RageUI.ButtonWithStyle("Licence Type:","This indiates if you hold a full licence",{RightLabel="Suspended",RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                    if aa then 
                        tARMA.notifyPicture(i.images.dict,i.images.govLarge,"Your licence has been suspended","UK Government","DVSA")
                    end 
                end)
                ...
                if a.active then 
                    RageUI.ButtonWithStyle("Licence Type:","This indiates if you hold a full licence",{RightLabel="Provisional",RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                        if aa then 
                    ...
                    RageUI.ButtonWithStyle("Licence Type:","This indiates if you hold a full licence",{RightLabel="No licence",RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                        if aa then 
                            tARMA.notifyPicture(i.images.dict,i.images.govLarge,"You do not hold a UK Driving licence.","UK Government","DVSA")
                        end 
                    end)
                    if not f then 
                        RageUI.ButtonWithStyle("Apply for a provisional licence","Apply for a provisional licence",{RightBadge=RageUI.BadgeStyle.Car},true,function(a8,a9,aa)
                            if aa then 
        ...
        if a.active then 
            ...
            if table.count(b)<0 then 
                ...
                for r,s in pairs(b)do 
                    RageUI.ButtonWithStyle(s.offence,"Date: "..s.date.." | Type: "..s.type,{RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                        if aa then 
    ...
    RageUI.IsVisible(RMenu:Get('dvsa','tests'),true,false,true,function(ab)
        if c~=nil and table.count(c)>0 then 
            for r,s in pairs(c)do 
                ...
                RageUI.ButtonWithStyle(s.date.." | Result: "..ac,"~r~Serious Faults~w~: "..s.serious.." | ~b~Minor Faults~w~: "..s.minor,{RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                    if aa then 
    ...
    RageUI.IsVisible(RMenu:Get('dvsa','alerts'),true,false,true,function(ab)
        for r,s in pairs(d)do 
            ...
            RageUI.ButtonWithStyle(s.title.." "..s.date,"Press to read this message",{RightBadge=RageUI.BadgeStyle.Alert},true,function(a8,a9,aa)
                if aa then 
