...
function ARMA.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  ARMAclient.openMenuData(source,{menudata})
end

-- force close player menu
function ARMA.closeMenu(source)
  ARMAclient.closeMenu(source,{})
end
...
function ARMA.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  ARMAclient.prompt(source,{title,default_text})
end
...
function ARMA.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  ARMAclient.request(source,{id,text,time}) -- send request to client
...
function ARMA.registerMenuBuilder(name, builder)
...
function ARMA.buildMenu(name, data, cbr)
...
function ARMA.openMainMenu(source)
  ARMA.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    ARMA.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tARMA.closeMenu(id)
...
function tARMA.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tARMA.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tARMA.requestResult(id,ok)
...
function tARMA.openMainMenu()
  ARMA.openMainMenu(source)
end
...
function ARMA.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for ARMA.addStaticMenuChoices calls
...
AddEventHandler("ARMA:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    ARMAclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
