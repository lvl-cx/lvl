...
local function atm_choice_deposit(player, choice)
    ...
    ARMA.prompt(source, lang.atm.deposit.prompt(), "", function(player, v)
        ...
        if v > 0 then
            ...
            if user_id ~= nil then
                if ARMA.tryDeposit(user_id, v) then
                    ARMAclient.notify(source, {lang.atm.deposit.deposited({v})})
                else
...
local function atm_choice_withdraw(player, choice)
    ...
    ARMA.prompt(source, lang.atm.withdraw.prompt(), "", function(player, v)
        ...
        if v > 0 then
            local user_id = ARMA.getUserId(source)
            if user_id ~= nil then
                if ARMA.tryWithdraw(user_id, v) then
                    ARMAclient.notify(source, {lang.atm.withdraw.withdrawn({v})})
                else
                    ARMAclient.notify(source, {lang.atm.withdraw.not_enough()})
                end
            end
...
local function atm_enter()
    ...
    if user_id ~= nil then
        ...
        end, lang.atm.info.bank({ARMA.getBankMoney(user_id)})}
        ARMA.openMenu(source, atm_menu)
    end
end
...
local function build_client_atms(source)
    ...
    if user_id ~= nil then
        for k, v in pairs(atms) do
            ...
            ARMAclient.addBlip(source, {x, y, z, 108, 4, lang.atm.title()})
            ARMAclient.addMarker(source, {x, y, z - 1, 0.7, 0.7, 0.5, 0, 255, 125, 125, 150})

            ARMA.setArea(source, "ARMA:atm" .. k, x, y, z, 1, 1.5, atm_enter, atm_leave)
        end
    end
end