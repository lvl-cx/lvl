...
RegisterNetEvent("ARMA:attemptBeginDeliverooJob")
AddEventHandler("ARMA:attemptBeginDeliverooJob",function()
    local source=source
    local user_id=ARMA.getUserId(source)
    local deliverooJobTable={}
    if ARMA.hasGroup(user_id,"Deliveroo")then
        if not usersInDeliverooJob[user_id] then
            ...
            TriggerClientEvent("ARMA:beginDeliverooJob",source)
            Wait(100)
            TriggerClientEvent("ARMA:deliverooJobSetNextBlip",source,deliverooStops[deliverooJobTable.stopNumber].stopPosition)
        else
            ARMAclient.notify(source,{"~r~You are already a Deliveroo Driver."})
        end
    else
        ARMAclient.notify(source,{"~r~You are not clocked on as a Deliveroo Driver."})
    end
end)
                    ...
                    if ARMA.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(ARMA.getUserSource(k)))-deliverooStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            ...
                            if v.currentJob.stopNumber==#deliverooStops then
                                ...
                                TriggerClientEvent("ARMA:deliverooJobSetNextBlip",ARMA.getUserSource(k),deliverooStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
