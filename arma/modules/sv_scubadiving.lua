...
AddEventHandler("ARMA:requestScubaJob", function()
    local source=source
    local user_id=ARMA.getUserId(source)
    if ARMA.hasGroup(user_id,"Scuba Diver")then
        if currentDivers[user_id]==nil then
            ...
            for k,v in pairs(a.locations)do
                if v.name==scubaLocations[randomJob] then
                    ...
                    TriggerClientEvent("ARMA:createScubaJob",source,jobInfo,ARMA.getUserId(source))
                end
            end
        end
    else
        ARMAclient.notify(source,{"~r~You are not clocked on as a Scuba Diver."})
    end
end)

RegisterNetEvent("ARMA:scubaSetVehicle")
AddEventHandler("ARMA:scubaSetVehicle", function(PermID,Vehicle)
    local source = source
    local user_id = ARMA.getUserId(source)
...
RegisterNetEvent("ARMA:finishScubaJob")
AddEventHandler("ARMA:finishScubaJob", function(PermID,CollectedPackages)
    local source = source
    local user_id = ARMA.getUserId(source)
    if PermID == user_id then
        if currentDivers[PermID]~=nil and currentDivers[PermID].currentJob.jobActive==true then
...
RegisterNetEvent("ARMA:claimScubaReward")
AddEventHandler("ARMA:claimScubaReward", function(PermID)
    local source = source
    local user_id = ARMA.getUserId(source)
    if PermID == user_id then
