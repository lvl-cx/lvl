...
function ARMA.getUserGroups(user_id)
...
function ARMA.addUserGroup(user_id, group)
    if not ARMA.hasGroup(user_id, group) then
        local user_groups = ARMA.getUserGroups(user_id)
        ...
        if ngroup then
            ...
            TriggerEvent("ARMA:playerJoinGroup", user_id, group, gtype)
        end
    end
end
...
function ARMA.getUserGroupByType(user_id, gtype)
    local user_groups = ARMA.getUserGroups(user_id)
    for k, v in pairs(user_groups) do
        local kgroup = groups[k]
        if kgroup then
...
function ARMA.getUsersByGroup(group)
    local users = {}

    for k, v in pairs(ARMA.rusers) do
        if ARMA.hasGroup(tonumber(k), group) then
...
function ARMA.getUsersByPermission(perm)
    local users = {}

    for k, v in pairs(ARMA.rusers) do
        if ARMA.hasPermission(tonumber(k), perm) then
            table.insert(users, tonumber(k))
        end
    end
...
function ARMA.removeUserGroup(user_id, group)
    local user_groups = ARMA.getUserGroups(user_id)
    ...
    TriggerEvent("ARMA:playerLeaveGroup", user_id, group, gtype)

    user_groups[group] = nil -- remove reference
end

-- check if the user has a specific group
function ARMA.hasGroup(user_id, group)
    local user_groups = ARMA.getUserGroups(user_id)
...
-- check if the user has a specific permission
function ARMA.hasPermission(user_id, perm)
    local user_groups = ARMA.getUserGroups(user_id)
    ...
    if fchar == "@" then -- special aptitude permission
        ...
        if #parts == 3 then -- decompose group.aptitude.operator
            ...
            local aarma = math.floor(ARMA.expToLevel(ARMA.getExp(user_id, group, aptitude)))

            local fop = string.sub(op, 1, 1)
            if fop == "<" then -- less (group.aptitude.<x)
                local arma = parseInt(string.sub(op, 2, string.len(op)))
                if aarma < arma then
                    return true
                end
            elseif fop == ">" then -- greater (group.aptitude.>x)
                ...
                local arma = parseInt(string.sub(op, 1, string.len(op)))
                if aarma == arma then
                    return true
                end
            end
    ...
    elseif fchar == "#" then -- special item permission
        ...
        if #parts == 2 then -- decompose item.operator
            ...
            local amount = ARMA.getInventoryItemAmount(user_id, item)
...
function ARMA.hasPermissions(user_id, perms)
...
local function ch_select(player, choice)
    ...
    if user_id ~= nil then
        ARMA.addUserGroup(user_id, choice)
        ...
        for k, v in pairs(selectors) do
            ...
            if gcfg and menu then
                ...
                local function selector_enter()
                    local user_id = ARMA.getUserId(source)
                    if user_id ~= nil and ARMA.hasPermissions(user_id, gcfg.permissions or {}) then
                ...
                ARMAclient.addBlip(source, {x, y, z, gcfg.blipid, gcfg.blipcolor, k})
                ARMAclient.addMarker(source, {x, y, z - 1, 0.7, 0.7, 0.5, 255, 154, 24, 125, 150})

                ARMA.setArea(source, "ARMA:gselector:" .. k, x, y, z, 1, 1.5, selector_enter, selector_leave)
            end
        end
    end
end
    ...
    if first_spawn then
        ...
        if user ~= nil then
            for k, v in pairs(user) do
                ARMA.addUserGroup(user_id, v)
            end
        end

        -- add default group user
        ARMA.addUserGroup(user_id, "user")
    end
