...
local function tr_remove_player(tr,player) -- remove player from transforming
  ...
  ARMAclient.removeProgressBar(player,{"ARMA:tr:"..tr.name})
...
local function tr_add_player(tr,player,recipe) -- add player to transforming
  ...
  ARMA.closeMenu(player)
  ARMAclient.setProgressBar(player,{"ARMA:tr:"..tr.name,"center",recipe.."...",tr.itemtr.r,tr.itemtr.g,tr.itemtr.b,0})
...
local function tr_tick(tr) -- do transformer tick
  for k,v in pairs(tr.players) do
    local user_id = ARMA.getUserId(tonumber(k))
    local player = ARMA.getUserSource(tonumber(user_id))
    if v and user_id ~= nil then -- for each player transforming
      
     ARMAclient.getPosition(player,{},function(x,y,z)
	    ARMAclient.getDistanceBetweenCoords(player,{x,y,z,tr.itemtr.x,tr.itemtr.y,tr.itemtr.z},function(distance)
      ...
      if tr.units > 0 and recipe then -- check units
        ...
        for l,w in pairs(recipe.reagents) do
          reagents_ok = reagents_ok and (ARMA.getInventoryItemAmount(user_id,l) >= w)
        end

        -- check money
        local money_ok = (ARMA.getMoney(user_id) >= recipe.in_money)
        ...
        local new_weight = ARMA.getInventoryWeight(user_id)+ARMA.computeItemsWeight(out_witems)-ARMA.computeItemsWeight(in_witems)

        local inventory_ok = true
        if new_weight > ARMA.getInventoryMaxWeight(user_id) then
          inventory_ok = false
          ARMAclient.notify(tonumber(k), {lang.inventory.full()})
        end
        ...
        if money_ok and reagents_ok and inventory_ok then -- do transformation
          ...
          -- consume reagents
          if recipe.in_money > 0 then ARMA.tryPayment(user_id,recipe.in_money) end
          for l,w in pairs(recipe.reagents) do
            ARMA.tryGetInventoryItem(user_id,l,w,true)
          end

          -- produce products
          if recipe.out_money > 0 then ARMA.giveMoney(user_id,recipe.out_money) end
          for l,w in pairs(recipe.products) do
            ARMA.giveInventoryItem(user_id,l,w,true)
          end
          ...
          for l,w in pairs(recipe.aptitudes or {}) do
            ...
            if #parts == 2 then
              ARMA.varyExp(user_id,parts[1],parts[2],w)
            end
          end
  ...
  for k,v in pairs(tr.players) do
    ARMAclient.setProgressBarValue(k,{"ARMA:tr:"..tr.name,math.floor(tr.units/tr.itemtr.max_units*100.0)})
    
    if tr.units > 0 then -- display units left
      ARMAclient.setProgressBarText(k,{"ARMA:tr:"..tr.name,v.."... "..tr.units.."/"..tr.itemtr.max_units})
    else
      ARMAclient.setProgressBarText(k,{"ARMA:tr:"..tr.name,"empty"})
    end
  end
end

local function bind_tr_area(player,tr) -- add tr area to client
  ARMA.setArea(player,"ARMA:tr:"..tr.name,tr.itemtr.x,tr.itemtr.y,tr.itemtr.z,tr.itemtr.radius,tr.itemtr.height,tr.enter,tr.leave)
end

local function unbind_tr_area(player,tr) -- remove tr area from client
  ARMA.removeArea(player,"ARMA:tr:"..tr.name)
end
...
function ARMA.setItemTransformer(name,itemtr)
  ARMA.removeItemTransformer(name) -- remove pre-existing transformer
  ...
  tr.enter = function(player,area)
    ...
    if user_id ~= nil and ARMA.hasPermissions(user_id,itemtr.permissions or {}) then
      ARMA.openMenu(player, tr.menu) -- open menu
    end
  end
...
function ARMA.removeItemTransformer(name)
...
SetTimeout(5000,function()
  ...
  for k,v in pairs(cfg.item_transformers) do
    ARMA.setItemTransformer("cfg:"..k,v)
  end
end)
...
local function hidden_placement_tick()
  ARMA.getSData("ARMA:hidden_trs", function(data)
    ...
    for k,v in pairs(cfg.hidden_transformers) do
      ...
      if tonumber(os.time())-htr.timestamp >= cfg.hidden_transformer_duration*60 then
        ...
        ARMA.removeItemTransformer("cfg:"..k)
      ...
      if transformers["cfg:"..k] == nil then
        ...
        ARMA.setItemTransformer("cfg:"..k, v.def)
      end
    end

    ARMA.setSData("ARMA:hidden_trs",json.encode(hidden_trs)) -- save hidden transformers
  end)
...
local function ch_informer_buy(player,choice)
  ...
  if user_id ~= nil and tr ~= nil then
    if ARMA.tryPayment(user_id, price) then
      ARMAclient.setGPS(player, {tr.itemtr.x,tr.itemtr.y}) -- set gps marker
      ARMAclient.notify(player, {lang.money.paid({price})})
      ARMAclient.notify(player, {lang.itemtr.informer.bought()})
    else
...
local function informer_enter()
  ...
  if user_id ~= nil then
    ARMA.openMenu(source,informer_menu) 
  end
end
...
local function informer_placement_tick()
  ...
  for k,v in pairs(ARMA.rusers) do
    ...
    ARMAclient.setNamedBlip(player,{"ARMA:informer",x,y,z,cfg.informer.blipid,cfg.informer.blipcolor,lang.itemtr.informer.title()})
    ARMAclient.setNamedMarker(player,{"ARMA:informer",x,y,z-1,0.7,0.7,0.5,0,255,125,125,150})
    ARMA.setArea(player,"ARMA:informer",x,y,z,1,1.5,informer_enter,informer_leave)
  end
  ...
  SetTimeout(cfg.informer.duration*60000, function()
    for k,v in pairs(ARMA.rusers) do
      local player = ARMA.getUserSource(tonumber(k))
      ARMAclient.removeNamedBlip(player,{"ARMA:informer"})
      ARMAclient.removeNamedMarker(player,{"ARMA:informer"})
      ARMA.removeArea(player,"ARMA:informer")
    end
  end)
