...
AddEventHandler("ARMA:playerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
        TriggerClientEvent('ARMA:policeSyncAllDoors', source, doors, doorStates)
    end
end)

RegisterNetEvent("ARMA:policeSetDoorState")
AddEventHandler("ARMA:policeSetDoorState", function(doorHash, state)
    local source = source
    local user_id = ARMA.getUserId(source)
    if ARMA.hasPermission(user_id, 'police.onduty.permission') then
        doorStates[doorHash] = state
        TriggerClientEvent('ARMA:policeSyncDoor', -1, doorHash, state)
    end
end)

RegisterNetEvent("ARMA:enterPoliceSyncDoors")
AddEventHandler("ARMA:enterPoliceSyncDoors", function()
    local source = source
    TriggerClientEvent('ARMA:gotAllDoorStates', source, doorStates)
end)