...
RegisterServerEvent('ARMACryptocore:getCryptoPrice', function()
    TriggerClientEvent('ARMACryptocore:setCryptoPrice', source, cryptoprice)
end)
...
RegisterServerEvent('ARMACrypto:Sell:System')
AddEventHandler('ARMACrypto:Sell:System', function(system)
    local source = source
    local user_id = ARMA.getUserId(source)
...
RegisterServerEvent("ARMACrypto:buy_crypto_system")
AddEventHandler("ARMACrypto:buy_crypto_system", function(system)
    local player = source
...
RegisterServerEvent("ARMACrypto:CRYPTO:ReceiveMiners")
AddEventHandler('ARMACrypto:CRYPTO:ReceiveMiners', function()
    local player = source
    local user_id = ARMA.getUserId(player)
    local miners = exports['ghmattimysql']:executeSync("SELECT * FROM c_cryptominers WHERE user_id = @user_id", {user_id = user_id})
    TriggerClientEvent('ARMACrypto:CRYPTO:SetMiners', player, miners)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        for i, p in pairs(GetPlayers()) do
            local user_id = ARMA.getUserId(p)
