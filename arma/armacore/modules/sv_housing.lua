local cfg = module("armacore/cfg/cfg_housing")
...
MySQL.createCommand("ARMA/rm_address","DELETE FROM arma_user_homes WHERE user_id = @user_id")
MySQL.createCommand("ARMA/set_address","REPLACE INTO arma_user_homes(user_id,home,number) VALUES(@user_id,@home,@number)")

--Functions (Thanks to DunkoARMA standard housing)

function ARMA.getUserAddress(user_id, cbr)
    local task = Task(cbr)
...
function ARMA.setUserAddress(user_id, home, number)
    MySQL.execute("ARMA/set_address", {user_id = user_id, home = home, number = number})
end
  
function ARMA.removeUserAddress(user_id)
    MySQL.execute("ARMA/rm_address", {user_id = user_id})
end

function ARMA.getUserByAddress(home, number, cbr)
    local task = Task(cbr)
...
function ARMA.leaveHome(user_id, home, number, cbr)
    local task = Task(cbr)
...
function ARMA.accessHome(user_id, home, number, cbr)
    local task = Task(cbr)
...
AddEventHandler("JudHousing:Buy", function(house)
    ...
    for k, v in pairs(cfg.homes) do
        if house == k then
            ARMA.getUserByAddress(house,1,function(noowner) --check if house already has a owner
                if noowner == nil then
                    ARMA.getUserAddress(user_id, function(address) -- check if user already has a home
                        if address == nil then --not already got a home
                            if ARMA.tryFullPayment(user_id,v.buy_price) then --try payment
                                ARMA.setUserAddress(user_id,house,1) --set address
                            ...
                            ARMAclient.notify(player,{"~r~You already own a home!"}) --already has a home
                        end
                    end)
                else
...
AddEventHandler('GrabHouseInfo', function(house)
    local source = source
    ARMA.getUserByAddress(house, 1, function(huser_id)
...
AddEventHandler("JudHousing:Enter", function(house)
    ...
    ARMA.getUserByAddress(house, 1, function(huser_id) --check if player owns home
        local hplayer = ARMA.getUserSource(huser_id) --temp id of home owner

        if huser_id ~= nil then
            if huser_id == user_id then
                ARMA.accessHome(user_id, house, 1, function(ok) --enter home
                    if not ok then
            ...
            ARMA.getUserAddress(user_id, function(address) -- check if user already has a home
                if address == nil then
                    ARMAclient.notify(player,{"~r~Nobody owns this "..house..", why dont you buy it?"}) --no home owner & user_id already doesn't have a house
                else
                    ARMAclient.notify(player,{"~r~Nobody owns "..house.."!"}) --already has a home, notify nobody owns it
                end
            end)
        end
    end)
...
AddEventHandler("JudHousing:Leave", function(house)
    ...
    ARMA.leaveHome(user_id, house, 1, function(ok) --leave home
        if not ok then
                                        ...
                                        ARMA.request(target,GetPlayerName(player).." wants to sell: " ..house.. " Price: £"..amount, 30, function(target,ok) --request new owner if they want to buy
                                            if ok then --bought
                                                ...
                                                if ARMA.tryFullPayment(buyer_id,amount) then
                                                    ARMA.setUserAddress(buyer_id, house, 1) --give house
                                                    ARMA.removeUserAddress(user_id) -- remove house
...
AddEventHandler("JudHousing:SaveOutfit", function(outfitName)
    ...
    ARMA.getUData(user_id, "ARMA:home:wardrobe", function(data)
        ...
        ARMAclient.getCustomization2(player,{},function(custom)
...
AddEventHandler("ARMA:playerSpawn",function(user_id, source, first_spawn)
    if first_spawn then
        for k, v in pairs(cfg.homes) do
            local x,y,z = table.unpack(v.entry_point)
            ARMA.getUserByAddress(k,1,function(owner)
                ...
                if owner == user_id then -- check if owner is user
                    ARMAclient.addBlip(source,{x,y,z,374,1,k}) -- add blip for owner of home, 374,1 red house symbol
