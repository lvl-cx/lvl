...
AddEventHandler('LargeArms:BuyArmour', function()
    ...
    if #(playerCoords - coords) <= 5.0 then 
        ...
        if ARMA.tryPayment(userid, 50000 + finalCommision) then
            SetPedArmour(source, 50)
            TriggerClientEvent("ARMA:PlaySound", source, 1)
            ARMAclient.notify(source, {"~g~Purchased Level 2 Armour. Paid £" .. tostring(50000 + finalCommision) .. " +" .. commision .. "% Commision!"})
