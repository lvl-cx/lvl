...
AddEventHandler('ARMA:TooFar', function(isnTurf)
	...
	if turfs[isnTurf] then
		...
		if(isPlayerInTurf[source]) then
			...
			TriggerClientEvent('chatMessage', -1, "^7[^1ARMA^7]:", {255, 255, 255}, "Turf capture failed at ^3" .. turf.nameofturf, "alert")
			...
			for I,L in pairs(array) do
				if tostring(user_id) == I then
					...
					local turf = turfs[isnTurf]
						...
						-- [Chat Message]
						TriggerClientEvent('chatMessage', -1, "^7[^1ARMA^7]:", {255, 255, 255}, "Turf capture started at ^3" .. turf.nameofturf, "alert")
					...
					ARMAclient.notify(source, {'~r~You must be in a Gang to capture a turf!'})
				end
			end
		end
	end)
