          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                TriggerServerEvent('ARMA:GatherCocaine')
                
                exports.rprogress:Start("", 5000)
                ClearPedTasksImmediately(ped)
                Action = false
              else
                ARMA.notify({"~r~You cant gather Cocaine while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TriggerServerEvent('ARMA:ProcessCocaine')
...
-- [RageUI Menu]
RMenu.Add('CocaineMenu', 'Cocaine Seller', RageUI.CreateMenu("", "ARMA Cocaine Seller",1300, 50, "cocaine", "cocaine"))

RageUI.CreateWhile(1.0, true, function()
  if RageUI.Visible(RMenu:Get('CocaineMenu', 'Cocaine Seller')) then
      RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
    
    RageUI.Button("Sell Cocaine" , nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected)
        if Selected then
