RMenu.Add('ARMAPDMenu', 'main', RageUI.CreateMenu("", "ARMA Police Menu", 1300, 50, 'police', 'police'))
...
AddEventHandler("ARMA:arrestCriminal",function(g)
        local h = PlayerPedId()
        ARMA.setWeapon(h, "WEAPON_GLOCKARMA", true)
        local i = GetEntityCoords(h)
        local j = GetPlayerPed(GetPlayerFromServerId(g))
        f = true
        ARMA.loadAnimDict("mp_arrest_paired")
...
AddEventHandler("ARMA:UnCuffAnim",function()
    ARMA.loadAnimDict("mp_arresting")
