...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('SandySmallArms', 'main')) then
        ...
        RageUI.Button("Level 1 Armour Plate ~b~[25%]" , nil, {RightLabel = "~g~£25,000"}, true, function(Hovered, Active, Selected)
            if Selected then
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("SandySmallArms", "confirm")) then
        ...
        RageUI.Button("~r~Decline" , nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected) end, RMenu:Get("SandySmallArms", "main"))
...
AddEventHandler("ARMA:PlaySound", function(status)
    if status == 1 then 
...
local clothingID = 4
local index = 0
RegisterNetEvent('ARMA:SetVest')
AddEventHandler('ARMA:SetVest', function()
    player = GetPlayerPed(-1)
    SetPedComponentVariation(player, 9, clothingID, index, 0) 
end)


RegisterNetEvent('ARMA:ChangeArmour2')
AddEventHandler('ARMA:ChangeArmour2', function(id, indexz)
  clothingID = id
  index = indexz
end)
