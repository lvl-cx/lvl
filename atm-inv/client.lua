...
tATM = Proxy.getInterface("ATM")

local LootBagCrouchLoop = false;
RegisterCommand('inventory', function()
    if not tATM.isInComa({}) then
        if not inventoryOpen then
            TriggerServerEvent('ATM:FetchPersonalInventory')
            inventoryOpen = true; 
            SetNuiFocus(true, true)
            SetNuiFocusKeepInput(true)
            SendNUIMessage({action = 'InventoryDisplay', showInv = true})
            local VehInRadius, VehType, NVeh = tATM.getNearestOwnedVehicle({3.5})
            if VehInRadius and IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then 
                ...
                tATM.vc_openDoor({VehTypeC, 5})
                inventoryType = 'CarBoot'
                
                TriggerServerEvent('ATM:FetchTrunkInventory', NVeh)
            end
            ...
            if IsLootBagOpening then
                ...
                TriggerServerEvent('ATM:CloseLootbag')
        ...
        tATM.notify({'~r~You cannot open your inventory while dead.'})
    end
end)
...
RegisterNetEvent('ATM:InventoryOpen')
AddEventHandler('ATM:InventoryOpen', function(toggle, lootbag)
...
RegisterNetEvent('ATM:ToggleNUIFocus')
AddEventHandler('ATM:ToggleNUIFocus', function(value)
    --print('focus', value)
    SetNuiFocus(value, value)
    SetNuiFocusKeepInput(value)
end)

RegisterNetEvent('ATM:SendSecondaryInventoryData')
AddEventHandler('ATM:SendSecondaryInventoryData', function(InventoryData, CurrentKG, MaxKg)
...
AddEventHandler('ATM:FetchPersonalInventory', function(table, CurrentKG, MaxKG)
...
RegisterNUICallback('UseBtn', function(data, cb)
    TriggerServerEvent('ATM:UseItem', data.itemId, data.invType)
    cb(true);
...
RegisterNUICallback('TrashBtn', function(data, cb)
    TriggerServerEvent('ATM:TrashItem', data.itemId, data.invType)
    cb(true);
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)

RegisterNUICallback('GiveBtn', function(data, cb)
    TriggerServerEvent('ATM:GiveItem', data.itemId, data.invType)
    cb(true)
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)
...
RegisterNUICallback('MoveBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('ATM:MoveItem', data.invType, data.itemId, VehTypeA)
        elseif inventoryType == "Housing" then
            TriggerServerEvent('ATM:MoveItem', data.invType, data.itemId, "home")
        end
    else 
        TriggerServerEvent('ATM:MoveItem', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNUICallback('MoveXBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('ATM:MoveItemX', data.invType, data.itemId, VehTypeA)
        elseif inventoryType == "Housing" then
            TriggerServerEvent('ATM:MoveItemX', data.invType, data.itemId, "home")
        end
    else 
        TriggerServerEvent('ATM:MoveItemX', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNUICallback('MoveAllBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            local nearestVeh2 = ATM.getNearestVehicle({3})
            TriggerServerEvent('ATM:MoveItemAll', data.invType, data.itemId, VehTypeA, NetworkGetNetworkIdFromEntity(nearestVeh2))
        elseif inventoryType == "Housing" then
            TriggerServerEvent('ATM:MoveItemAll', data.invType, data.itemId, "home")
        end
    else 
        TriggerServerEvent('ATM:MoveItemAll', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
AddEventHandler('ATM:IsInComa', function(coma)
    PlayerInComa = coma;
    if coma then 
        LootBagCoords = false;
        NearLootBag = false; 
...
--                 TriggerServerEvent('ATM:LootBag', LootBagIDNew)
--             end
--         end
--     end
-- end)
