    ...
    if #(playerCoords - boxCoords) < 2.0 then
        if not used then
                ...
                if lootrandom == 1 then -- [Legendary]
                    -- [Legendary]
                    Sentry.giveInventoryItem(user_id, "wbody|" .. 'WEAPON_MOSIN', 1, true)
                    Sentry.giveInventoryItem({user_id, '7.62 Bullets', 250, true})

                    Sentry.giveInventoryItem(user_id, "wbody|" .. 'WEAPON_HK45', 1, true)
                    Sentry.giveInventoryItem(user_id, '9mm Bullets', 250, true)
                    Sentry.giveInventoryItem(user_id, "body_armor", 3, true)
                    Sentryclient.notify(source,{'Received ~g~£200,000 ~w~Cash.'})
...
RegisterServerEvent('updateLoot', function(source, item, amount)
    ...
    if #currentLoot == 0 then
        if not used then
            ...
            TriggerClientEvent('Infinite:SendSecondaryInventoryData', source, currentLoot, Sentry.computeItemsWeight({currentLoot}), 30)
end) 
