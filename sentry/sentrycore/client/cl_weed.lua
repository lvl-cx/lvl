          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                TriggerServerEvent('Sentry:GatherWeed')
                
                exports.rprogress:Start("", 5000)
                ClearPedTasksImmediately(ped)
                Action = false
              else
                Sentry.notify({"~r~You cant gather Weed while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TriggerServerEvent('Sentry:ProcessWeed')
...
-- [RageUI Menu]
RMenu.Add('WeedMenu', 'Weed Seller', RageUI.CreateMenu("", "~g~Sentry Weed Seller",1300, 50, "weed", "weed"))

RageUI.CreateWhile(1.0, true, function()
  if RageUI.Visible(RMenu:Get('WeedMenu', 'Weed Seller')) then
      RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
    
    RageUI.Button("Sell Weed" , nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected)
        if Selected then
 
            TriggerServerEvent('Sentry:SellWeed')
        end
    end)
