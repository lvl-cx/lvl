...
RMenu.Add("CosmeticMenu", "main",  RageUI.CreateMenu("", "~g~Sentry Cosmetic Menu", 1300, 50, "cosmetics", "cosmetics"))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("CosmeticMenu", "main")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            RageUI.Button('Buy Cosmetics', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
            end, RMenu:Get("CosmeticMenu", "buycosmetics"))

            RageUI.Button('Your Cosmetics', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                if Selected then 
                    TriggerServerEvent('Sentry:GetCosmetic')
                end
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("CosmeticMenu", "buycosmetics")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            RageUI.Separator("Cosmetic Shop Day: ~g~1", function() end)
            RageUI.Button('Standard Items', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
            end, RMenu:Get("CosmeticMenu", "standard"))

            RageUI.Button('Limited Time Items', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("CosmeticMenu", "limited")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            RageUI.Separator("Cosmetic shop will reset: ~g~" .. 'Tommorow', function() end)
            for i,v in pairs(cosmetics.limitedshop) do 

                RageUI.Button(v.item, nil, {RightLabel = "~g~£" .. getMoneyStringFormatted(v.price)}, true, function(Hovered, Active, Selected)
                    if Selected then 
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("CosmeticMenu", "standard")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            RageUI.Separator("~g~These items will always be in the cosmetic shop", function() end)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for i,v in pairs(cosmetics.shop) do 
                if CosmeticItem == v.item then
                    RageUI.Separator("Cosmetic Name: ~g~" .. v.item, function() end)
                    RageUI.Separator("Cosmetic Price: ~g~£" .. getMoneyStringFormatted(v.price), function() end)
                    RageUI.Separator("~g~Are you sure you want to purchase this cosmetic?", function() end)

                    RageUI.Button('Confirm', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                        if Selected then 
                            TriggerServerEvent('Sentry:BuyCosmetic', v.item)
                        end
                    end, RMenu:Get("CosmeticMenu", "standard"))

                    RageUI.Button('Decline', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                        if Selected then 
    ...
    if RageUI.Visible(RMenu:Get("CosmeticMenu", "buycosmeticsublimit")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for i,v in pairs(cosmetics.limitedshop) do 
                if CosmeticItem == v.item then
                    RageUI.Separator("Cosmetic Name: ~g~" .. v.item, function() end)
                    RageUI.Separator("Cosmetic Price: ~g~£" .. v.price, function() end)
                    RageUI.Separator("~g~Are you sure you want to purchase this cosmetic?", function() end)
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for i,v in pairs(cosmetics.cfg) do
                if SelectedCosmetic == v.item then 
                    RageUI.Separator("Cosmetic Name: ~g~" .. v.item, function() end)
                    RageUI.Separator("Cosmetic Price: ~g~£" .. tostring(getMoneyStringFormatted(v.price)), function() end)
                    RageUI.Separator("Cosmetic Type: ~g~" .. v.type, function() end)
                    RageUI.Separator("Cosmetic Refund Price: ~g~£" .. tostring(getMoneyStringFormatted(v.price * 0.25)), function() end)
                    -- [Equip Cosmetic]
                            ...
                            if Selected then 
                                if not VestEquip then
                                    if GetPedArmour(PlayerPedId()) ~= 0 then 
                                        ...
                                        TriggerEvent('Sentry:ChangeArmour', v.clothingid, v.index)
                                        TriggerEvent('Sentry:ChangeArmour2', v.clothingid, v.index)
                                        VestEquip = true
                                    else
                    ...
                    -- [Other Options]
                    RageUI.Button('Sell Cosmetic to player', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                        if Selected then

                            TriggerServerEvent('Sentry:SellCosmeticToPlayer', v.item)

                        end
                    end)

                    RageUI.Button('Refund Cosmetic ~g~[25% Back]', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                    end, RMenu:Get("CosmeticMenu", "refundconfirm"))


                    RageUI.Button('List on Sentry Market Place', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 

                    end, RMenu:Get("CosmeticMenu", "marketplace"))

                    RageUI.Separator("~g~discord.gg/sentrygta - #market-place", function() end)
                end
            end
    end)            
end
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for i,v in pairs(cosmetics.cfg) do 
                if SelectedCosmetic == v.item then
                    ...
                    RageUI.Button('Enter Listing Price - ~g~£' .. listingprice, nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                        if Selected then 
                    ...
                    RageUI.Button('Enter Custom Message - ~g~' .. listingmessage, nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                        if Selected then 
                    ...
                    RageUI.Button('Confirm', 'Listing Price: ~g~£' .. listingprice .. '\n~w~Listing Message: ~g~' .. listingmessage .. '\n~w~Item Name: ~g~' .. v.item, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                        if Selected then 
                            if cooldown <= 0 then 
                                TriggerServerEvent('Sentry:CosmeticMarketPlace', v.item, tonumber(listingprice), listingmessage)
                                TriggerServerEvent('Sentry:GetCosmetic')
                                cooldown = 300
                            else
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for i,v in pairs(cosmetics.cfg) do 
                if SelectedCosmetic == v.item then
                    ...
                    RageUI.Separator("Cosmetic Type: ~g~" .. v.type, function() end)
                    RageUI.Separator("Cosmetic Refund Price: ~g~£" .. tostring(getMoneyStringFormatted(v.price * 0.25)), function() end)
                    RageUI.Separator("~g~Are you sure you want to refund this ?", function() end)

                    RageUI.Button('Confirm', nil, {RightLabel = "~g~→"}, true, function(Hovered, Active, Selected) 
                        if Selected then 
                            TriggerServerEvent('Sentry:RefundCosmetic', v.item)
                            TriggerServerEvent('Sentry:GetCosmetic')
                        end
                    end, RMenu:Get("CosmeticMenu", "cosmetics"))
...
-- [Functions/ Events]
RegisterNetEvent('Sentry:ResetCosmetics')
AddEventHandler('Sentry:ResetCosmetics', function(type2) -- [Remove All of Ped Cosmetics]
    if type2 == 'Vest' then 
        SetPedComponentVariation(PlayerPedId(), 9, 0, 0, 0) 
...
RegisterNetEvent('Sentry:ReturnCosmetic')
AddEventHandler('Sentry:ReturnCosmetic', function(rdata)
    data = rdata
end)
