          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              if not pedinveh then
                ...
                TriggerServerEvent('Sentry:GatherCocaine')
                
                exports.rprogress:Start("", 5000)
                ClearPedTasksImmediately(ped)
                Action = false
              else
                Sentry.notify({"~r~You cant gather Cocaine while Driving!"})
              end
            end
          end
        end)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              TriggerServerEvent('Sentry:ProcessCocaine')
...
-- [RageUI Menu]
RMenu.Add('CocaineMenu', 'Cocaine Seller', RageUI.CreateMenu("", "~g~Sentry Cocaine Seller",1300, 50, "cocaine", "cocaine"))

RageUI.CreateWhile(1.0, true, function()
  if RageUI.Visible(RMenu:Get('CocaineMenu', 'Cocaine Seller')) then
      RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
    
    RageUI.Button("Sell Cocaine" , nil, {}, true, function(Hovered, Active, Selected)
        if Selected then
 
            TriggerServerEvent('Sentry:SellCocaine')
        end
    end)
