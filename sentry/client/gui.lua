...
-- pause
AddEventHandler("Sentry:pauseChange", function(paused)
  SendNUIMessage({act="pause_change", paused=paused})
end)

-- MENU

function tSentry.openMenuData(menudata)
  SendNUIMessage({act="open_menu", menudata = menudata})
end

function tSentry.closeMenu()
  SendNUIMessage({act="close_menu"})
end

-- PROMPT

function tSentry.prompt(title,default_text)
...
function tSentry.request(id,text,time)
...
RegisterNUICallback("menu",function(data,cb)
  if data.act == "close" then
    Sentryserver.closeMenu({data.id})
  elseif data.act == "valid" then
    Sentryserver.validMenuChoice({data.id,data.choice,data.mod})
  end
end)
...
RegisterNUICallback("prompt",function(data,cb)
  if data.act == "close" then
    SetNuiFocus(false)
    SetNuiFocus(false)
    Sentryserver.promptResult({data.result})
  end
end)
...
RegisterNUICallback("request",function(data,cb)
  if data.act == "response" then
    Sentryserver.requestResult({data.id,data.ok})
  end
end)
...
function tSentry.announce(background,content)
...
-- try to fix missing cfg issue (cf: https://github.com/ImagicTheCat/Sentry/issues/89)
for i=1,5 do
...
function tSentry.setProgressBar(name,anchor,text,r,g,b,value)
...
function tSentry.setProgressBarValue(name,value)
  SendNUIMessage({act="set_pbar_val", name = name, value = value})
end

-- set progress bar text
function tSentry.setProgressBarText(name,text)
  SendNUIMessage({act="set_pbar_text", name = name, text = text})
end

-- remove a progress bar
function tSentry.removeProgressBar(name)
...
Citizen.CreateThread(function()
  while true do
    ...
    -- open general menu
    if IsControlJustPressed(table.unpack(cfg.controls.phone.open)) and (not tSentry.isInComa() or not cfg.coma_disable_menu) and (not tSentry.isHandcuffed() or not cfg.handcuff_disable_menu) and GetLastInputMethod(0) then 
      Sentryserver.openMainMenu({})
    end
    ...
    if pause_menu and not paused then
      paused = true
      TriggerEvent("Sentry:pauseChange", paused)
