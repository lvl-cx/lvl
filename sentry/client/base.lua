...
Tunnel.bindInterface("Sentry",tSentry)

-- get server interface
Sentryserver = Tunnel.getInterface("Sentry","Sentry")

-- add client proxy interface (same as tunnel interface)
Proxy.addInterface("Sentry",tSentry)

-- functions


function tSentry.teleport(x,y,z)
  tSentry.unjail() -- force unjail before a teleportation
  SetEntityCoords(GetPlayerPed(-1), x+0.0001, y+0.0001, z+0.0001, 1,0,0,1)
  Sentryserver.updatePos({x,y,z})
end

-- return x,y,z
function tSentry.getPosition()
  local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
  return x,y,z
end

--returns the distance between 2 coordinates (x,y,z) and (x2,y2,z2)
function tSentry.getDistanceBetweenCoords(x,y,z,x2,y2,z2)
...
function tSentry.addPlayer(player)
  players[player] = true
end

function tSentry.removePlayer(player)
  players[player] = nil
end

function tSentry.getNearestPlayers(radius)
  local r = {}
...
function tSentry.getNearestPlayer(radius)
  local p = nil

  local players = tSentry.getNearestPlayers(radius)
...
function tSentry.notify(msg)
  SetNotificationTextEntry("STRING")
  AddTextComponentString(msg)
  DrawNotification(true, false)
end

function tSentry.notifyPicture(icon, type, sender, title, text)
    SetNotificationTextEntry("STRING")
...
function tSentry.playScreenEffect(name, duration)
...
-- name, see https://wiki.fivem.net/wiki/Screen_Effects
function tSentry.stopScreenEffect(name)
  StopScreenEffect(name)
end
...
function tSentry.stopAnim(upper)
...
function tSentry.setRagdoll(flag)
  ragdoll = flag
end
...
function tSentry.playSpatializedSound(dict,name,x,y,z,range)
  PlaySoundFromCoord(-1,name,x+0.0001,y+0.0001,z+0.0001,dict,0,range+0.0001,0)
end

-- play sound
function tSentry.playSound(dict,name)
...
AddEventHandler("playerSpawned",function()
  TriggerServerEvent("Sentrycli:playerSpawned")
end)

AddEventHandler("onPlayerDied",function(player,reason)
  TriggerServerEvent("Sentrycli:playerDied")
end)
