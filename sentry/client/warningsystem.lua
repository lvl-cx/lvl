...
RegisterNetEvent("sentry:showWarningsOfUser")
AddEventHandler("sentry:showWarningsOfUser",function(sentrywarningstables)
	showWarningSystem = true
	sentryWarnings = sentrywarningstables
end)

RegisterNetEvent("sentry:recievedRefreshedWarningData")
AddEventHandler("sentry:recievedRefreshedWarningData",function(sentrywarningstables)
	sentryWarnings = sentrywarningstables
end)
...
RegisterCommand('warnings', function()
    ...
    if showWarningSystem then
        TriggerServerEvent("sentry:refreshWarningSystem")
    end
end)
...
Citizen.CreateThread(function()
	while true do
		if showWarningSystem then
			...
			DrawAdvancedText(0.59, 0.198, 0.005, 0.0028, 0.619, 'Sentry' .. ' Warnings', 255, 255, 255, 255, 7, 0)
			...
			for warningID,warningTable in pairs(sentryWarnings) do
...
RegisterCommand("warn",function()
	...
	TriggerServerEvent("sentry:warnPlayer",tonumber(userIDtoWarn),userWarningMessage)
end)
