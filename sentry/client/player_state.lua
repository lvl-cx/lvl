...
Citizen.CreateThread(function()
  while true do
    ...
    if IsPlayerPlaying(PlayerId()) and state_ready then
      ...
      Sentryserver.updateHealth({tSentry.getHealth()})
      Sentryserver.updateArmour({GetPedArmour(PlayerPedId())})
      Sentryserver.updateWeapons({tSentry.getWeapons()})
      Sentryserver.updateCustomization({tSentry.getCustomization()})
    end
  end
end)
...
function tSentry.getWeaponTypes()
  return weapon_types
end

function tSentry.getWeapons()
...
function tSentry.giveWeapons(weapons,clear_before)
  local player = GetPlayerPed(-1)
...
function tSentry.getDrawables(part)
...
function tSentry.getDrawableTextures(part,drawable)
  local isprop, index = parse_part(part)
  if isprop then
...
function tSentry.getCustomization()
...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(60000)
    if state_ready then
      local custom = tSentry.getCustomization()
      custom.model = nil
      custom.modelhash = nil
      tSentry.setCustomization(custom)
    end
  end
end)
--]]
