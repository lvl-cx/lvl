...
function tSentry.addBlip(x,y,z,idtype,idcolor,text)
...
function tSentry.removeBlip(id)
  RemoveBlip(id)
end
...
-- return native id
function tSentry.setNamedBlip(name,x,y,z,idtype,idcolor,text)
  tSentry.removeNamedBlip(name) -- remove old one

  named_blips[name] = tSentry.addBlip(x,y,z,idtype,idcolor,text)
  return named_blips[name]
end

-- remove a named blip
function tSentry.removeNamedBlip(name)
  if named_blips[name] ~= nil then
    tSentry.removeBlip(named_blips[name])
...
function tSentry.setGPS(x,y)
  SetNewWaypoint(x+0.0001,y+0.0001)
end

-- set route to native blip id
function tSentry.setBlipRoute(id)
...
-- return marker id
function tSentry.addMarker(x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
...
-- remove marker
function tSentry.removeMarker(id)
...
-- return id
function tSentry.setNamedMarker(name,x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
  tSentry.removeNamedMarker(name) -- remove old marker

  named_markers[name] = tSentry.addMarker(x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
  return named_markers[name]
end

function tSentry.removeNamedMarker(name)
  if named_markers[name] ~= nil then
    tSentry.removeMarker(named_markers[name])
...
function tSentry.setArea(name,x,y,z,radius,height)
...
-- remove area
function tSentry.removeArea(name)
...
-- locked: boolean
-- doorswing: -1 to 1
function tSentry.setStateOfClosestDoor(doordef, locked, doorswing)
...
function tSentry.openClosestDoor(doordef)
  tSentry.setStateOfClosestDoor(doordef, false, 0)
end

function tSentry.closeClosestDoor(doordef)
  tSentry.setStateOfClosestDoor(doordef, true, 0)
end