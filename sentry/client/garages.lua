...
RMenu.Add('SentryGarages', 'main', RageUI.CreateMenu("", "~g~Sentry Garage Menu",1300, 50, 'garage', 'garage'))
RMenu.Add('SentryGarages', 'owned_vehicles',  RageUI.CreateSubMenu(RMenu:Get("SentryGarages", "main")))
RMenu.Add('SentryGarages', 'rented_vehicles',  RageUI.CreateSubMenu(RMenu:Get("SentryGarages", "main")))
RMenu.Add('SentryGarages', 'rented_vehicles_manage',  RageUI.CreateSubMenu(RMenu:Get("SentryGarages", "rented_vehicles")))
RMenu.Add('SentryGarages', 'buy_vehicles',  RageUI.CreateSubMenu(RMenu:Get("SentryGarages", "main")))
RMenu.Add('SentryGarages', 'buy_vehicles_submenu',  RageUI.CreateSubMenu(RMenu:Get("SentryGarages", "buy_vehicles")))
RMenu.Add('SentryGarages', 'buy_vehicles_submenu_manage',  RageUI.CreateSubMenu(RMenu:Get("SentryGarages", "buy_vehicles_submenu")))
RMenu.Add('SentryGarages', 'owned_vehicles_submenu',  RageUI.CreateSubMenu(RMenu:Get("SentryGarages", "owned_vehicles")))
RMenu.Add('SentryGarages', 'owned_vehicles_submenu_manage',  RageUI.CreateSubMenu(RMenu:Get("SentryGarages", "owned_vehicles_submenu")))
RMenu.Add('SentryGarages', 'scrap_vehicle_confirmation',  RageUI.CreateSubMenu(RMenu:Get("SentryGarages", "owned_vehicles_submenu_manage")))
...
RageUI.CreateWhile(1.0, true, function()
            ...
            RageUI.Button("Owned Vehicles", nil, {RightLabel = '~g~→'}, true, function(Hovered, Active, Selected) 
                if Selected then 
                    if Table_Type == nil or not Table_Type then 
                        Table_Type = true;
                        TriggerServerEvent('Sentry:FetchCars', true, garage_type)
                    end
                end
            end, RMenu:Get("SentryGarages", "owned_vehicles"))
            RageUI.Button("Rented Vehicles", nil, {RightLabel = '~g~→'}, true, function(Hovered, Active, Selected) end, RMenu:Get("SentryGarages", "rented_vehicles"))
            RageUI.Button("Store Vehicle", nil, {RightLabel = '~g~→'}, true, function(Hovered, Active, Selected) 
                if Selected then 
                    tSentry.despawnGarageVehicle(garage_type,250)
                end
            end)
        end)
    end
    if RageUI.Visible(RMenu:Get('SentryGarages', 'buy_vehicles')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            DeleteCar(veh)
            for i,v in pairs(VehiclesFetchedTable) do 
                if garage_type == VehiclesFetchedTable[i].config.vtype then 
                    ...
                    end, RMenu:Get("SentryGarages", "buy_vehicles_submenu"))
                end
            end
        end)
    end
            ...
            for i,v in pairs(selected_category) do 
                RageUI.Button(v[1], nil, {RightLabel = '~g~→'}, true, function(Hovered, Active, Selected)
                    if Selected then 
                        ...
                        RMenu:Get('SentryGarages', 'buy_vehicles_submenu_manage'):SetSubtitle("~b~" .. v[1] .. ' Price: $' .. v[2])
                    end
                    if Active then 
                        Hovered_Vehicles = i
                    end
                end,RMenu:Get("SentryGarages", "buy_vehicles_submenu_manage")) 
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('SentryGarages', 'buy_vehicles_submenu_manage')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            RageUI.Button("Buy Vehicle", "", {}, true, function(Hovered, Active, Selected)
                if Selected then 
                    TriggerServerEvent('Sentry:BuyVehicle', SelectedCar.spawncode)
    ...
    if RageUI.Visible(RMenu:Get('SentryGarages', 'owned_vehicles')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            DeleteCar(veh)
            RentedVeh = false;
            for i,v in pairs(VehiclesFetchedTable) do 
                if garage_type == VehiclesFetchedTable[i].config.vtype then 
                    ...
                    end, RMenu:Get("SentryGarages", "owned_vehicles_submenu"))
                end
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('SentryGarages', 'owned_vehicles_submenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for i,v in pairs(selected_category) do 
                if RentedVeh then 
                    ...
                    end,RMenu:Get("SentryGarages", "owned_vehicles_submenu_manage"))
                else 
    ...
    if RageUI.Visible(RMenu:Get('SentryGarages', 'owned_vehicles_submenu_manage')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            if not RentedVeh then 
                ...
                RageUI.Button('~r~[Scrap Vehicle]', nil, {RightLabel = '~r~→'}, true, function(Hovered, Active, Selected)end,RMenu:Get("SentryGarages", "scrap_vehicle_confirmation"))
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('SentryGarages', 'scrap_vehicle_confirmation')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            RageUI.Button('Yes', nil, {}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent('Sentry:ScrapVehicle', SelectedCar.spawncode) 
                    Table_Type = nil;
                    RageUI.ActuallyCloseAll()
                    RageUI.Visible(RMenu:Get('SentryGarages', 'main'), true)  
                end
            end)
            RageUI.Button('No', nil, {}, true, function(Hovered, Active, Selected)end,RMenu:Get("SentryGarages", "owned_vehicles_submenu_manage"))
        end)
    end
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Button('Rented Vehicles In', nil, {RightLabel = '~g~→'}, true, function(Hovered, Active, Selected)
                if Selected then
                    Table_Type = nil;
                    RentedVeh = true;
                    TriggerServerEvent('Sentry:FetchVehiclesIn')
                end
            end,RMenu:Get("SentryGarages", "rented_vehicles_manage"))
        end)
    end
    if RageUI.Visible(RMenu:Get('SentryGarages', 'rented_vehicles_out_manage')) then 
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            Hovered_Vehicles = nil 
            DeleteCar(veh)
            for i,v in pairs(VehiclesFetchedTable) do 
                if garage_type == VehiclesFetchedTable[i].config.vtype then 
                    ...
                    end, RMenu:Get("SentryGarages", "rented_vehicles_out_manage_submenu"))
                end
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('SentryGarages', 'rented_vehicles_out_manage_submenu')) then 
        RageUI.DrawContent({header = true, glare = false, instructionalButton = true}, function()
            for i,v in pairs(selected_category) do 
    ...
    if RageUI.Visible(RMenu:Get('SentryGarages', 'rented_vehicles_manage')) then 
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            Hovered_Vehicles = nil 
            DeleteCar(veh)
            for i,v in pairs(VehiclesFetchedTable) do 
...
RegisterNetEvent('Sentry:ReturnFetchedCars')
AddEventHandler('Sentry:ReturnFetchedCars', function(table)
    VehiclesFetchedTable = table;
end)

RegisterNetEvent('Sentry:CloseGarage')
AddEventHandler('Sentry:CloseGarage', function()
    DeleteCar(veh)
    Table_Type = nil;
    RageUI.ActuallyCloseAll()
end)
