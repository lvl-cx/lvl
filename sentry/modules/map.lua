...
function Sentry.setArea(source,name,x,y,z,radius,height,cb_enter,cb_leave)
  local areas = client_areas[source] or {}
  client_areas[source] = areas

  areas[name] = {enter=cb_enter,leave=cb_leave}
  Sentryclient.setArea(source,{name,x,y,z,radius,height})
end

-- delete a player area
function Sentry.removeArea(source,name)
  -- delete remote area
  Sentryclient.removeArea(source,{name})
...
function tSentry.enterArea(name)
  local areas = client_areas[source]
  if areas then
...
function tSentry.leaveArea(name)
  local areas = client_areas[source]
