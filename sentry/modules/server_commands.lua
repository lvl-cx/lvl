RegisterCommand('addgroup', function(source, args)
    ...
    if tonumber(args[1]) and args[2] then
        ...
        local group = args[2]
        Sentry.addUserGroup(userid,group)
...
RegisterCommand('removegroup', function(source, args)
    ...
    if tonumber(args[1]) and args[2] then
        local userid = tonumber(args[1])
        local group = args[2]
        Sentry.removeUserGroup(userid,group)
...
RegisterCommand('ban', function(source, args)
    ...
    if tonumber(args[1]) and args[2] then
        ...
        if reason then 
            Sentry.banConsole(userid,hours,reason)
        else 
...
RegisterCommand('whitelist', function(source, args)
    ...
    if tonumber(args[1])  then
        local userid = tonumber(args[1])
        Sentry.setWhitelisted(userid,true)
...
RegisterCommand('unwhitelist', function(source, args)
    ...
    if tonumber(args[1])  then
        local userid = tonumber(args[1])
        Sentry.setWhitelisted(userid,false)
