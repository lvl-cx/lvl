...
function Sentry.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  Sentryclient.openMenuData(source,{menudata})
end

-- force close player menu
function Sentry.closeMenu(source)
  Sentryclient.closeMenu(source,{})
end
...
function Sentry.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  Sentryclient.prompt(source,{title,default_text})
end
...
function Sentry.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  Sentryclient.request(source,{id,text,time}) -- send request to client
...
function Sentry.registerMenuBuilder(name, builder)
...
function Sentry.buildMenu(name, data, cbr)
...
function Sentry.openMainMenu(source)
  Sentry.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    Sentry.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tSentry.closeMenu(id)
...
function tSentry.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tSentry.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tSentry.requestResult(id,ok)
...
function tSentry.openMainMenu()
  Sentry.openMainMenu(source)
end
...
function Sentry.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for Sentry.addStaticMenuChoices calls
...
AddEventHandler("Sentry:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    Sentryclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
