...
local function atm_choice_deposit(player, choice)
    ...
    Sentry.prompt(source, lang.atm.deposit.prompt(), "", function(player, v)
        ...
        if v > 0 then
            ...
            if user_id ~= nil then
                if Sentry.tryDeposit(user_id, v) then
                    Sentryclient.notify(source, {lang.atm.deposit.deposited({v})})
                else
...
local function atm_choice_withdraw(player, choice)
    ...
    Sentry.prompt(source, lang.atm.withdraw.prompt(), "", function(player, v)
        ...
        if v > 0 then
            local user_id = Sentry.getUserId(source)
            if user_id ~= nil then
                if Sentry.tryWithdraw(user_id, v) then
                    Sentryclient.notify(source, {lang.atm.withdraw.withdrawn({v})})
                else
                    Sentryclient.notify(source, {lang.atm.withdraw.not_enough()})
                end
            end
...
local function atm_enter()
    ...
    if user_id ~= nil then
        ...
        end, lang.atm.info.bank({Sentry.getBankMoney(user_id)})}
        Sentry.openMenu(source, atm_menu)
    end
end
...
local function build_client_atms(source)
    ...
    if user_id ~= nil then
        for k, v in pairs(atms) do
            ...
            Sentryclient.addBlip(source, {x, y, z, 108, 4, lang.atm.title()})
            Sentryclient.addMarker(source, {x, y, z - 1, 0.7, 0.7, 0.5, 0, 255, 125, 125, 150})

            Sentry.setArea(source, "Sentry:atm" .. k, x, y, z, 1, 1.5, atm_enter, atm_leave)
        end
    end
end
