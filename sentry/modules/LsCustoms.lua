--[[
Sentry Los Santos Customs V1.2
Credits - MythicalBro and マーモット#2533 for the Sentry version and some bug fixes
]]
...
AddEventHandler("LSC:buttonSelected", function(name, button)
	...
	if user_id then
		TriggerClientEvent("LSC:buttonSelected", source, name, button, Sentry.tryFullPayment(Sentry.getUserId(source), button.price or 0)) -- money
	end
end)
...
AddEventHandler("LSC:applyModifications", function (model, vehicle)
	...
	if model and vehicle and user_id then
		local rows = MySQL.query("Sentryls/get_vehicle_modifications", {user_id = user_id, vehicle = model}, function(rows, affected) 
			if rows ~= nil then 
				if #rows > 0 then
