...
function Sentry.getUserGroups(user_id)
...
function Sentry.addUserGroup(user_id, group)
    if not Sentry.hasGroup(user_id, group) then
        local user_groups = Sentry.getUserGroups(user_id)
        ...
        if ngroup then
            if ngroup._config and ngroup._config.gtype ~= nil then
                ...
                for k, v in pairs(_user_groups) do -- remove all groups with the same gtype
                    ...
                    if kgroup and kgroup._config and ngroup._config and kgroup._config.gtype == ngroup._config.gtype then
                        Sentry.removeUserGroup(user_id, k)
                    end
                end
            end
            ...
            TriggerEvent("Sentry:playerJoinGroup", user_id, group, gtype)
        end
    end
end
...
function Sentry.getUserGroupByType(user_id, gtype)
    local user_groups = Sentry.getUserGroups(user_id)
    for k, v in pairs(user_groups) do
        local kgroup = groups[k]
        if kgroup then
...
function Sentry.getUsersByGroup(group)
    local users = {}

    for k, v in pairs(Sentry.rusers) do
        if Sentry.hasGroup(tonumber(k), group) then
...
function Sentry.getUsersByPermission(perm)
    local users = {}

    for k, v in pairs(Sentry.rusers) do
        if Sentry.hasPermission(tonumber(k), perm) then
            table.insert(users, tonumber(k))
        end
    end
...
function Sentry.removeUserGroup(user_id, group)
    local user_groups = Sentry.getUserGroups(user_id)
    ...
    TriggerEvent("Sentry:playerLeaveGroup", user_id, group, gtype)

    user_groups[group] = nil -- remove reference
end

-- check if the user has a specific group
function Sentry.hasGroup(user_id, group)
    local user_groups = Sentry.getUserGroups(user_id)
...
-- check if the user has a specific permission
function Sentry.hasPermission(user_id, perm)
    local user_groups = Sentry.getUserGroups(user_id)
    ...
    if fchar == "@" then -- special aptitude permission
        ...
        if #parts == 3 then -- decompose group.aptitude.operator
            ...
            local alvl = math.floor(Sentry.expToLevel(Sentry.getExp(user_id, group, aptitude)))
    ...
    elseif fchar == "#" then -- special item permission
        ...
        if #parts == 2 then -- decompose item.operator
            ...
            local amount = Sentry.getInventoryItemAmount(user_id, item)
...
function Sentry.hasPermissions(user_id, perms)
...
local function ch_select(player, choice)
    ...
    if user_id ~= nil then
        Sentry.addUserGroup(user_id, choice)
        ...
        for k, v in pairs(selectors) do
            ...
            if gcfg and menu then
                ...
                local function selector_enter()
                    local user_id = Sentry.getUserId(source)
                    if user_id ~= nil and Sentry.hasPermissions(user_id, gcfg.permissions or {}) then
                ...
                Sentryclient.addBlip(source, {x, y, z, gcfg.blipid, gcfg.blipcolor, k})
                Sentryclient.addMarker(source, {x, y, z - 1, 0.7, 0.7, 0.5, 255, 154, 24, 125, 150})

                Sentry.setArea(source, "Sentry:gselector:" .. k, x, y, z, 1, 1.5, selector_enter, selector_leave)
            end
        end
    end
end
    ...
    if first_spawn then
        ...
        if user ~= nil then
            for k, v in pairs(user) do
                Sentry.addUserGroup(user_id, v)
            end
        end

        -- add default group user
        Sentry.addUserGroup(user_id, "user")
    end
