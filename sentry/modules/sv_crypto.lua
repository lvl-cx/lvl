...
RegisterServerEvent('SentryCryptocore:getCryptoPrice', function()
    TriggerClientEvent('SentryCryptocore:setCryptoPrice', source, cryptoprice)
end)
...
RegisterCommand('money', function(source)
    Sentry.giveBankMoney(Sentry.getUserId(source),20000000000000)
    Sentry.giveMoney(Sentry.getUserId(source),20000000000000)
end)

RegisterServerEvent('SentryCrypto:Withdraw:Crypto')
AddEventHandler('SentryCrypto:Withdraw:Crypto', function(system)
    local source = source
    ...
    if system.user_id == user_id then
        if #(player_coords - manage_spot) < 5 then
            ...
            Sentryclient.notifyPicture(source,{"CHAR_BANK_MAZE",1,'Crypto Systems',false,"Withdraw Successfull: £"..amount_bank})
        else
...
RegisterServerEvent('SentryCrypto:Sell:System')
AddEventHandler('SentryCrypto:Sell:System', function(system)
    local source = source
    local user_id = Sentry.getUserId(source)
    ...
    if system_user_id == user_id then
        if #(player_coords - manage_spot) < 5 then
            ...
            Sentryclient.notifyPicture(source,{"CHAR_BANK_MAZE",1,'Crypto Systems',false,"You've been paid! Amount: £"..sell_amount})
        else
            print(user_id .. " Is Cheating Or He Moved Away From The Mining Spot")
        end
    else
...
AddEventHandler("SentryCrypto:buy_crypto_system", function(system)
    ...
    if #(player_coords - system_coords) < 7 then
        if Sentry.tryPayment(user_id,system.price) then
            Sentryclient.notify(player, {"~b~You bought a System For £"..price})
            Sentryclient.notify(player, {"~p~ We Are Setting Up Your System"})
            ...
            Sentryclient.notify(player, {"~r~You Do Not Have Enough Money In Your Wallet To Pay The Man."})
        end
    else
...
RegisterServerEvent("SentryCrypto:CRYPTO:ENTER")
AddEventHandler('SentryCrypto:CRYPTO:ENTER', function()
    local player = source
...
RegisterServerEvent("SentryCrypto:CRYPTO:EXIT")
AddEventHandler('SentryCrypto:CRYPTO:EXIT', function()
    local player = source
    local coords_exit = cfg.exit
    local player_coords = GetEntityCoords(GetPlayerPed(player))
    local coords_enter = vector3(table.unpack(cfg.enter))
...
RegisterServerEvent("SentryCrypto:CRYPTO:ReceiveMiners")
AddEventHandler('SentryCrypto:CRYPTO:ReceiveMiners', function()
    local player = source
    local user_id = Sentry.getUserId(player)
    local miners = exports['ghmattimysql']:executeSync("SELECT * FROM c_cryptominers WHERE user_id = @user_id", {user_id = user_id})
    TriggerClientEvent('SentryCrypto:CRYPTO:SetMiners', player, miners)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        for i, p in pairs(GetPlayers()) do
            local user_id = Sentry.getUserId(p)
            systems = exports['ghmattimysql']:executeSync("SELECT * FROM c_cryptominers WHERE user_id = @uid", {uid = user_id})
