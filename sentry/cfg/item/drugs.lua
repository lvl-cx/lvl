...
local function play_drink(player)
  ...
  Sentryclient.playAnim(player,{true,seq,false})
end
...
pills_choices["Take"] = {function(player,choice)
  ...
  if user_id ~= nil then
    Sentryclient.isInComa(player,{}, function(in_coma)    
        if not in_coma then
          if Sentry.tryGetInventoryItem(user_id,"pills",1) then
            Sentryclient.varyHealth(player,{25})
            Sentryclient.notify(player,{"~g~ Taking pills."})
            play_drink(player)
...
local function play_smoke(player)
  ...
  Sentryclient.playAnim(player,{true,seq2,false})
end
...
smoke_choices["Take"] = {function(player,choice)
  ...
  if user_id ~= nil then
    if Sentry.tryGetInventoryItem(user_id,"weed",1) then
	  Sentry.varyHunger(user_id,(20))
      Sentryclient.notify(player,{"~g~ smoking weed."})
      play_smoke(player)
      Sentry.closeMenu(player)
    end
  end
...
local function play_smell(player)
  ...
  Sentryclient.playAnim(player,{true,seq3,false})
end
...
smell_choices["Take"] = {function(player,choice)
  ...
  if user_id ~= nil then
    if Sentry.tryGetInventoryItem(user_id,"cocaine",1) then
	  Sentry.varyThirst(user_id,(20))
      Sentryclient.notify(player,{"~g~ smell cocaine."})
      play_smell(player)
      Sentry.closeMenu(player)
    end
  end
...
local function play_lsd(player)
  ...
  Sentryclient.playAnim(player,{true,seq4,false})
end
...
lsd_choices["Take"] = {function(player,choice)
  ...
  if user_id ~= nil then
    if Sentry.tryGetInventoryItem(user_id,"lsd",1) then
	  Sentry.varyThirst(user_id,(20))
      Sentryclient.notify(player,{"~g~ Taking lsd."})
      play_lsd(player)
      Sentry.closeMenu(player)
    end
  end
