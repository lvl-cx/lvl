...
local function gen(ftype, vary_hunger, vary_thirst)
  local fgen = function(args)
    ...
    local name = Sentry.getItemName(idname)

    choices[act] = {function(player,choice)
      local user_id = Sentry.getUserId(player)
      if user_id ~= nil then
        if Sentry.tryGetInventoryItem(user_id,idname,1,false) then
          if vary_hunger ~= 0 then Sentry.varyHunger(user_id,vary_hunger) end
          if vary_thirst ~= 0 then Sentry.varyThirst(user_id,vary_thirst) end

          if ftype == "drink" then
            Sentryclient.notify(player,{"~b~ Drinking "..name.."."})
            play_drink(player)
          elseif ftype == "eat" then
            Sentryclient.notify(player,{"~o~ Eating "..name.."."})
            play_eat(player)
          end
