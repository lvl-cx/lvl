...
cfg.Buttons = {
    ...
    TriggerServerEvent('SentryAdmin:Ban', self, hours, reason)
    end, "player.ban"},
    ["Kick"] = {function(self)
    ...
    if (GetOnscreenKeyboardResult()) then
        ...
        if result then 
            result = result
            TriggerServerEvent('SentryAdmin:Kick', self, result, false)
        end
    end
    end, "player.kick"},
    ["No Warning Kick"] = {function(self)
        ...
        if (GetOnscreenKeyboardResult()) then
            ...
            if result then 
                result = result
                TriggerServerEvent('SentryAdmin:Kick', self, result, true)
            end
        end
    end, "player.kick"},
    ["Revive"] = {function(self)
        TriggerServerEvent('SentryAdmin:Revive', self)
    end, "player.revive"},
    ["Slap"] = {function(self)
        TriggerServerEvent('SentryAdmin:SlapPlayer', self)
    end, "player.slap"},
    ["Spectate"] = {function(self)
        TriggerServerEvent('SentryAdmin:SpectatePlr', self)
    end, "player.spectate"},
    ...
    ["Add Car"] = {function(self)
        ...
        if (GetOnscreenKeyboardResult()) then
            local result = GetOnscreenKeyboardResult()
            if result then 
                TriggerServerEvent('SentryAdmin:AddCar', self, result)
            end
        end
    end, "player.addcar"},
    ["TP To Player"] = {function(self)
        TriggerServerEvent('SentryAdmin:TPTo', self)
    end, "player.tpto"},
    ["Bring Player"] = {function(self)
        TriggerServerEvent('SentryAdmin:Bring', self)
    end, "player.tpbring"},
    ["Add Group"] = {function(self)
        ...
        if (GetOnscreenKeyboardResult()) then
            local result = GetOnscreenKeyboardResult()
            if result then 
                TriggerServerEvent('SentryAdmin:AddGroup', self, result)
            end
        end
    ...
    ["Warn Player"] = {function(self)
        ...
        if userWarningMessage then
            TriggerServerEvent("sentry:warnPlayer",self,userWarningMessage)
        else
            tSentry.notify('Please enter a warning message!')
        end
    end, "player.kick"},
...
--Sentry:RemoveWarning
cfg.MiscButtons = {
    ["Entity Cleanup"] = {function(self)
        TriggerServerEvent('SentryAdmin:PropCleanup')
    ...
    ["Remove Warning"] = {function(self)
        ...
        if (GetOnscreenKeyboardResult()) then
            local result = GetOnscreenKeyboardResult()
            if result then 
                TriggerServerEvent('Sentry:RemoveWarning', result)
            end
        end
    end, "admin.removewarning", "Removes a warning"},
    ["Entity Cleanup Gun"] = {function(self)
        TriggerServerEvent('SentryAdmin:EntityCleanupGun')
    ...
    ["Shutdown Server"] = {function(self)
        TriggerServerEvent('SentryAdmin:ServerShutdown')
        ...
        if (GetOnscreenKeyboardResult()) then
            ...
            if result then 
                ...
                while not HasModelLoaded(mhash) and i < 10000 do
                    ...
                    i = i+1
                    if i > 10000 then 
                        tSentry.notify('~r~Model could not be loaded!')
                        break 
                    end
                end
                -- spawn car
