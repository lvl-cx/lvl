--[[ local cfg = module("BW","cfg/cfg_spawn")
local spawnCam = nil
local spawnCam2 = nil
...
Citizen.CreateThread(function()
    ...
    local success, result = pcall(function()
        local homeCfg = module("BW","cfg/homes")
        for homeName, homeInfo in pairs(homeCfg.homes) do
            cfg.spawnLocations[homeName] = {
                name = homeName,
                coords = vector3(homeInfo.entry_point[1],homeInfo.entry_point[2],homeInfo.entry_point[3]),
                permission = {},
                image = homeInfo.image or "https://cdn.cmg.city/content/fivem/houses/citysmallhome.png",
                price = 5000
            }
        end
    end)
...
RegisterNUICallback("respawnButtonClicked", function(data, cb)
    ...
    if price and price > 0 then
        TriggerServerEvent('BW:takeAmountRespawn', price)
    end
    local spawnCoords =  cfg.spawnLocations[name].coords
    TriggerEvent("BW:playGTAIntro")
...
RegisterNetEvent("BW:openSpawnMenu")
AddEventHandler("BW:openSpawnMenu", function(location_table)
    DoScreenFadeIn(1000)
    TriggerScreenblurFadeIn(100.0)
    ExecuteCommand("hideui")
    SetPlayerControl(PlayerId(), 0, 0)
    FreezeEntityPosition(PlayerPedId(),true)
    TriggerEvent("BW:removeBackpack2")
    ...
    for _, homeName in pairs(location_table) do
        if cfg.spawnLocations[homeName] then
            table.insert(locations, cfg.spawnLocations[homeName])
        end
    end
    SendNUIMessage({
