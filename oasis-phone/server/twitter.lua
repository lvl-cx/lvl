  ...
  getUser(username, password, function (user)
    ...
    MySQL.Async.insert("INSERT INTO twitter_tweets (`authorId`, `message`, `realUser`) VALUES(@authorId, @message, @realUser);", {
      ...
      MySQL.Async.fetchAll('SELECT * from twitter_tweets WHERE id = @id', {
        ...
        TriggerClientEvent('OASIS:twitter_newTweets', -1, tweet)
        TriggerEvent('OASIS:twitter_newTweets', tweet)
      end)
    end)
  end)
end
        ...
        if (row[1] == nil) then
          MySQL.Async.insert('INSERT INTO twitter_likes (`authorId`, `tweetId`) VALUES(@authorId, @tweetId)', {
            ...
            MySQL.Async.execute('UPDATE `twitter_tweets` SET `likes`= likes + 1 WHERE id = @id', {
              ...
              TriggerClientEvent('OASIS:twitter_updateTweetLikes', -1, tweet.id, tweet.likes + 1)
              TriggerClientEvent('OASIS:twitter_setTweetLikes', sourcePlayer, tweet.id, true)
              TriggerEvent('OASIS:twitter_updateTweetLikes', tweet.id, tweet.likes + 1)
            end)    
          end)
        else
          MySQL.Async.execute('DELETE FROM twitter_likes WHERE id = @id', {
            ...
            MySQL.Async.execute('UPDATE `twitter_tweets` SET `likes`= likes - 1 WHERE id = @id', {
              ...
              TriggerClientEvent('OASIS:twitter_updateTweetLikes', -1, tweet.id, tweet.likes - 1)
              TriggerClientEvent('OASIS:twitter_setTweetLikes', sourcePlayer, tweet.id, false)
              TriggerEvent('OASIS:twitter_updateTweetLikes', tweet.id, tweet.likes - 1)
            end)
          end)
        end
...
function TwitterShowError (sourcePlayer, title, message)
  TriggerClientEvent('OASIS:twitter_showError', sourcePlayer, message)
end
function TwitterShowSuccess (sourcePlayer, title, message)
  TriggerClientEvent('OASIS:twitter_showSuccess', sourcePlayer, title, message)
end

RegisterServerEvent('OASIS:twitter_login')
AddEventHandler('OASIS:twitter_login', function(username, password)
  ...
  getUser(username, password, function (user)
    if user == nil then
      ...
      TriggerClientEvent('OASIS:twitter_setAccount', sourcePlayer, username, password, user.authorIcon)
    end
  end)
end)

RegisterServerEvent('OASIS:twitter_changePassword')
AddEventHandler('OASIS:twitter_changePassword', function(username, password, newPassword)
  local sourcePlayer = tonumber(source)
  getUser(username, password, function (user)
    if user == nil then
      ...
      MySQL.Async.execute("UPDATE `twitter_accounts` SET `password`= @newPassword WHERE twitter_accounts.username = @username AND twitter_accounts.password = @password", {
        ...
        if (result == 1) then
          TriggerClientEvent('OASIS:twitter_setAccount', sourcePlayer, username, newPassword, user.authorIcon)
...
RegisterServerEvent('OASIS:twitter_createAccount')
AddEventHandler('OASIS:twitter_createAccount', function(username, password, avatarUrl)
  local sourcePlayer = tonumber(source)
  TwitterCreateAccount(username, password, avatarUrl, function (id)
    if (id ~= 0) then
      TriggerClientEvent('OASIS:twitter_setAccount', sourcePlayer, username, password, avatarUrl)
...
RegisterServerEvent('OASIS:twitter_getTweets')
AddEventHandler('OASIS:twitter_getTweets', function(username, password)
  local sourcePlayer = tonumber(source)
  if username ~= nil and username ~= "" and password ~= nil and password ~= "" then
    getUser(username, password, function (user)
      local accountId = user and user.id
      TwitterGetTweets(accountId, function (tweets)
        TriggerClientEvent('OASIS:twitter_getTweets', sourcePlayer, tweets)
      end)
    end)
  else
...
RegisterServerEvent('OASIS:twitter_getFavoriteTweets')
AddEventHandler('OASIS:twitter_getFavoriteTweets', function(username, password)
  local sourcePlayer = tonumber(source)
  if username ~= nil and username ~= "" and password ~= nil and password ~= "" then
    getUser(username, password, function (user)
      local accountId = user and user.id
      TwitterGetFavotireTweets(accountId, function (tweets)
        TriggerClientEvent('OASIS:twitter_getFavoriteTweets', sourcePlayer, tweets)
      end)
    end)
  else
...
RegisterServerEvent('OASIS:twitter_postTweets')
AddEventHandler('OASIS:twitter_postTweets', function(username, password, message)
  local sourcePlayer = tonumber(source)
  local srcIdentifier = getPlayerID(source)
  TwitterPostTweet(username, password, message, sourcePlayer, srcIdentifier)
end)

RegisterServerEvent('OASIS:twitter_toogleLikeTweet')
AddEventHandler('OASIS:twitter_toogleLikeTweet', function(username, password, tweetId)
  local sourcePlayer = tonumber(source)
  TwitterToogleLike(username, password, tweetId, sourcePlayer)
end)


RegisterServerEvent('OASIS:twitter_setAvatarUrl')
AddEventHandler('OASIS:twitter_setAvatarUrl', function(username, password, avatarUrl)
  local sourcePlayer = tonumber(source)
