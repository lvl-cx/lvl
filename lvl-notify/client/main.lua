RegisterNetEvent('lvl-notify:client:SendAlert')
AddEventHandler('lvl-notify:client:SendAlert', function(data)
	DoCustomHudText(data.type, data.text, data.length, data.style)
end)

RegisterNetEvent('lvl-notify:client:PersistentHudText')
AddEventHandler('lvl-notify:client:PersistentHudText', function(data)
	PersistentHudText(data.action, data.id, data.type, data.text, data.style)
end)

RegisterNetEvent('lvlCUSTOMNOTIFY:notify')
AddEventHandler('lvlCUSTOMNOTIFY:notify', function(type, message)
  exports['lvl-notify']:SendAlert(type, message)
end)
