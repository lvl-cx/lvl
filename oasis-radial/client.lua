                ...
                elseif EntityType == 3 then
                    ...
                    if `xs_prop_arena_bag_01` == entityModel then
                        ...
                        if IsControlJustReleased(1, 38) then
                            ...
                            TriggerServerEvent('OASIS:LootBag', LootBagIDNew)
                    ...
                    elseif `prop_poly_bag_money` == entityModel then
                        ...
                        if IsControlJustReleased(1, 38) then
                            ...
                            TriggerServerEvent('OASIS:Moneydrop', MoneydropIDNew)
                            TriggerEvent("OASIS:startCombatTimer", false)
                            Wait(1000)
                        end
                    ...
                    elseif `xs_prop_arena_crate_01a` == entityModel then
                        ...
                        if IsControlJustReleased(1, 38) then
                            ...
                            TriggerEvent("OASIS:startCombatTimer", false)
                            TriggerServerEvent("OASIS:openCrate",id)
                            Wait(1000)
                        end
                    end
                else
...
function lockCar(entityId)
    TriggerEvent("OASIS:lockNearestVehicle")
end
...
function openBoot(entityId)
    ...
    TriggerEvent("OASIS:clOpenTrunk")
    trunkStatus = true
    SendNUIMessage({
        closeMenu = true
    })
...
RegisterNetEvent("OASIS:clCloseTrunk")
...
function lockpick(entityId)
    TriggerEvent("OASIS:verifyLockpick",entityId)
end

function repairVehicle(entityId)
    TriggerServerEvent("OASIS:attemptRepairVehicle")
end
...
function searchVehicle(entityId)
    if globalOnPoliceDuty then
        TriggerEvent("OASIS:searchClient",entityId)
    end
end
...
function impoundVehicle(entityId)
    if globalOnPoliceDuty then
        ...
        if user_id > 0 then
            exports.oasis:impound(user_id, GetEntityModel(entityId), VehToNet(entityId), entityId)
        else
            TriggerEvent("OASIS:Notify","~r~Vehicle is not owned by anyone")
            if GetPedInVehicleSeat(entityId, -1) == 0 and GetPedInVehicleSeat(entityId, 0) == 0 and NetworkGetEntityIsNetworked(entityId) then
                TriggerServerEvent("OASIS:impoundDeleteVehicle", NetworkGetNetworkIdFromEntity(entityId))
            end
        end
    end
end
...
function robPerson(entityId)
    ...
    if playerSrc > 0 then
        if GetSelectedPedWeapon(PlayerPedId()) ~= GetHashKey("WEAPON_UNARMED") then
            TriggerServerEvent("OASIS:robPlayer",playerSrc)
        else
            TriggerEvent("OASIS:Notify","~r~You need a weapon in your hands.")
        end
    end
end
...
function askId(entityId)
    ...
    local playerSrc = GetPlayerServerId(player)
    if playerSrc > 0 then
        TriggerServerEvent("OASIS:askId",playerSrc)
    end
end
...
function giveCash(entityId)
    ...
    local playerSrc = GetPlayerServerId(player)
    if playerSrc > 0 then
        TriggerServerEvent("OASIS:giveCashToPlayer",playerSrc)
    end
end

function searchPlayer(entityId)
    if not exports["oasis"]:canAnim() then
        return
    end
    if GetEntityHealth(entityId) <= 102 then
        TriggerEvent("OASIS:Notify","~r~You can not search a player who is dead.")
        return
    end
    local player = GetPlayerByEntityID(entityId)
    if not globalOnPoliceDuty and not globalOnPrisonDuty then
        ...
        if ped ~= nil then
            if IsEntityPlayingAnim(ped, 'missminuteman_1ig_2', 'handsup_enter', 3) or IsEntityPlayingAnim(ped, "random@arrests", "idle_2_hands_up", 3) or IsEntityPlayingAnim(ped, "random@arrests@busted", "idle_a", 3) then
                local playerSrc = GetPlayerServerId(player)
                if playerSrc > 0 then
                    TriggerServerEvent("OASIS:searchPlayer",playerSrc)
                end
            else
                TriggerEvent("OASIS:Notify","~r~Player must have their hands up or be on their knees!")
            end
        end
    else
        local playerSrc = GetPlayerServerId(player)
...
function revive(entityId)
    ...
    if playerSrc > 0 then
        if globalOnNHSDuty then
            TriggerServerEvent("OASIS:nhsRevive",playerSrc)
        else
            TriggerServerEvent("OASIS:attemptCPR",playerSrc)
        end
    end
end
...
function drag(entityId)
    if globalOnPoliceDuty or globalOnPrisonDuty then
        ...
        local playerSrc = GetPlayerServerId(player)
        if playerSrc > 0 then
            TriggerServerEvent("OASIS:dragPlayer",playerSrc)
        end
    end
end
...
function putInCar(entityId)
    if globalOnPoliceDuty or globalOnPrisonDuty then
        ...
        local playerSrc = GetPlayerServerId(player)
        if playerSrc > 0 then
            TriggerServerEvent("OASIS:putInVehicle",playerSrc)
        end
    end
end
...
local function gunshotTest(entityId)
    ...
    local playerSrc = GetPlayerServerId(player)
    if playerSrc > 0 then
        TriggerServerEvent("OASIS:gunshotTest", playerSrc)
    end
end
...
function jail(entityId) -- need to rewrite jail i'll do it then
    if globalOnPoliceDuty or globalOnPrisonDuty then
        ...
        local playerSrc = GetPlayerServerId(player)
        if playerSrc > 0 then
            TriggerServerEvent("OASIS:seizeWeapons",playerSrc)
            TriggerServerEvent("OASIS:jailPlayer",playerSrc)
        end
    end
end
...
function requestTransport(entityId) -- do later
    if globalOnPoliceDuty or globalOnPrisonDuty then
        ...
        local playerSrc = GetPlayerServerId(player)
        if playerSrc > 0 then
            TriggerServerEvent("OASIS:requestTransport",playerSrc)
        end
    end
end
...
function seizeillegals(entityId) -- need to write drug checks
    if globalOnPoliceDuty or globalOnPrisonDuty then
        ...
        local playerSrc = GetPlayerServerId(player)
        if playerSrc > 0 then
            TriggerServerEvent("OASIS:seizeIllegals",playerSrc)
        end
    end
end
...
RegisterNetEvent("oasis-radial:globalOnPoliceDuty")
AddEventHandler("oasis-radial:globalOnPoliceDuty",function(a)
    globalOnPoliceDuty = a
end)
RegisterNetEvent("oasis-radial:globalOnNHSDuty")
AddEventHandler("oasis-radial:globalOnNHSDuty",function(b)
    globalOnNHSDuty = b
end)
RegisterNetEvent("oasis-radial:globalOnPrisonDuty")
AddEventHandler("oasis-radial:globalOnPrisonDuty",function(c)
