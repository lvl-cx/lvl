...
function ATMbm.loadFreeze(notify,god,ghost)
	if not frozen then
	  if notify then
...
function ATMbm.lockpickVehicle(wait,any)
	Citizen.CreateThread(function()
		...
		if DoesEntityExist(vehicleHandle) then
		  if GetVehicleDoorsLockedForPlayer(vehicleHandle,PlayerId()) or any then
			...
			ATM.notify({"~g~Vehicle already unlocked."})
		  end
		else
			ATM.notify({"~r~Too far away from vehicle."})
		end
	end)
end

function ATMbm.spawnVehicle(model) 
    -- load vehicle model
    local i = 0
    local mhash = GetHashKey(model)
    while not HasModelLoaded(mhash) and i < 1000 do
	  if math.fmod(i,100) == 0 then
	    ATM.notify({"~b~Loading vehicle model."})
	  end
      RequestModel(mhash)
      Citizen.Wait(30)
	  i = i + 1
    ...
    if HasModelLoaded(mhash) then
	  ...
	  ATM.notify({"~g~Vehicle spawned."})
	else
	  ATM.notify({"~r~Vehicle model invalid."})
	end
end
...
function ATMbm.getVehicleInDirection( coordFrom, coordTo )
    local rayHandle = CastRayPointToPoint( coordFrom.x, coordFrom.y, coordFrom.z, coordTo.x, coordTo.y, coordTo.z, 10, GetPlayerPed( -1 ), 0 )
    local _, _, _, _, vehicle = GetRaycastResult( rayHandle )
    return vehicle
end

function ATMbm.getNearestVehicle(radius)
...
function ATMbm.deleteVehicleInFrontOrInside(offset)
  local ped = GetPlayerPed(-1)
  local veh = nil
  if (IsPedSittingInAnyVehicle(ped)) then 
    veh = GetVehiclePedIsIn(ped, false)
  else
    veh = ATMbm.getVehicleInDirection(GetEntityCoords(ped, 1), GetOffsetFromEntityInWorldCoords(ped, 0.0, offset, 0.0))
  end
  ...
  if IsEntityAVehicle(veh) then
    ...
    ATM.notify({"~g~Vehicle deleted."})
  else
    ATM.notify({"~r~Too far away from vehicle."})
  end
end

function ATMbm.deleteNearestVehicle(radius)
  local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
  local veh = ATMbm.getNearestVehicle(radius)
