...
tLVL = Proxy.getInterface("LVL")

local LootBagCrouchLoop = false;
RegisterCommand('inventory', function()
    if not tLVL.isInComa({}) then
        if not inventoryOpen then
            TriggerServerEvent('LVL:FetchPersonalInventory')
            inventoryOpen = true; 
            SetNuiFocus(true, true)
            SetNuiFocusKeepInput(true)
            SendNUIMessage({action = 'InventoryDisplay', showInv = true})
            local VehInRadius, VehType, NVeh = tLVL.getNearestOwnedVehicle({3.5})
            if VehInRadius and IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then 
                ...
                tLVL.vc_openDoor({VehTypeC, 5})
                inventoryType = 'CarBoot'
                
                TriggerServerEvent('LVL:FetchTrunkInventory', NVeh)
            end
            ...
            if IsLootBagOpening then
                ...
                TriggerServerEvent('LVL:CloseLootbag')
        ...
        tLVL.notify({'~r~You cannot open your inventory while dead.'})
    end
end)
...
RegisterNetEvent('LVL:InventoryOpen')
...
RegisterNetEvent('LVL:ToggleNUIFocus')
AddEventHandler('LVL:ToggleNUIFocus', function(value)
    --print('focus', value)
    SetNuiFocus(value, value)
    SetNuiFocusKeepInput(value)
end)

RegisterNetEvent('LVL:SendSecondaryInventoryData')
AddEventHandler('LVL:SendSecondaryInventoryData', function(InventoryData, CurrentKG, MaxKg)
...
AddEventHandler('LVL:FetchPersonalInventory', function(table, CurrentKG, MaxKG)
...
RegisterNUICallback('UseBtn', function(data, cb)
    TriggerServerEvent('LVL:UseItem', data.itemId, data.invType)
    cb(true);
...
RegisterNUICallback('TrashBtn', function(data, cb)
    TriggerServerEvent('LVL:TrashItem', data.itemId, data.invType)
    cb(true);
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)

RegisterNUICallback('GiveBtn', function(data, cb)
    TriggerServerEvent('LVL:GiveItem', data.itemId, data.invType)
    cb(true)
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)
...
RegisterNUICallback('MoveBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('LVL:MoveItem', data.invType, data.itemId, VehTypeA)
        elseif inventoryType == "Housing" then
            TriggerServerEvent('LVL:MoveItem', data.invType, data.itemId, "home")
        end
    else 
        TriggerServerEvent('LVL:MoveItem', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNUICallback('MoveXBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('LVL:MoveItemX', data.invType, data.itemId, VehTypeA)
        elseif inventoryType == "Housing" then
            TriggerServerEvent('LVL:MoveItemX', data.invType, data.itemId, "home")
        end
    else 
        TriggerServerEvent('LVL:MoveItemX', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNUICallback('MoveAllBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            local nearestVeh2 = LVL.getNearestVehicle({3})
            TriggerServerEvent('LVL:MoveItemAll', data.invType, data.itemId, VehTypeA, NetworkGetNetworkIdFromEntity(nearestVeh2))
        elseif inventoryType == "Housing" then
            TriggerServerEvent('LVL:MoveItemAll', data.invType, data.itemId, "home")
        end
    else 
        TriggerServerEvent('LVL:MoveItemAll', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
AddEventHandler('LVL:IsInComa', function(coma)
    PlayerInComa = coma;
    if coma then 
        LootBagCoords = false;
        NearLootBag = false; 
