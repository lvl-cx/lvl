...
function module(rsc, path) -- load a LUA resource file as module
    if path == nil then -- shortcut for lvl, can omit the resource parameter
        path = rsc
        rsc = "lvl"
    end
    local key = rsc .. path
    ...
    if modules[key] then -- cached module
        ...
        if f then
            ...
            if ar[1] then
                ...
                modules[key] = nil
                print("[LVL] error loading module " .. rsc .. "/" .. path .. ":" .. ar[2])
            end
        else
            print("[LVL] error parsing module " .. rsc .. "/" .. path .. ":" .. err)
        end
    end
end
