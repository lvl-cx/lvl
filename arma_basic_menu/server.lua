...
-- LOG FUNCTION
function ARMAbm.logInfoToFile(file,info)
  file = io.open(file, "a")
  if file then
...
local choice_service = {function(player,choice)
  ...
  if user_id ~= nil then
    if ARMA.hasGroup({user_id,service}) then
	  ARMA.removeUserGroup({user_id,service})
	  if ARMA.hasMission({player}) then
		ARMA.stopMission({player})
	  end
      ARMAclient.notify(player,{"~r~Off service"})
	else
	  ARMA.addUserGroup({user_id,service})
      ARMAclient.notify(player,{"~g~On service"})
	end
  end
...
local ch_fixhair = {function(player,choice)
    local custom = {}
    local user_id = ARMA.getUserId({player})
    ARMA.getUData({user_id,"ARMA:head:overlay",function(value)
	  if value ~= nil then
	    custom = json.decode(value)
        ARMAbsC.setOverlay(player,{custom,true})
	  end
	end})
...
local ch_spikes = {function(player,choice)
	...
	BMclient.isCloseToSpikes(player,{},function(closeby)
		if closeby and (spikes[player] or ARMA.hasPermission({user_id,"admin.spikes"})) then
		  BMclient.removeSpikes(player,{})
		  spikes[player] = false
		elseif closeby and not spikes[player] and not ARMA.hasPermission({user_id,"admin.spikes"}) then
		  ARMAclient.notify(player,{"~r~You can carry only one set of spikes!"})
		elseif not closeby and spikes[player] and not ARMA.hasPermission({user_id,"admin.spikes"}) then
		  ARMAclient.notify(player,{"~r~You can deploy only one set of spikes!"})
		elseif not closeby and (not spikes[player] or ARMA.hasPermission({user_id,"admin.spikes"})) then
...
local ch_crun = {function(player,choice)
  ARMA.prompt({player,"Function:","",function(player,stringToRun) 
...
local choice_store_money = {function(player, choice)
  ...
  if user_id ~= nil then
    ...
    if ARMA.tryPayment({user_id, amount}) then -- unpack the money
      ARMA.giveInventoryItem({user_id, "money", amount, true})
    end
  end
...
emergency_medkit["Take"] = {function(player,choice)
	local user_id = ARMA.getUserId({player}) 
	ARMA.giveInventoryItem({user_id,"medkit",25,true})
	ARMA.giveInventoryItem({user_id,"pills",25,true})
end}
...
emergency_heal["Heal"] = {function(player,choice)
	local user_id = ARMA.getUserId({player}) 
	ARMAclient.setHealth(player,{1000})
end}
...
local choice_loot = {function(player,choice)
  ...
  if ARMA.hasPermission({user_id,"police.easy_fine"}) then
    return   ARMAclient.notify(player,{"~r~You can't loot as a cop!"})
  end 
  if ARMA.hasPermission({nuser_id,"police.easy_fine"}) then
    return   ARMAclient.notify(player,{"~r~You cannot loot cops!"})
  end 
			local revive_seq = {
            ...
            SetTimeout(1, function()
              local ndata = ARMA.getUserDataTable({nuser_id})
              if ndata ~= nil then
          ...
          -- ARMA.clearInventory({nuser_id})
                  ...
                  for k,v in pairs(ndata.inventory) do 
                    ...
                    local itemweight = ARMA.getItemWeight({k})*tonumber(v.amount)
                    local new_weight = tonumber(weight)+ itemweight
                    local maxweight = ARMA.getInventoryMaxWeight({user_id})
                    print(weight,new_weight,maxweight,itemweight) 
                    if new_weight > maxweight then
                      ARMAclient.notify(player,{"Darn your inventory is too full."})
                    else 
                      if ARMA.tryGetInventoryItem({nuser_id, k, v.amount, true}) then 
                      ARMA.giveInventoryItem({user_id,k,v.amount,true})
                      end 
                    end 
	              end
				end
			  end
			  local nmoney = ARMA.getMoney({nuser_id})
			  if ARMA.tryPayment({nuser_id,nmoney}) then
			    ARMA.giveMoney({user_id,nmoney})
			  end
            end)
...
local ch_hack = {function(player,choice)
  ...
  local user_id = ARMA.getUserId({player})
  if user_id ~= nil then
    ARMAclient.getNearestPlayer(player,{25},function(nplayer)
      if nplayer ~= nil then
        local nuser_id = ARMA.getUserId({nplayer})
        if nuser_id ~= nil then
          -- prompt number
		  local nbank = ARMA.getBankMoney({nuser_id})
          local amount = math.floor(nbank*0.01)
		  local nvalue = nbank - amount
		  if math.random(1,100) == 1 then
			ARMA.setBankMoney({nuser_id,nvalue})
            ARMAclient.notify(nplayer,{"Hacked ~r~".. amount .."£."})
		    ARMA.giveInventoryItem({user_id,"dirty_money",amount,true})
		  else
            ARMAclient.notify(nplayer,{"~g~Hacking attempt failed."})
            ARMAclient.notify(player,{"~r~Hacking attempt failed."})
		  end
        else
          ARMAclient.notify(player,{lang.common.no_player_near()})
        end
      ...
      if nplayer ~= nil then
        ...
        if nuser_id ~= nil then
		  ...
		  if math.random(1,3) == 1 then
            if ARMA.tryPayment({nuser_id,amount}) then
              ARMAclient.notify(nplayer,{"Mugged ~r~"..amount.."£."})
		      ARMA.giveInventoryItem({user_id,"dirty_money",amount,true})
            else
              ARMAclient.notify(player,{lang.money.not_enough()})
            end
		  else
            ARMAclient.notify(nplayer,{"~g~Mugging attempt failed."})
            ARMAclient.notify(player,{"~r~Mugging attempt failed."})
		  end
        else
          ARMAclient.notify(player,{lang.common.no_player_near()})
        end
        ...
        if nuser_id ~= nil then
		  ARMAclient.isHandcuffed(nplayer,{},function(handcuffed)
			if handcuffed then
				...
				ARMAclient.notify(player,{"Player is not handcuffed."})
			end
		  end)
        else
          ARMAclient.notify(player,{lang.common.no_player_near()})
  ...
  ARMAclient.getNearestPlayer(player,{5},function(nplayer)
    ...
    if nuser_id ~= nil then
      ...
      ARMAclient.getWeapons(nplayer,{},function(weapons)
        ...
        ARMAclient.setDiv2(player,{"police_check",".div_police_check{ background-color: rgba(0,0,0,0.75); color: white; font-weight: bold; width: 500px; padding: 10px; margin: auto; margin-top: 150px; }",lang.police.menu.check.info({money,items,weapons_info})})
        -- request to hide div
...
RegisterServerEvent("ARMA:SearchNearest")
AddEventHandler("ARMA:SearchNearest", function()
  local player = source
  ARMAclient.getNearestPlayer(player,{5},function(nplayer)
    local nuser_id = ARMA.getUserId({nplayer})
    if nuser_id ~= nil then
...
-- armor item
ARMA.defInventoryItem({"body_armor","Body Armor","Intact body armor.",
function(args)
  local choices = {}
  ...
  if user_id ~= nil then
    BMclient.getArmour(player,{},function(armour)
      if armour > 95 then
	    ...
	    ARMAclient.notify(player, {"~r~Damaged armor can't be stored!"})
      end
    end)
  end
			...
			if custom ~= nil then
			  ...
			  if tonumber(custom) > 0 or ARMA.hasPermission({user_id,"admin.easy_unjail"}) then
				...
				if target ~= nil then
				  ...
				  ARMAbm.logInfoToFile("jailLog.txt",user_id .. " freed "..target_id.." from a " .. custom .. " minutes sentence")
				else
				  ARMAclient.notify(player,{"~r~That ID seems invalid."})
				end
			  else
...
-- (server) called when a logged player spawn to check for ARMA:jail in user_data
AddEventHandler("ARMA:playerSpawn", function(user_id, source, first_spawn) 
  local target = ARMA.getUserSource({user_id})
  SetTimeout(35000,function()
    local custom = {}
    ARMA.getUData({user_id,"ARMA:jail:time",function(value)
	  if value ~= nil then
	    custom = json.decode(value)
	    if custom ~= nil then
		  if tonumber(custom) > 0 then
			BMclient.loadFreeze(target,{false,false,false})
      TriggerClientEvent("ARMA:UnFreezeRespawn", source)
			SetTimeout(1,function()
			  BMclient.loadFreeze(target,{false,false,false})
			end)
			    ...
			    if reason ~= nil and reason ~= "" then 
				  ...
				  if target ~= nil then
		            ...
		            if ARMA.tryFullPayment({tonumber(target_id), tonumber(fine)}) then
					  ...
					  local user_id = ARMA.getUserId({player})
					  ARMAbm.logInfoToFile("fineLog.txt",user_id .. " fined "..target_id.." the amount of " .. fine .. " for ".. reason)
                      ARMA.closeMenu({player})
                    else
                      ARMAclient.notify(player,{lang.money.not_enough()})
                    end
...
local ch_handcuff = {function(player,choice)
  ARMAclient.getNearestPlayer(player,{10},function(nplayer)
    ...
    if nuser_id ~= nil then
	  ...
	  local user_id = ARMA.getUserId({player})
	  ARMAbm.logInfoToFile("jailLog.txt",user_id .. " cuffed "..nuser_id)
      ARMA.closeMenu({nplayer})
    else
      ARMAclient.notify(player,{lang.common.no_player_near()})
    end
  end)
    ...
    if player_lists[player] then -- hide
      ...
      for k,v in pairs(users) do
        ...
        ARMA.getUserIdentity({k, function(identity)
		  if source ~= nil then
            content = content.."<br /><span class=\"id\">"..k.."</span><span class=\"pseudo\">"..ARMA.getPlayerName({source}).."</span>"
            if identity then
              content = content.."<span class=\"name\">"..htmlEntities.encode(identity.firstname).." "..htmlEntities.encode(identity.name).."</span><span class=\"job\">"..ARMA.getUserGroupByType({k,"job"}).."</span>"
            end
          end
...
function ARMAbm.chargePhoneNumber(user_id,phone)
  local player = ARMA.getUserSource({user_id})
  local directory_name = ARMA.getPhoneDirectoryName({user_id, phone})
  if directory_name == "unknown" then
	directory_name = phone
  end
  ARMA.prompt({player,"Amount to be charged to "..directory_name..":","0",function(player,charge)
	if charge ~= nil and charge ~= "" and tonumber(charge)>0 then 
	  ARMA.getUserByPhone({phone, function(target_id)
		if target_id~=nil then
			if charge ~= nil and charge ~= "" then 
	          local target = ARMA.getUserSource({target_id})
			  if target ~= nil then
				ARMA.getUserIdentity({user_id, function(identity)
				  local my_directory_name = ARMA.getPhoneDirectoryName({target_id, identity.phone})
				  ...
				  ARMA.request({target,text,600,function(req_player,ok)
				    if ok then
					  local target_bank = ARMA.getBankMoney({target_id}) - tonumber(charge)
					  local my_bank = ARMA.getBankMoney({user_id}) + tonumber(charge)
		              if target_bank>0 then
					    ARMA.setBankMoney({user_id,my_bank})
					    ARMA.setBankMoney({target_id,target_bank})
					    ARMAclient.notify(player,{"You charged ~y~£"..charge.." ~w~from ~g~"..directory_name .."~w~ for your services."})
						ARMAclient.notify(target,{"~g~"..my_directory_name.."~w~ charged you ~r~£"..charge.."~w~ for his services."})
					    --ARMAbm.logInfoToFile("mchargeLog.txt",user_id .. " mobile charged "..target_id.." the amount of " .. charge .. ", user bank post-payment for "..user_id.." equals £"..my_bank.." and for "..user_id.." equals £"..target_bank)
					    ARMA.closeMenu({player})
                      else
                        ARMAclient.notify(target,{lang.money.not_enough()})
                        ARMAclient.notify(player,{"~g~" .. directory_name .. "~w~ tried to, but~r~ can't~w~ pay for your services."})
                      end
				    else
                      ARMAclient.notify(player,{"~g~" .. directory_name .. "~r~ refused~w~ to pay for your services."})
				    end
				  end})
				end})
			  else
			    ARMAclient.notify(player,{"~r~You can't make charges to offline players."})
			  end
			else
			  ARMAclient.notify(player,{"~r~Your charge has to have a value."})
			end
		else
		  ARMAclient.notify(player,{"~r~That phone number seems invalid."})
		end
	  end})
	else
	  ARMAclient.notify(player,{"~r~The value has to be bigger than 0."})
	end
  end})
end

function ARMAbm.payPhoneNumber(user_id,phone)
  local player = ARMA.getUserSource({user_id})
  local directory_name = ARMA.getPhoneDirectoryName({user_id, phone})
  if directory_name == "unknown" then
	directory_name = phone
  end
  ARMA.prompt({player,"Amount to be sent to "..directory_name..":","0",function(player,transfer)
	if transfer ~= nil and transfer ~= "" and tonumber(transfer)>0 then 
	  ARMA.getUserByPhone({phone, function(target_id)
	    local my_bank = ARMA.getBankMoney({user_id}) - tonumber(transfer)
		if target_id~=nil then
          if my_bank >= 0 then
		    local target = ARMA.getUserSource({target_id})
			if target ~= nil then
			  ARMA.setBankMoney({user_id,my_bank})
              ARMAclient.notify(player,{"~g~You tranfered ~r~£"..transfer.." ~g~to ~g~"..directory_name})
			  local target_bank = ARMA.getBankMoney({target_id}) + tonumber(transfer)
			  ARMA.setBankMoney({target_id,target_bank})
			  ARMAbm.logInfoToFile("mpayLog.txt",user_id .. " mobile paid "..target_id.." the amount of " .. transfer .. ", user bank post-payment for "..user_id.." equals £"..my_bank.." and for "..user_id.." equals £"..target_bank)
			  ARMA.getUserIdentity({user_id, function(identity)
		        local my_directory_name = ARMA.getPhoneDirectoryName({target_id, identity.phone})
			    if my_directory_name == "unknown" then
		          my_directory_name = identity.phone
			    end
                ARMAclient.notify(target,{"~g~You received ~y~£"..transfer.." ~g~from ~g~"..my_directory_name})
			  end})
              ARMA.closeMenu({player})
			else
			  ARMAclient.notify(player,{"~r~You can't make payments to offline players."})
			end
          else
            ARMAclient.notify(player,{lang.money.not_enough()})
          end
...
local ch_mobilepay = {function(player,choice) 
    ...
    menu.onclose = function(player) ARMA.openMainMenu({player}) end -- nest menu
	menu[">Type Number"] = {
	  -- payment function
	  function(player,choice) 
	    ARMA.prompt({player,"Phone Number:","000-0000",function(player,phone)
	      if phone ~= nil and phone ~= "" then 
		    ARMAbm.payPhoneNumber(user_id,phone)
		  else
		    ARMAclient.notify(player,{"~r~You have to digit a phone number."})
		  end
	    end})
	...
	for k,v in pairs(directory) do
	  menu[k] = {
	    -- payment function
	    function(player,choice) 
		  ARMAbm.payPhoneNumber(user_id,v)
	    end
...
local ch_mobilecharge = {function(player,choice) 
	...
	menu[">Type Number"] = {
	  ...
	  function(player,choice) 
	    ARMA.prompt({player,"Phone Number:","000-0000",function(player,phone)
	      if phone ~= nil and phone ~= "" then 
		    ARMAbm.chargePhoneNumber(user_id,phone)
		  else
		    ARMAclient.notify(player,{"~r~You have to digit a phone number."})
		  end
	    end})
	...
	for k,v in pairs(directory) do
	  menu[k] = {
	    -- payment function
	    function(player,choice) 
		  ARMAbm.chargePhoneNumber(user_id,v)
	    end
	  ,v} -- number as description
	end
	ARMA.openMenu({player, menu})
...
local ch_spawnveh = {function(player,choice) 
	ARMA.prompt({player,"Vehicle Model:","",function(player,model)
	  if model ~= nil and model ~= "" then 
	    BMclient.spawnVehicle(player,{model})
	  else
		ARMAclient.notify(player,{"~r~You have to type a vehicle model."})
	  end
	end})
...
local ch_freeze = {function(player,choice) 
	local user_id = ARMA.getUserId({player})
	if ARMA.hasPermission({user_id,"admin.bm_freeze"}) then
	  ARMA.prompt({player,"Player ID:","",function(player,target_id) 
	    if target_id ~= nil and target_id ~= "" then 
	      local target = ARMA.getUserSource({tonumber(target_id)})
		  if target ~= nil then
		    ARMAclient.notify(player,{"~g~You un/froze that player."})
...
-- lockpicking item
ARMA.defInventoryItem({"lockpicking_kit","Lockpicking Kit","Used to lockpick vehicles.", -- add it for sale to arma/cfg/markets.lua if you want to use it
function(args)
  local choices = {}
  ...
  choices["Lockpick"] = {function(player,choice)
    local user_id = ARMA.getUserId({player})
    if user_id ~= nil then
      if ARMA.tryGetInventoryItem({user_id, "lockpicking_kit", 1, true}) then
		BMclient.lockpickVehicle(player,{20,true}) -- 20s to lockpick, allow to carjack unlocked vehicles (has to be true for NoCarJack Compatibility)
        ARMA.closeMenu({player})
      end
    end
...
-- ADD STATIC MENU CHOICES // STATIC MENUS NEED TO BE ADDED AT ARMA/cfg/gui.lua
ARMA.addStaticMenuChoices({"police_weapons", police_weapons}) -- police gear
ARMA.addStaticMenuChoices({"emergency_medkit", emergency_medkit}) -- pills and medkits
ARMA.addStaticMenuChoices({"emergency_heal", emergency_heal}) -- heal button
...
local ch_player_menu = {function(player,choice)
    ...
    if ARMA.hasPermission({user_id,"player.fix_haircut"}) then
      menu["Fix Haircut"] = ch_fixhair -- just a work around for barbershop green hair bug while I am busy
    end
	
    if ARMA.hasPermission({user_id,"player.userlist"}) then
      menu["User List"] = ch_userlist -- a user list for players with ARMA ids, player name and identity names only.
    end
	
    if ARMA.hasPermission({user_id,"player.store_armor"}) then
      menu["Store armor"] = choice_store_armor -- store player armor
    end
	
    if ARMA.hasPermission({user_id,"player.check"}) then
      menu["Inspect"] = choice_player_check -- checks nearest player inventory, like police check from arma
    end
...
ARMA.registerMenuBuilder({"main", function(add, data)
  local user_id = ARMA.getUserId({data.player})
  if user_id ~= nil then
    local choices = {}
	
    if ARMA.hasPermission({user_id,"player.player_menu"}) then
      choices["Player"] = ch_player_menu -- opens player submenu
    end
	
    if ARMA.hasPermission({user_id,"toggle.service"}) then
      choices["Service"] = choice_service -- toggle the receiving of missions
    end
	
    if ARMA.hasPermission({user_id,"player.loot"}) then
      choices["Loot"] = choice_loot -- take the items of nearest player in coma
    end
	
    if ARMA.hasPermission({user_id,"mugger.mug"}) then
      choices["Mug"] = ch_mug -- steal nearest player wallet
    end
	
    if ARMA.hasPermission({user_id,"hacker.hack"}) then
      choices["Hack"] = ch_hack --  1 in 100 chance of stealing 1% of nearest player bank
    end
	
    if ARMA.hasPermission({user_id,"carjacker.lockpick"}) then
...
ARMA.registerMenuBuilder({"admin", function(add, data)
  local user_id = ARMA.getUserId({data.player})
  if user_id ~= nil then
    local choices = {}
	
	if ARMA.hasPermission({user_id,"admin.deleteveh"}) then
      choices["DeleteVeh"] = ch_deleteveh -- Delete nearest vehicle (Fixed pull request https://github.com/Sighmir/arma_basic_menu/pull/11/files/419405349ca0ad2a215df90cfcf656e7aa0f5e9c from benjatw)
	end
	
	if ARMA.hasPermission({user_id,"admin.spawnveh"}) then
      choices["SpawnVeh"] = ch_spawnveh -- Spawn a vehicle model
	end
	
	if ARMA.hasPermission({user_id,"admin.godmode"}) then
      choices["Godmode"] = ch_godmode -- Toggles admin godmode (Disable the default admin.god permission to use this!) 
	end
	
    if ARMA.hasPermission({user_id,"player.blips"}) then
      choices["Blips"] = ch_blips -- turn on map blips and sprites
    end
	
    if ARMA.hasPermission({user_id,"player.sprites"}) then
      choices["Sprites"] = ch_sprites -- turn on only name sprites
    end
	
    if ARMA.hasPermission({user_id,"admin.crun"}) then
      choices["Crun"] = ch_crun -- run any client command, any GTA V client native http://www.dev-c.com/nativedb/
    end
	
    if ARMA.hasPermission({user_id,"admin.srun"}) then
	...
	if ARMA.hasPermission({user_id,"admin.easy_unjail"}) then
	...
	if ARMA.hasPermission({user_id,"admin.spikes"}) then
...
ARMA.registerMenuBuilder({"police", function(add, data)
  local user_id = ARMA.getUserId({data.player})
  if user_id ~= nil then
    local choices = {}
	
	if ARMA.hasPermission({user_id,"police.easy_jail"}) then
      choices["Easy Jail"] = ch_jail -- Send a nearby handcuffed player to jail with prompt for choice and user_list
    end
	
	if ARMA.hasPermission({user_id,"police.easy_unjail"}) then
	...
	if ARMA.hasPermission({user_id,"police.easy_cuff"}) then
      choices["Easy Cuff"] = ch_handcuff -- Toggle cuffs AND CLOSE MENU for nearby player
    end
	
	if ARMA.hasPermission({user_id,"police.spikes"}) then
      choices["Spikes"] = ch_spikes -- Toggle spikes
    end
	
    if ARMA.hasPermission({user_id,"police.drag"}) then
      choices["Drag"] = ch_drag -- Drags closest handcuffed player
    end
	
	if ARMA.hasPermission({user_id,"police.bm_freeze"}) then
      choices["Freeze"] = ch_freeze -- Toggle freeze
    end
...
-- TO USE THIS FUNCTION YOU NEED TO HAVE THE ORIGINAL ARMA UPDATED TO THE LASTEST VERSION
ARMA.registerMenuBuilder({"phone", function(add) -- phone menu is created on server start, so it has no permissions.
