...
AddEventHandler('showBlips', function()
	...
	if showblip then
		showsprite = true
		ARMA.notify({"~g~Blips enabled"})
	else
		showsprite = false
		ARMA.notify({"~r~Blips disabled"})
	end
end)
...
AddEventHandler('showSprites', function()
	...
	if showsprite then
		ARMA.notify({"~g~Sprites enabled"})
	else
		ARMA.notify({"~r~Sprites disabled"})
	end
end)
	...
	while true do
		...
		for id = 0, 64 do
			...
			if NetworkIsPlayerActive( id ) and GetPlayerPed( id ) ~= GetPlayerPed( -1 ) then
				...
				wantedARMA = GetPlayerWantedLevel( id )
				...
				if showsprite then
					...
					if wantedARMA then
						...
						Citizen.InvokeNative( 0xCF228E2AA03099C3, headId, wantedARMA ) -- Set wanted number
